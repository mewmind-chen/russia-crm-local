...
| 交叉发布 FR-01：模块化前端首轮双外壳 | 发布候选 | `codex/frontend-modular-refactor-design` @ `660345a` | 35 项前端能力全部映射；聚焦 31/31、完整回归 615/615、JavaScript 语法和 diff 检查通过；管理员/经理/销售、桌面/390px、旧深链、客户详情、AI 分层和移动无障碍验收通过；证据见 `docs/evidence/frontend-modular-refactor-stage-gate.md` | 发布前 `79800f529d1d` + `CRM_UX_REDESIGN_ENABLED=false` | 首次发布保留旧外壳；生产 smoke 后开新外壳，稳定观察后另行退役；不改变 35/38 和 R5-01 准入 |
