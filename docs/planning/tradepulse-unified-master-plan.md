...
**文档状态：** 17/38 个任务已完成；A1-08 进行中，A1-08.1 至 A1-08.4 已完成；下一步执行 A1-08.5 权限、审计和降级
**版本：** v1.3
**日期：** 2026-07-24
...
| 阶段 1：评分、Control Plane 与客户补全 | 进行中 | 生产 `92e9f609`；开发集成 `codex/ai-integration` @ `87d9942d` | A1-01 至 A1-07 已部署；A1-08.1 持久队列/DAG/取消/恢复和独立 Worker 由 PR #17 合并；A1-08.2 数据库全局与资源槽位、速率窗口、公平调度、每客户串行和 Router 引擎调用期占位由 PR #18 合并，6 个独立 Worker 进程完成 20 个跨客户任务且全局峰值 4，全量 307/307；A1-08.3 四级预算、原子预占/结算、80% 告警、100% 阻断、attempt 级 usage/cost/fallback 台账、缺失 usage 保守估算、缓存/去重零费用记录和孤儿预占恢复由 PR #20 合并，全量 323/323；A1-08.4 统一聚合 AI Station、Recon、Prospect、经理评价和对话 AI，提供分页筛选、详情时间线、队列/并发/预算/失败率指标及权限化重试/取消/复核，由 PR #22 和 #23 合并，专项 18/18、全量 330/330、GitHub CI 通过；生产 current/health 不变，AI Station 和 Worker 仍关闭且未部署 | `releases/2b55ed0fb7fc` + 部署前备份 | A1-08 整体仍进行中；下一步 A1-08.5 权限/审计/降级，之后执行 A1-09 新客户自动补全 |
...
当前总进度：38 个计划任务中已完成 17 个，剩余 21 个。A1-08.1 至 A1-08.4 已完成但 A1-08 仍进行中，因此顶层任务计数不增加；下一步为 A1-08.5 权限、审计和降级。任何范围、目录、数据模型、上线门或时间目标的改变，都必须先修改本文档并重新确认，不能只在临时消息中改变执行方向。
