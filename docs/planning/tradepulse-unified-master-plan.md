...
**文档状态：** 17/38 个任务已完成；A1-08 进行中，A1-08.1 至 A1-08.2 已完成；下一步执行 A1-08.3 用量、费用和预算
**版本：** v1.1
**日期：** 2026-07-24
...
| 阶段 1：评分、Control Plane 与客户补全 | 进行中 | 生产 `92e9f609`；开发集成 `codex/ai-integration` @ `0a62398d` | A1-01 至 A1-07 已部署；A1-08.1 持久队列/DAG/取消/恢复和独立 Worker 已由 PR #17 合并；A1-08.2 数据库全局与资源槽位、速率窗口、公平调度、每客户串行、Router 引擎调用期占位和环境配置已由 PR #18 合并，6 个独立 Worker 进程完成 20 个跨客户任务且全局峰值 4，专项 49/49、全量 307/307、GitHub CI 通过；生产 current/health 不变，AI Station 和 Worker 仍关闭且未部署 | `releases/2b55ed0fb7fc` + 部署前备份 | A1-08 整体仍进行中；下一步 A1-08.3 用量、费用和预算，之后完成任务中心和 A1-09 新客户自动补全 |
...
当前总进度：38 个计划任务中已完成 17 个，剩余 21 个。A1-08.1 至 A1-08.2 已完成但 A1-08 仍进行中，因此顶层任务计数不增加；下一步为 A1-08.3 用量、费用和预算。任何范围、目录、数据模型、上线门或时间目标的改变，都必须先修改本文档并重新确认，不能只在临时消息中改变执行方向。
