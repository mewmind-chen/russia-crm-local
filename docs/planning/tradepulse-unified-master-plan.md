...
**文档状态：** 35/38 个任务已完成；A4-04 已完成开发、CI 和生产发布；下一项 R5-01
**版本：** v2.0
**日期：** 2026-07-25
...
| 阶段 4：经理与反馈 | 已完成 | 功能 SHA `e1d3e611f5ef`；发布记录 SHA `634372f750d7` | A4-01 至 A4-04 已发布；经理授权范围、销售团队数据隔离、离线版本治理、新旧指标比较和历史决策追溯验收通过。schema v16 固化任务版本快照，管理端显示白名单化版本、上下文、证据和 stale 状态；聚焦 40/40、完整回归 535/535、GitHub CI、桌面/390px、生产备份/quick_check、schema v16 与 local/public smoke 通过；PR #87/#88 已合并 | `previous` + AI 独立开关；Qwen Batch 价格/汇率保护保持关闭 | 阶段 4 已完成；下一步 R5-01 影子运行 |
| 交叉发布 FR-01：模块化前端首轮双外壳 | 已发布 | PR #90；`main`/生产 `9aaa8b38c85d93eaa5f1a9244538dd670ea065e6` | 35 项前端能力全部映射；聚焦 31/31、完整回归 615/615、PR/main CI、JavaScript 语法和 diff 检查通过；管理员/经理/销售独立库验收及生产桌面/390px未登录 smoke 通过；自动备份和活动库 `quick_check=ok`，local/public health 为目标 SHA；证据见 `docs/evidence/frontend-modular-refactor-stage-gate.md` | `releases/79800f529d1d` + `CRM_UX_REDESIGN_ENABLED=false` | 新外壳已开启，旧外壳继续保留用于快速回退；稳定观察后另行退役；不改变 35/38 和 R5-01 准入 |
| 阶段 5：生产试运行 | 未开始 | 待填写 | 待填写 | previous release + flags | 分批开放 |

当前总进度：38 个计划任务中已完成 35 个，剩余 3 个。A4-04 已完成开发、GitHub CI、
