...
| FR-01 | 发布候选 | 模块化前端首轮双外壳发布：统一客户详情七标签、AI 五层业务呈现、三角色 AI 权限边界、旧深链兼容和移动无障碍完成；实现提交 `660345a`，聚焦 31/31、完整回归 615/615、35 项能力零遗漏、JavaScript 语法/diff、管理员/经理/销售桌面与 390px 验收通过；首次部署保持 `CRM_UX_REDESIGN_ENABLED` 关闭，旧外壳和 local/public health 通过后再开启，回滚点为发布前 `79800f529d1d`；证据见 `docs/evidence/frontend-modular-refactor-stage-gate.md`；不改变 35/38 和下一项 R5-01 |
