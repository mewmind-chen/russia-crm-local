...
> **执行方式：** 按依赖关系拆成并发波次。文件边界不冲突的任务由多个代理同时实现；每个波次完成后只做一次集中代码审查和验证。复选框用于跟踪进度。
...
- 同一波次内任务并行实现；共享文件有冲突的任务必须分到不同波次。
- 每个波次只做一次集中审查，聚焦测试未通过不得进入下一波次。
- 多代理直接在共享工作区协作，代理不得提交 git；主代理集中审查、验证和提交。
- 保留任何无关用户修改，不得回滚或覆盖。
- Task 14 前不得部署生产。
...
1. 波次 1：Task 1（等价基线）、Task 2（浏览器核心）、Task 10（AI 来源）并发。
2. 波次 2：Task 3（路由注册）、Task 4（service/按需加载）并发；Task 5 在两者完成后接入。
3. 波次 3：Task 6（角色首页）、Task 7（客户流程）、Task 9（管理模块）按独立目录并发。
4. 波次 4：Task 8（统一详情）、Task 11（AI 业务呈现）并发开发，最后集中处理共享详情文件。
5. 波次 5：Task 12（AI 角色权限）与 Task 13（浏览器门）并发。
6. 波次 6：Task 14 单独执行生产切换和旧外壳退役。
