...
> 自动生成于 `2026-09-01 09:26:16`；运行 `npm run board` 手动重新生成，每个切片收尾自动更新。
...
| HEAD | `54950d9`（相对 origin/main ahead 285） |
| 工作区 | 干净 |
| 全量测试 | `node --test` 2071/2071 |
| 核心测试 | `npm test` 1709/1709 |
| sales_crm.js | 12945 行 |
| lib/domains | 44 个文件，生产接线 41 个 |
| 最近会话 | `2026-09-01-list-widget-maintenance-runs.md` |
...
- [ ] **frontend** 跟进更正历史只读列表迁移；明确排除 AI 字段，保留既有筛选/分页与 target/proposal/review 审批流
- [ ] **frontend** 用户/归档用户、审计/迁移复核列表迁移；先拆分 bootstrap 数据与高风险行操作边界
