...
## 7.5 字段级自由显示与统一列表组合层（Field Schema + List Widget）

用户需求：不止 UI 结构，**所有业务列表页面**的字段内容、列顺序和行排序都要能按用户自由配置；客户资料等详情视图继续复用同一字段 schema。
...
Widget 按字段 schema 渲染列/详情/表单（无硬编码字段名）
        ↓
统一 List widget（所有列表页共用）
  - 授权字段中的列显隐
  - 列顺序与布局
  - 授权排序字段、升降序与稳定次序
        ↓
用户级列表偏好（强制目标）：visibleColumns / columnOrder / sort，配置驱动，无需改代码
```

覆盖页面：客户列表、线索池（`intake/lead_flow`）、管道、告警、洞察、回收站、主管任务/风险/指标、通知，以及后续纳入统一壳的联系人、Recon 等所有表格/列表结果页。客户资料（`/profile/:customerId`）和抽屉/时间线等详情组件复用字段 schema，但不强制套用列表布局。

统一列表目标约束：

- 可选列 = 服务端有效字段 schema 与用户偏好的交集；偏好不能扩大权限，也不能使未授权字段下发。
- 列顺序、显隐和排序偏好按用户保存；可提供角色默认值，但不能覆盖用户自己的配置。
- 行排序由服务端白名单字段执行，至少支持升序/降序，并以稳定主键作最终 tie-breaker；非法字段或过期 schema 必须拒绝或回退到默认值。
- 列表展示偏好只改变呈现与排序，不改变数据范围、筛选授权、导出权限或业务动作权限。
- 该目标不引入生成式内容、推荐列或其他智能功能。

效果：
- 字段显隐 = 改配置（角色/权限/开关或用户偏好），不改代码。
- 所有列表页共享同一列配置、排序和偏好协议，新增列表字段不再复制页面级硬编码。
