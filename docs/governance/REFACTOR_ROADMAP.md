...
执行分支：`codex/frontend-widget-pilot@13c5368`（相对基线 ahead 87，未合并）
状态：路线图执行中；阶段 B §1 完成门达成（lib/ 对 crm_accounts 状态/计划/主管列零裸写），阶段 A 接线恢复推进至 25 个模块已接入，全绿
...
| 阶段 A：后端结构化切分 | 接线恢复中（25 模块已接入） | `lib/domains/` 42 个文件；25 个域模块已重新接入 sales_crm.js（7 个接线切片、17 契约断言），sales_crm.js 13,352 行 | 剩余模块需逐个核验（部分按裁定保持内联、部分已漂移需同步调用点、部分仅函数级一致）；聚合文件仍超 1.3 万行 |
...
2. 阶段 A 接线恢复 7 个切片已落地（`0560e9c`/`d51596c`/`873d1b0`/`7328b51`/`48ba93c`/`ad657ac`/`13c5368`：25 个域模块重新接入，含 4 处 csvSerialize 调用点），接线契约 17/17；全量 1902/1902 绿灯。
3. 继续阶段 A：全量逐字核验结论已固化——14 个模块可 drop-in、6 个函数级部分可接、约 15 个不宜接线（已接线 10 + 用户裁定保持内联的 identity/filter/middleware + 已漂移的 normalize/create/progress/request/errors/today_task）。下一批优先 B 组函数级接线（`commerce/rules` 的 advanceStage/idempotencyKey、`customer/recycle` 的 manualReturnBatchId）。
