...
执行分支：`codex/frontend-widget-pilot@873d1b0`（相对基线 ahead 80，未合并）
状态：路线图执行中；阶段 B §1 完成门达成（lib/ 对 crm_accounts 状态/计划/主管列零裸写），阶段 A 接线恢复首批落地，全绿
...
| 阶段 A：后端结构化切分 | 接线恢复中（首批落地） | `lib/domains/` 42 个文件；7 个纯 helper 域模块已重新接入 sales_crm.js | 其余被回退模块待逐块核验恢复；聚合文件仍超 1.3 万行 |
...
2. 阶段 A 接线恢复首批已落地（`0560e9c`/`d51596c`/`873d1b0`：8 个纯 helper 域模块 + 4 处 csvSerialize 调用点），接线契约 8/8；全量 1889/1889 绿灯。
3. 继续阶段 A：逐块核验被 WIP 回退的 customer/activity/planning/intake/commerce/auth/reporting-builders 等域模块与内联版一致性，一致者 drop-in 恢复。
