...
14. 阶段 E 最新恢复点：`e59bf22` profile-only 只读兼容契约、`8a86425` 独立 widget host 隔离、`3adc1d1` identity/source tags UMD、`c246360` List widget 协议、`ed40d76` 主管指标列表、`a52e42b` Team 三类业务列表、`75a30b7` Insights 人工评价列表、`f1fe7d1` 受保护客户目录、`6001f61` 维护运行记录、`61a6572` 跟进更正历史只读列表、`3e55b41` 审计只读列表、`8d1bb05` 账号/归档/权限组/迁移复核/入库批次列表均已落地；本轮列表/访问控制/API 定向 `62/62`、core `npm test` `1713/1713`、全量 `node --test` `2074/2074`，`node --check`、diff check 通过，治理/AI boundary 待本 checkpoint 复跑。权限配置矩阵、事务预览/审核工作区明确为专用组件；AI 功能弃用冻结。阶段 E 仍进行中，浏览器双角色验收待执行。
