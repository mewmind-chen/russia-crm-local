...
| 重构后/开发中 | `/Users/ylf/Desktop/projects/tradepulse-refactor/after` | `codex/frontend-widget-pilot@3e55b41`，受保护客户目录、维护运行记录、跟进更正历史与审计只读列表已迁移，前序业务列表与 Phase E 隔离预览 harness 均已提交；工作区当前干净 | 当前唯一重构开发入口 |

- 远程：`https://github.com/mewmind-chen/russia-crm-local.git`
- 当前 `origin/main`：`57c4c42a89e7730545b726b29fd932c5bfb20574`
- 当前重构提交：`3e55b41`（阶段 E：审计只读列表已迁移到 List widget，使用人工字段目录、按用户隔离的列显隐/顺序/布局偏好和当前 bootstrap 结果本地安全排序；保留 `view_users` 权限、既有审计脱敏与详情截断，不改后端查询/写入；跟进更正历史、维护运行记录、受保护客户目录与前置业务列表均已迁移；隔离 preview harness 保持显式 opt-in、临时 SQLite/loopback/随机端口、AI provider/monitor 关闭、浏览器依赖缺失 fail-closed）
...
`after/` 当前业务提交 `3e55b41`（审计只读列表迁移；跟进更正历史、维护运行记录与受保护客户目录为前序业务提交）与前置治理提交已落地；本治理 checkpoint（`CURRENT_STATE.md`、路线图、看板生成器、生成看板与本次 session 记录）随独立治理提交落地。生产目录保持只读。
...
- `npm test`：全量 core `1711/1711` 通过。
- `node --test`：全量 `2073/2073` 通过。
- 本轮 `3e55b41`：审计/更正历史/维护 runs/受保护目录/API/List widget 与访问控制定向 `60/60`；core `1711/1711`、全量 `2073/2073`；`node --check`、`git diff --check`、`npm run check:governance-authority`、`npm run check:ai-boundary` 均通过。真实浏览器双角色验收未执行（依赖未锁定时入口 fail-closed），生产或部署验证未执行。
...
下一动作固定为：迁移用户/归档用户与迁移复核列表，先拆分 bootstrap 数据与高风险行操作边界；审计只读表已先完成。具备锁定浏览器依赖后再运行 harness 完成 sales/manager 验收。
...
国家矩阵/分配批次/细分报表
列表已接入同一协议，均由服务端授权字段目录约束；仍不触碰 AI。随后 Team 进度销售汇总、客户/待办/事实时间线明细与协作支持列表迁移（`a52e42b`），Insights 人工评价列表迁移（`75a30b7`）、受保护客户目录迁移（`f1fe7d1`）、维护运行记录迁移（`6001f61`）、跟进更正历史迁移（`61a6572`）与审计只读列表迁移（`3e55b41`）均保留原有筛选、分页和权限，已有钻取/导出/行操作继续保持；审计仍使用原有 bootstrap 只读数据。用户/归档用户与迁移复核列表仍待迁移。Phase E 浏览器预览 harness 已建立（显式 opt-in、临时 SQLite、`127.0.0.1`、随机端口、AI provider/monitor 关闭、缺失浏览器依赖 fail-closed），真实浏览器验收尚未运行。
