...
| 重构后/开发中 | `/Users/ylf/Desktop/projects/tradepulse-refactor/after` | `codex/frontend-widget-pilot@807b56c`，Markets、主管任务/风险列表迁移与 Phase E 隔离预览 harness 均已提交；工作区干净 | 当前唯一重构开发入口 |

- 远程：`https://github.com/mewmind-chen/russia-crm-local.git`
- 当前 `origin/main`：`57c4c42a89e7730545b726b29fd932c5bfb20574`
- 当前重构提交：`807b56c`（阶段 E：Markets 国家矩阵、分配批次、细分报表、主管任务与风险明细列表已迁移到 List widget；并建立隔离 preview harness，显式 opt-in、临时 SQLite/loopback/随机端口、AI provider/monitor 关闭、浏览器依赖缺失 fail-closed；前置 Dashboard、customers、Research People、Research Recon、不对口记录、Pipeline、Intake/lead_flow、Alerts/今日待办与通知中心样板已完成）
...
- 当前验证（2026-09-01）：manager_tasks/manager_risks/List/field-schema/回归定向 `41/41`，影响面权限/筛选回归 `116/116`，Phase E harness 契约 `4/4`，`npm test`：全量 core `1699/1699`、`node --test`：全量 `2060/2060`；`node --check`、`git diff --check`、治理权威门禁与 AI 边界门禁均通过。
- 重构分支未合并；本轮未执行浏览器双角色验收、生产验证或部署。
- 旧目录 `/Users/ylf/Desktop/projects/tradepulse-development` 只保留为迁移来源，不再作为当前权威路径。
- 用户新增目标（2026-09-01）：所有业务列表页统一支持按用户配置列显隐、列顺序、升降序/多级排序和布局偏好；配置只能在服务端授权字段范围内生效，不引入智能内容或推荐功能。本轮已完成通用协议、Dashboard 国家快照、Markets 国家矩阵/分配批次/细分报表、manager_tasks、manager_risks、customers、Research People、Research Recon、不对口记录、Pipeline、Intake/lead_flow、Alerts/今日待办与通知中心列表迁移；其余业务列表页仍按页面逐步迁移。
...
CRM 活动时间线评估下沉。隔离 preview/mock runtime 已建立为显式 opt-in harness；当前
环境缺少锁定浏览器依赖，浏览器入口 fail-closed，真实浏览器验收不得写成已通过。
下一动作固定为：在具备锁定浏览器依赖的环境运行该 harness，完成 sales/manager 验收；其后
才处理剩余复杂主体/activity timeline。
