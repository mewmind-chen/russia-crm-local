...
- 当前验证摘要（2026-09-03）：新增 S4/P4 account/commerce/timeline 动态字段契约后，`node --test` 全量 `2143/2143`、`npm test` core `1781/1781`；本轮专项 `node --test test/phase_c_s4_dynamic_fields_contract.test.js` `4/4`，masterProfile 专项 `4/4`，S6 专项 `5/5`，S4 首轮专项 `4/4`，S7 专项 `3/3`。动态契约覆盖 hydrated object/array、嵌套/双层 JSON 文本、常见联系人别名、malformed JSON、受限回收/初始 aggregate 和 admin 原样保留；既有 Stage C P1/P3 递归脱敏、S5/P5 凭据、legacy customer whitelist、阶段 G 路由/权限回归及浏览器验收证据均通过。`npm run phase:e:browser-preview` 在隔离 SQLite、loopback、AI 关闭环境下 Playwright `1.62.1` 双角色通过（默认 widget、无 legacy iframe、profile-only 只读）；治理权威、AI 边界、语法与差异门禁均已复核通过。AI runtime、AI 专用 UI 与生产目录继续保持冻结/只读。
