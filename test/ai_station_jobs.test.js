...
test('AI schema installation is idempotent and leaves identity/router tables unchanged', () => {
  ...
  installAIStationSchema(db);
  assert.equal(db.prepare("SELECT count(*) count FROM sqlite_master WHERE type='table' AND name LIKE 'crm_ai_%'").get().count, 19);
...
test('AI schema migration is serialized across concurrent processes', async t => {
  ...
  assert.equal(verified.prepare('SELECT MAX(version) version FROM crm_ai_schema_migrations').get().version, 5);
  assert.equal(verified.prepare("SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name LIKE 'crm_ai_%'").get().count, 19);
  verified.close();
});
