...
test('sales can return an owned CRM customer and manager can reassign the same account', async t => {
  ...
  assert.deepEqual(
    ...
    { owner_id: null, lifecycle_status: 'recycled', recycle_kind: 'sales_return', assignment_status: 'returned' },
  );
  ...
  assert.equal(bin.rows.some(item => item.customerId === 'CRM-OTHER'), true);

  const reassigned = await fx.request('/api/sales-crm/accounts/CRM-OTHER/reassign', {
  ...
  assert.deepEqual(
    ...
    { id: 'CRM-OTHER', owner_id: 'U-OTHER', lifecycle_status: 'active', recycle_kind: '', assignment_status: 'assigned' },
  );
});
