...
test('Issue 209 recycle-kind filter only exposes mismatch and manual delete in Chinese', async t => {
  ...
  recycleAccount.run('mismatch', '2026-08-04 08:02:00', 'CRM-OTHER');
  ...
  assert.equal(labels.mismatch, '不对口');
  assert.equal(labels.manual_delete, '手动删除');
  assert.deepEqual(Object.keys(labels).sort(), ['manual_delete', 'mismatch']);
  assert.equal(kind.options.some(option => option.value === 'sales_return'), false);
});
