...
“未开发线索每日分配”会在每日筛选交付完成后自动运行。`customer_pool` 中的企业
统一视为未开发线索，不计入 CRM 客户；A/B/C/D 组线索（不限 L0/L1/L2/L3）
去重后全部进入分配中心，保留联系质量、筛选证据和背调入口。风险拦截项进入
待审核队列，其余线索按规则自动推送。
系统按国家、语言、联系渠道、当前负荷和每日配额分配销售，但此时只创建待领取
任务；销售确认领取后才创建 CRM 客户。默认每人每天 5 个、24 小时内领取、
领取后 48 小时内首次触达。管理员可切换为审核模式，也可以批量按规则分配。
