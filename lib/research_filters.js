...
function researchFilterOptions(db, user, pageKey, authorizedFields) {
  const config = pageConfig(pageKey);
  assertPageAccess(user, config);
  const definitions = DEFINITIONS_BY_PAGE.get(config.pageKey);
  const result = {};
  for (const fieldKey of normalizeAuthorizedFieldKeys(authorizedFields)) {
    ...
    const scopeParams = [];
    const ownerCondition = researchOwnerCondition(user, config.alias, scopeParams);
    const conditions = [
      `TRIM(COALESCE(CAST(${expression} AS TEXT),''))!=''`,
      ownerCondition,
    ].filter(Boolean);
    result[fieldKey] = db.prepare(`SELECT CAST(${expression} AS TEXT) value,COUNT(*) count
      FROM ${config.from}
      WHERE ${conditions.join(' AND ')}
      GROUP BY ${expression}
      ORDER BY value COLLATE NOCASE`).all(...scopeParams).map(row => ({
