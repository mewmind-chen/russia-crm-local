...
function customerActivities(db, accountId) {
  ...
  return db.prepare(`SELECT ${selected.join(',')} FROM crm_activities
    WHERE customer_id=? ORDER BY occurred_at,created_at,id`).all(accountId);
}
