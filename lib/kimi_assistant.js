...
function effectiveKimiTimeout(options = {}) {
  const config = kimiConfig();
  return boundedInteger(options.timeoutMs, config.timeoutMs, 1000, 120000);
}
...
async function callKimi(messages, options = {}) {
  const config = kimiConfig();
  const effectiveTimeout = effectiveKimiTimeout(options);
      ...
      execFile(config.bin, args, {
        ...
        if (error) {
          ...
          const wrapped = new Error(
            error.killed
              ? `Kimi 请求超过 ${Math.round(effectiveTimeout / 1000)} 秒，已停止。`
