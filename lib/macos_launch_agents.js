...
const AUTO_DEPLOY_NODE_RUNNER = [
  'const { spawnSync } = require("node:child_process");',
  'const result = spawnSync("/bin/zsh", [process.argv[1]], { stdio: "inherit", env: process.env });',
...
function buildServiceDefinitions(options) {
  ...
  const services = [
    ...
    base('com.russia-crm.ai-station-worker', 'code', [
      nodeBin,
      path.join(runtimeRoot, 'scripts', 'ai-station-worker.js'),
    ], {
      environment: { HOME: homeDir, PATH: SYSTEM_PATH, NODE_ENV: 'production' },
      runAtLoad: true,
      keepAlive: true,
    ...
    base('com.russia-crm.qwen-batch-worker', 'code', [
      nodeBin,
      path.join(runtimeRoot, 'scripts', 'qwen-batch-worker.js'),
    ], {
      environment: { HOME: homeDir, PATH: SYSTEM_PATH, NODE_ENV: 'production' },
      runAtLoad: true,
      startInterval: 300,
