...
function resolutionIntegrityValid(
  ...
  if (row.decision === 'confirm_new' && details.resolutionKind === 'distinct_sources') {
    const ids = parseJson(row.latest_external_customer_ids_json, []);
    if (ids.length < 2 || !ids.includes(row.target_external_customer_id)) return false;
    if (details.comparisonEvidenceVersion !== identityComparisonEvidenceVersion(db, ids)) {
      return false;
    }
    return Boolean(db.prepare(`SELECT 1 FROM crm_customer_identity_conflict_audit
...
function optionalSelect(columns, available) {
  return columns.map(column => available.has(column) ? column : `'' AS ${column}`).join(',');
}

function identityRecords(db, row) {
  const ids = [...new Set((row.externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean))];
  if (!ids.length) return [];
  const placeholders = ids.map(() => '?').join(',');
  const poolColumns = tableColumns(db, 'customer_pool');
  const crmColumns = tableColumns(db, 'crm_accounts');
  const poolRows = new Map(db.prepare(`SELECT ${optionalSelect([
    'customer_id', 'company_name', 'nickname', 'country', 'city', 'website', 'email', 'industry',
    'created_at', 'updated_at',
  ], poolColumns)} FROM customer_pool WHERE customer_id IN (${placeholders})`).all(...ids)
    .map(item => [String(item.customer_id), item]));
  const crmRows = new Map(db.prepare(`SELECT ${optionalSelect([
    'external_customer_id', 'company_name', 'nickname', 'country', 'city', 'website', 'industry',
    'created_at', 'updated_at',
  ], crmColumns)} FROM crm_accounts WHERE external_customer_id IN (${placeholders})`).all(...ids)
    .map(item => [String(item.external_customer_id), item]));
  const contactRows = new Map();
  const contactColumns = tableColumns(db, 'crm_account_contacts');
  if (contactColumns.has('external_customer_id') && contactColumns.has('email')) {
    const archivedClause = contactColumns.has('archived_at')
      ? "AND TRIM(COALESCE(archived_at,''))=''" : '';
    const orderClause = contactColumns.has('updated_at')
      ? `ORDER BY updated_at DESC${contactColumns.has('id') ? ',id DESC' : ''}` : '';
    for (const contact of db.prepare(`SELECT ${optionalSelect(
      ['external_customer_id', 'email', 'updated_at'], contactColumns,
    )} FROM crm_account_contacts WHERE external_customer_id IN (${placeholders})
        AND TRIM(COALESCE(email,''))!='' ${archivedClause} ${orderClause}`).all(...ids)) {
      const id = String(contact.external_customer_id || '');
      if (!contactRows.has(id)) contactRows.set(id, contact);
    }
  }
  const aliasNames = new Map();
  for (const alias of row.aliases || []) {
    const id = String(alias?.externalCustomerId || '');
    if (id && !aliasNames.has(id)) aliasNames.set(id, String(alias?.rawName || ''));
  }
  return ids.map(externalCustomerId => {
    const pool = poolRows.get(externalCustomerId) || {};
    const crm = crmRows.get(externalCustomerId) || {};
    const contact = contactRows.get(externalCustomerId) || {};
    const recordType = crm.external_customer_id ? 'crm' : 'lead';
    return {
      externalCustomerId,
      recordType,
      companyName: crm.company_name || pool.company_name || aliasNames.get(externalCustomerId) || '',
      nickname: crm.nickname || pool.nickname || aliasNames.get(externalCustomerId) || '',
      country: crm.country || pool.country || '',
      city: crm.city || pool.city || '',
      website: crm.website || pool.website || '',
      email: contact.email || pool.email || '',
      industry: crm.industry || pool.industry || '',
      createdAt: crm.created_at || pool.created_at || '',
      updatedAt: crm.updated_at || pool.updated_at || '',
    };
  }).sort((left, right) => {
    if (left.recordType !== right.recordType) return left.recordType === 'lead' ? -1 : 1;
    return String(right.updatedAt || right.createdAt).localeCompare(String(left.updatedAt || left.createdAt))
      || right.externalCustomerId.localeCompare(left.externalCustomerId);
  });
}
...
function attachIdentityGrouping(db, row, resolutions) {
  ...
  return {
    ...
    identityRecords: identityRecords(db, row),
    complementaryInfo,
  };
}
...
function listProtectedIdentityConflicts(db, user, options = {}) {
  ...
  const rows = rawRows.map(row => attachIdentityGrouping(db, row, resolutions))
...
function validateTarget(row, decision, targetExternalCustomerId, liveConflict, liveReport) {
  ...
  if (liveConflict) {
    if (!liveConflict.externalCustomerIds.includes(targetExternalCustomerId)) {
      throw privateError(
        '所选客户编号不属于该冲突',
        'PROTECTED_IDENTITY_CONFLICT_TARGET_INVALID',
        422,
...
function resolveProtectedIdentityConflict(db, user, payload = {}) {
  ...
  let details = detailsValue(payload.details);
  delete details.resolutionKind;
  delete details.comparisonEvidenceVersion;
  ...
  return db.transaction(() => {
    ...
    const distinctLiveConfirmation = decision === 'confirm_new' && Boolean(liveConflict);
    if (distinctLiveConfirmation) {
      details = {
        ...details,
        resolutionKind: 'distinct_sources',
        comparisonEvidenceVersion: identityComparisonEvidenceVersion(
          db, currentSource.externalCustomerIds,
        ),
      };
    }
    const decisionRow = {
    ...
    if (status === 'resolved' && !zeroSourceConfirmation && !distinctLiveConfirmation) {
