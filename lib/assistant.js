...
function applyAssistantScope(db, accessContext) {
  ...
  installScopedView(db, 'recon_jobs', { empty: !canViewRecon });
...
function assistantEngine() {
  const value = String(process.env.ASSISTANT_ENGINE || 'hermes').trim().toLowerCase();
...
async function callAssistantModel(messages, options = {}) {
  ...
  if (engine === 'deepseek') {
    ...
    return { ...result, engine: 'deepseek' };
  }
  if (engine === 'kimi-cli') return callKimi(messages, options);
  try {
    return await callHermes(messages, options);
  } catch (error) {
    const fallbackEnabled = String(process.env.ASSISTANT_HERMES_FALLBACK || 'true').toLowerCase() !== 'false';
    if (!fallbackEnabled || !process.env.DEEPSEEK_API_KEY) throw error;
    const fallback = await callDeepSeek(messages);
    return {
      ...fallback,
      model: `${fallback.model} · Hermes fallback`,
      engine: 'deepseek_fallback',
      fallbackReason: error.code || error.message || 'hermes_failed',
...
async function currentCustomerAssistantAnswer(message, contextPayload = {}, history = [], sessionId = '') {
...
async function answerAssistantQuestionScoped(payload = {}) {
  ...
  if (kind === 'current_customer') {
    const current = await currentCustomerAssistantAnswer(retrievalMessage, contextPayload, history, sessionId);
  ...
  const vector = await semanticContext(retrievalMessage, contextPayload, assistantEngine() === 'hermes');
...
async function answerAssistantQuestion(payload = {}, accessContext = null) {
  assertAssistantTargetAccess(payload, accessContext);
  const result = await assistantAccess.run(accessContext, () => answerAssistantQuestionScoped(payload));
