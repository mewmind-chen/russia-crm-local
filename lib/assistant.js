...
const { callQwen } = require('./qwen_assistant');
...
const { readExistingFileWithinRoot } = require('./report_files');
const { databasePath, runtimePaths } = require('./runtime_paths');
...
function safeReadLocalReport(reportPath, reportRoot) {
  ...
  if (!value) return '';
  try {
    const report = readExistingFileWithinRoot(
      reportRoot,
      value,
      ['.html', '.htm', '.md', '.markdown', '.txt'],
    );
    if (!report) return '';
    const raw = report.content.toString('utf8').slice(0, 250000);
    const text = /\.html?$/i.test(report.path) ? htmlToReadableText(raw) : cleanText(raw);
...
function safeReadReconReport(reportPath) {
  return safeReadLocalReport(reportPath, runtimePaths().reconOutputDir);
}

function safeReadContactReport(reportPath) {
  return safeReadLocalReport(reportPath, runtimePaths().contactReconReportDir);
}
...
function compactHistory(history = []) {
  return (Array.isArray(history) ? history : [])
    .filter(item => ['system', 'user', 'assistant'].includes(item?.role) && cleanText(item.content))
    .slice(-6)
...
function buildAssistantPrompt(message, context, history = []) {
  ...
  const system = [
    ...
    '输出要结论先行、紧凑可执行。内部分析、解释和建议必须使用简体中文；只有用户明确要求翻译或客户外发文案时才可给英文/俄文草稿。你不能承诺已经修改 CRM。',
  ].join('\n');
  return [
...
async function callDeepSeek(messages, options = {}) {
  ...
  try {
    ...
    try {
      ...
      const err = new Error(`DeepSeek 返回了非 JSON 响应：${text.slice(0, 180)}`);
      err.code = 'DEEPSEEK_INVALID_RESPONSE';
      err.statusCode = 502;
      throw err;
    }
    if (!response.ok) {
    ...
    return {
      ...
      model: data?.model || options.model || process.env.DEEPSEEK_MODEL || DEFAULT_MODEL,
      requestId: String(data?.id || response.headers?.get?.('x-request-id') || ''),
    ...
    if (!err.code) err.code = 'DEEPSEEK_REQUEST_FAILED';
    else if (!String(err.code).startsWith('DEEPSEEK_')) err.code = 'DEEPSEEK_NETWORK_ERROR';
...
function startAssistantRuntimeMonitor(options = {}) {
  const { router = getAssistantRouter(), adapters = assistantAdapters() } = options;
  router.start(adapters);
}
...
function buildCurrentCustomerAnalysisPrompt(message, snapshot, webContext = null) {
  const system = [
    ...
    '先给销售结论，再说明依据、可卖产品、风险/缺口和下一步；内部分析、解释和建议必须使用简体中文，保持紧凑。',
...
async function currentCustomerAssistantAnswer(
  ...
  return {
    ...
    engineAttempts: result.engineAttempts,
