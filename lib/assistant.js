...
function searchCrmContext(message) {
...
async function fetchWebPagesContext(message, context = {}, webContext = null) {
...
async function callDeepSeek(messages) {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      const err = new Error('DeepSeek 请求超时，请稍后重试或调大 ASSISTANT_TIMEOUT_MS。');
      err.statusCode = 504;
      throw err;
    }
    throw e;
...
function buildCurrentCustomerAnalysisPrompt(message, snapshot, webContext = null) {
  const system = [
    ...
    USER_BUSINESS_PROFILE,
    '当前请求已经由后端锁定到一个具体客户；你只能分析“事实快照”中的这个客户，禁止切换到全库画像、典型客户示例或其他客户清单。',
    '如果事实快照显示 Recon 正在 running/queued 或尚未回填结果，要明确说明当前 Recon 还没有可用结论，不能编造 Recon 分数、联系人、合规结论或证据。',
    '不要说“用户未指定具体客户”，因为当前客户 ID 已经由系统上下文提供。',
    '按销售判断工作流思考：这个客户是谁、靠什么赚钱、为什么可能买、证据够不够、找谁联系、怎么开口、风险在哪里、下一步最小动作是什么。',
    '不要机械复述字段；先给业务判断，再说明依据、风险/缺口、下一步动作。',
    '可以做谨慎推断，但要标注“推断依据”；不能把推断写成事实。',
    '如果提供了公开搜索线索，只能作为待核验线索，不能当成 CRM 已验证联系人或采购结论。',
    '官网相关问题必须判断来源类型：official_candidate 只能说“疑似官网候选”，third_party/irrelevant_or_generic 必须说明不是官网或不能证明官网。',
    'webPages 是后端实际打开网页后抽取的正文；回答官网、产品、采购入口、联系人时优先依据 webPages，而不是只看搜索摘要。',
    '如果 webPages 打开失败，要说明失败 URL 和失败原因，再给下一步核验建议。',
    '信息不足时不能停在“缺信息”：必须说明缺什么、当前已有哪类证据、公开搜索线索是否可用、下一步应查官网/注册信息/采购页/招聘/社媒中的哪一项。没有 URL 或来源的内容只能标为待核验。',
    '回答要短而有业务判断，优先中文。根据任务选择结构，不要每次套同一个 1-6 模板。',
  ].join('\n');
  return [
    { role: 'system', content: system },
    {
      ...
      content: [
        ...
        snapshot.answer,
        '',
        webContext?.results?.length ? '公开搜索线索（待核验，不等同于 CRM 已验证事实）：' : '',
        webContext?.results?.length ? JSON.stringify({ query: webContext.query, results: webContext.results }, null, 2) : '',
        webContext?.pages?.length ? '已打开网页正文（后端实际 fetch，仍需看来源类型和可信度）：' : '',
        webContext?.pages?.length ? JSON.stringify(webContext.pages, null, 2) : '',
        '',
...
async function currentCustomerAssistantAnswer(message, contextPayload = {}, history = []) {
  ...
  const result = await callDeepSeek(buildCurrentCustomerAnalysisPrompt(message, snapshot, webContext));
...
function currentCustomerAnswer(contextPayload = {}, message = '') {
  ...
  try {
    ...
    if (!pool && !followed && !job && !result) return null;
    ...
    `, [customerId]) : [];
    const base = {
      source: pool ? 'pool' : 'followup',
      customer_id: customerId || followed?.customer_id || result?.customer_id || job?.customer_id || '',
      follow_id: followed?.follow_id || '',
      company_name: pool?.company_name || followed?.company_name || result?.company_name || job?.company_name || '',
      website: pool?.website || pool?.domain || followed?.website || result?.website || job?.website || '',
      ...
      notes: pool?.notes || followed?.notes || '',
      next_action: followed?.next_action || result?.next_action || result?.outreach_angle || pool?.notes || '',
    };
    ...
    const action = base.next_action || (result ? normalizedOpportunity(result) : '') || '先确认采购入口、常用品牌/型号和是否接受国产替代，再决定外联节奏。';
    const answer = hasAny(message, ['画像', '当前客户'])
      ? [
        ...
        `5. 确认点：${contact === '-' ? '缺少明确联系人/采购入口；' : `联系方式 ${contact}；`} ${reconLine}；${tagText ? `标签 ${tagText}` : '暂无标签'}`,
...
async function semanticContext(message, contextPayload) {
  if (!wantsSemantic(message) && !wantsTemplate(message)) return null;
  ...
  try {
    const result = await vectorSearch(query, { limit: 10 });
...
async function answerAssistantQuestion(payload = {}) {
  ...
  if (kind === 'current_customer') {
    const current = await currentCustomerAssistantAnswer(retrievalMessage, contextPayload, history);
  ...
  const result = await callDeepSeek(buildAssistantPrompt(message, sqlContext, history));
  if (!cleanText(result.answer)) {
    const err = new Error('DeepSeek 没有返回有效内容，请稍后重试。');
    err.statusCode = 502;
    throw err;
