...
function createAssistantRouter(options = {}) {
  ...
  const routerTimeoutMs = boundedInteger(options.routerTimeoutMs ?? process.env.ASSISTANT_ROUTER_TIMEOUT_MS, 75000, 100, 95000);
  const maxAttempts = boundedInteger(options.maxAttempts ?? process.env.ASSISTANT_ROUTER_MAX_ATTEMPTS, 2, 1, 3);
  ...
  async function route(messages, requestOptions = {}, adapters = {}) {
    ...
    if (current.mode !== 'auto') {
      ...
      const startedAt = now();
      try {
        const result = await invoke(engine, messages, requestOptions, adapters, requestOptions.timeoutMs);
        markSuccess(engine, startedAt);
