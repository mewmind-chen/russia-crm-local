...
function buildIntakeFlowFilterScope(user, pageKey, ast = { page: pageKey, filters: [] }) {
  ...
  if (user?.role === 'sales' || !hasPermission(user, 'manage_intake')) {
    conditions.push("i.status!='duplicate'");
...
function intakeFlowFilterOptions(db, user, pageKey, authorizedFields) {
  const config = pageConfig(pageKey);
  assertPageAccess(user, config);
  const scope = buildIntakeFlowFilterScope(user, pageKey, { page: pageKey, filters: [] });
  const result = {};
  for (const item of authorizedFieldKeys(user, pageKey, authorizedFields)) {
    if (item.type === 'text' || item.type === 'date_range') {
      ...
      result[item.key] = tagOptions(db, scope, item);
    } else if (item.key === 'source_batch') {
      result[item.key] = db.prepare(`SELECT b.id value,
      ...
      result[item.key] = db.prepare(`SELECT i.assigned_owner_id value,
      ...
      result[item.key] = Object.entries(INTAKE_STATUS_LABELS)
        .filter(([status]) => {
          if (status === 'duplicate' && (user?.role === 'sales' || !hasPermission(user, 'manage_intake'))) {
            return false;
          }
          return true;
        })
      ...
      result[item.key] = simpleOptions(db, scope, expression);
    }
  }
  return result;
}
...
function queryIntakeFlowPage(db, user, pageKey, ast, pagination = {}) {
  ...
  let items = db.prepare(`SELECT i.*,
    ...
    ORDER BY CASE i.status
      ...
      i.created_at DESC,i.id ASC LIMIT ? OFFSET ?`)
  ...
  if (!hasPermission(user, 'view_contacts')) items = redactContactFields(items);
  return {
    items,
    page,
    pageSize,
