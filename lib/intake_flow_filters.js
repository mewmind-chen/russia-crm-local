...
// customer_pool 的业务字段投影。线索池列表沿用当前 intake item 作为工作流事实，
// 同时把客户主档字段以 pool_* 前缀带出，避免与 intake/crm 字段同名覆盖。
const CUSTOMER_POOL_LIST_FIELDS = Object.freeze([
  ['customer_id', 'pool_customer_id'],
  ['domain', 'pool_domain'],
  ['company_name', 'pool_company_name'],
  ['nickname', 'pool_nickname'],
  ['russian_name', 'pool_russian_name'],
  ['english_name', 'pool_english_name'],
  ['country', 'pool_country'],
  ['city', 'pool_city'],
  ['website', 'pool_website'],
  ['industry', 'pool_industry'],
  ['customer_type', 'pool_customer_type'],
  ['established_year', 'pool_established_year'],
  ['description', 'pool_description'],
  ['products', 'pool_products'],
  ['rating', 'pool_rating'],
  ['current_pool', 'pool_current_pool'],
  ['assigned_to', 'pool_assigned_to'],
  ['assigned_at', 'pool_assigned_at'],
  ['country_code', 'pool_country_code'],
  ['phone', 'pool_phone'],
  ['email', 'pool_email'],
  ['email_raw', 'pool_email_raw'],
  ['inn', 'pool_inn'],
  ['risk_status', 'pool_risk_status'],
  ['website_verification', 'pool_website_verification'],
  ['contact_count', 'pool_contact_count'],
  ['deep_report', 'pool_deep_report'],
  ['source_file', 'pool_source_file'],
  ['first_found', 'pool_first_found'],
  ['last_found', 'pool_last_found'],
  ['search_count', 'pool_search_count'],
  ['verified', 'pool_verified'],
  ['notes', 'pool_notes'],
  ['created_at', 'pool_created_at'],
  ['updated_at', 'pool_updated_at'],
  ['best_contact_level', 'pool_best_contact_level'],
  ['best_person_id', 'pool_best_person_id'],
  ['sales_ready_contact_count', 'pool_sales_ready_contact_count'],
  ['contact_recon_status', 'pool_contact_recon_status'],
  ['contact_last_checked_at', 'pool_contact_last_checked_at'],
  ['contact_next_action', 'pool_contact_next_action'],
]);
const CUSTOMER_POOL_CONTACT_FIELDS = new Set([
  // Product and report narratives can embed a named person or direct contact
  // just like the legacy account fields, so keep the existing no-contact
  // boundary for list responses even though their columns remain discoverable.
  'pool_description', 'pool_products', 'pool_phone', 'pool_email', 'pool_email_raw', 'pool_contact_count',
  'pool_deep_report', 'pool_source_file', 'pool_notes', 'pool_best_contact_level',
  'pool_best_person_id', 'pool_sales_ready_contact_count', 'pool_contact_recon_status',
  'pool_contact_last_checked_at', 'pool_contact_next_action',
]);
const CUSTOMER_POOL_RECON_FIELDS = new Set([
  'pool_description', 'pool_products', 'pool_deep_report', 'pool_source_file', 'pool_notes',
]);
function customerPoolListProjection(alias = 'p', db = null) {
  const available = db
    ? new Set(db.prepare('PRAGMA table_info(customer_pool)').all().map(row => String(row.name || '')))
    : null;
...
function redactCustomerPoolListFields(rows, user) {
  const canContacts = hasPermission(user, 'view_contacts');
  const canRecon = hasPermission(user, 'view_recon');
  return rows.map(row => {
    const safe = { ...row };
    for (const key of CUSTOMER_POOL_CONTACT_FIELDS) {
      if (!canContacts) delete safe[key];
    }
    for (const key of CUSTOMER_POOL_RECON_FIELDS) {
      if (!canRecon) delete safe[key];
    }
    return safe;
  });
}
const INTAKE_SORT_FIELDS = Object.freeze({
  company_name: 'company_name COLLATE NOCASE',
  country: "COALESCE(i.country,'') COLLATE NOCASE",
  status: 'i.status COLLATE NOCASE',
  assigned_owner_name: "COALESCE(assigned.name,'') COLLATE NOCASE",
  updated_at: "COALESCE(i.updated_at,'')",
  claim_due_at: "COALESCE(i.claim_due_at,'')",
  contact_level: "COALESCE(i.contact_level,'')",
  pool_customer_id: "COALESCE(p.customer_id,'')",
  pool_domain: "COALESCE(p.domain,'')",
...
function buildIntakeFlowFilterScope(user, pageKey, ast = { page: pageKey, filters: [] }, db = null) {
  const config = pageConfig(pageKey);
  ...
  return {
    ...
    alias: 'i',
    from: db && hasTable(db, 'customer_pool')
      ? 'crm_intake_items i LEFT JOIN customer_pool p ON p.customer_id=i.external_customer_id'
      : 'crm_intake_items i',
    conditions,
    params,
...
function queryIntakeFlowPage(db, user, pageKey, ast, pagination = {}) {
  const scope = buildIntakeFlowFilterScope(user, pageKey, ast, db);
  const poolAvailable = scope.from.includes('customer_pool');
  const { page, pageSize, offset } = normalizePagination(pagination);
  const requestedSort = String(pagination.sort || '').trim();
  const poolColumns = poolAvailable
    ? new Set(db.prepare('PRAGMA table_info(customer_pool)').all().map(row => String(row.name || '')))
    : new Set();
  const sortFields = Object.fromEntries(Object.entries(INTAKE_SORT_FIELDS).filter(([key]) => {
    if (key === 'contact_level' && !hasPermission(user, 'view_contacts')) return false;
    if (CUSTOMER_POOL_CONTACT_FIELDS.has(key) && !hasPermission(user, 'view_contacts')) return false;
    if (CUSTOMER_POOL_RECON_FIELDS.has(key) && !hasPermission(user, 'view_recon')) return false;
    if (!key.startsWith('pool_')) return true;
    return poolColumns.has(key.replace(/^pool_/, ''));
  }));
  const customFields = sortFields;
  const customSort = parseSortDescriptors(requestedSort, customFields);
  const orderBy = customSort.length
    ? orderByForSort(requestedSort, customFields, { tieBreaker: 'i.id ASC' }).orderBy
    : INTAKE_SORT_ORDER[requestedSort] || INTAKE_SORT_ORDER.status_priority;
  const total = Number(db.prepare(`SELECT COUNT(*) total
    FROM ${scope.from}${scope.where}`).get(...scope.params).total || 0);
  const poolSelect = poolAvailable
    ? `${customerPoolListProjection('p', db)},
      COALESCE(p.nickname,'') nickname,
      COALESCE(NULLIF(p.company_name,''),i.company_name) company_name,`
    : "'' nickname,i.company_name company_name,";
  ...
  items = redactCustomerPoolListFields(items, user);
  if (user?.role === 'sales') {
