...
async function vectorSearch(query, options = {}) {
  ...
  try {
    ...
    // Existing document chunks are intentionally rich and may embed contact data in
    // generic customer/recon text. Until a separate contact-free index is built,
    // contact-restricted sessions must not retrieve any vector chunk.
    if (options.canViewContacts === false) filters.push('0');
    if (customerId) {
