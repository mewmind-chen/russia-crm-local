...
function scopedPlanEvents(db, user, table, type, nowMs) {
  ...
  const rows = db.prepare(`SELECT e.*,a.id account_id,a.owner_id current_owner_id
    ...
    WHERE ${scope.conditions.join(' AND ')}
      AND ${effectivePlanWhereClause(db, table, 'e')}`).all(...scope.params);
  return rows.map(row => ({
    ...row,
    type,
...
function scopedActions(db, user, nowMs) {
  ...
  if (hasTable(db, 'crm_activities')) {
    sources.push(...db.prepare(`SELECT x.*,a.external_customer_id
      ...
      WHERE ${scope.conditions.join(' AND ')}
        AND ${effectiveActivityWhereClause(db, 'x')}`).all(...scope.params).map(row => ({
      kind: 'activity',
  ...
  for (const [table, kind, dateColumn] of [
    ...
    const commerceSql = effectiveCommerceSql(db, kind, { commerce: 'x', activity: 'linked_activity' });
    sources.push(...db.prepare(`SELECT x.*,a.external_customer_id
      FROM ${table} x ${commerceSql.join}
      JOIN crm_accounts a ON a.id=x.customer_id
      WHERE ${scope.conditions.join(' AND ')} AND ${commerceSql.condition}`)
...
const MANAGER_METRIC_DRILLDOWN_LABELS = Object.freeze({
  activeCustomers: '当前开发客户',
  deferredCustomers: '延期客户',
  thresholdCustomers: '需要主管关注',
  plannedAfterDeferredCustomers: '延期后已形成计划',
  onTimeActionCustomers: '计划后按时行动',
  firstTouchSilentCustomers: '首次触达后未继续推进',
  unimprovedAfterInterventionCustomers: '协助后仍未改善',
});

function buildManagerMetricDrilldown(db, options = {}) {
  const rangeDays = Number(options.rangeDays);
  if (![30, 90].includes(rangeDays)) {
    throw metricError('统计周期只支持30天或90天', 'MANAGER_METRICS_RANGE_INVALID');
  }
  const kind = String(options.kind || '').trim();
  if (!Object.hasOwn(MANAGER_METRIC_DRILLDOWN_LABELS, kind)) {
    throw metricError('统计明细类型无效', 'MANAGER_METRICS_DRILLDOWN_INVALID');
  }
  const user = options.user || {};
  const nowMs = requiredNow(options.now);
  const startMs = nowMs - rangeDays * 86400000;
  ...
  const tasks = scopedTasks(db, user, nowMs);
  const interventions = scopedInterventions(db, user, nowMs);
  const actions = scopedActions(db, user, nowMs);
  const deferredWindow = deferredEvents.filter(row => row.createdMs >= startMs);
  const taskWindow = tasks.filter(row => row.triggeredMs >= startMs
    && CUSTOMER_TRIGGER_REASONS.has(String(row.reason || '')));
  const silentByOwner = firstTouchSilentByOwner(accounts, actions, startMs, nowMs, settings);
  const unimproved = unimprovedByOwner(tasks, interventions, startMs, nowMs);
  const visibleActors = salesUserIds(db, user, accounts, deferredWindow, taskWindow);
  const requestedActor = String(options.actorId || '').trim();
  if (requestedActor && !visibleActors.includes(requestedActor)) {
    throw metricError('销售不在当前授权范围内', 'MANAGER_METRICS_ACTOR_NOT_FOUND', 404);
  }
  const actorIds = requestedActor ? [requestedActor] : visibleActors;
  const ids = new Set();
  for (const actorId of actorIds) {
    ...
    const sets = {
      ...
      deferredCustomers,
      thresholdCustomers,
      plannedAfterDeferredCustomers: outcomes.planned,
      onTimeActionCustomers: outcomes.actedOnTime,
      firstTouchSilentCustomers: silentByOwner.get(actorId) || new Set(),
      unimprovedAfterInterventionCustomers: unimproved.get(actorId) || new Set(),
    };
    sets[kind].forEach(id => ids.add(String(id)));
  }
  const names = actorNames(db, [...new Set(accounts.map(row => String(row.owner_id || '')).filter(Boolean))]);
  const actionByCustomer = new Map();
  for (const action of actions) {
    const customerId = String(action.customerId || '');
    if (!customerId) continue;
    const current = actionByCustomer.get(customerId);
    if (!current || action.occurredMs > current.occurredMs) actionByCustomer.set(customerId, action);
  }
  const allRows = accounts.filter(account => ids.has(String(account.external_customer_id || account.id)))
    .map(account => {
      const customerId = String(account.external_customer_id || account.id);
      const lastAction = actionByCustomer.get(customerId);
      return {
        customerId,
        ...
        ownerName: names.get(String(account.owner_id || '')) || '',
        stage: String(account.stage || ''),
        lastActivityAt: String(lastAction?.occurredAt || account.last_activity_at || ''),
        nextAction: String(account.next_action || ''),
        nextActionAt: String(account.next_action_at || ''),
        reason: MANAGER_METRIC_DRILLDOWN_LABELS[kind],
      };
    }).sort((left, right) => (
      String(left.nextActionAt || '9999').localeCompare(String(right.nextActionAt || '9999'))
  ...
  const pageSize = Math.min(100, Math.max(1, Number.parseInt(options.pageSize, 10) || 50));
  const offset = (page - 1) * pageSize;
  return Object.freeze({
    kind,
    label: MANAGER_METRIC_DRILLDOWN_LABELS[kind],
    rangeDays,
    actorId: requestedActor,
    page,
    pageSize,
    total: allRows.length,
    hasMore: offset + pageSize < allRows.length,
    rows: Object.freeze(allRows.slice(offset, offset + pageSize).map(Object.freeze)),
  });
}
...
function buildCustomerPlanRisk(db, options = {}) {
  ...
  const allDeferred = hasTable(db, 'crm_deferred_plan_events')
  ...
  const deferred = hasTable(db, 'crm_deferred_plan_events')
    ? db.prepare(`SELECT * FROM crm_deferred_plan_events e
      WHERE customer_id=? AND ${effectivePlanWhereClause(db, 'crm_deferred_plan_events', 'e')}
      ORDER BY created_at,id`).all(stableId)
    : [];
  const explicit = hasTable(db, 'crm_next_plan_events')
    ? db.prepare(`SELECT * FROM crm_next_plan_events e
      WHERE customer_id=? AND ${effectivePlanWhereClause(db, 'crm_next_plan_events', 'e')}
      ORDER BY created_at,id`).all(stableId)
    : [];
  const effectiveDeferredIds = new Set(deferred.map(row => String(row.id)));
  const events = [
  ...
  return Object.freeze({
    ...
    unplannedDurationDays: consecutive
      ...
      : 0,
    thresholdAt,
    history: Object.freeze(allDeferred.filter(row => timestamp(row.created_at) <= nowMs).map(row => Object.freeze({
      id: row.id,
      ...
      reason: row.reason,
      source: row.source,
      effective: effectiveDeferredIds.has(String(row.id)),
    }))),
  });
}
