...
function employeeFacingText(name, value) {
  ...
  if (name === 'sales_coaching') {
    return [...(value.strengths || []), ...(value.gaps || []), ...(value.recommendations || [])];
  }
  return [];
}
...
function validateStationOutput(name, version, value, context = {}) {
  ...
  if (name === 'sales_coaching') {
    const constraints = [
      ['salesUserIds', value.salesUserId, 'sales user'],
      ['sampleSizes', value.sampleSize, 'sample size'],
      ['sampleStatuses', value.sampleStatus, 'sample status'],
    ];
    for (const [key, selected, label] of constraints) {
    ...
    if (value.sampleStatus === 'limited' && value.confidence > 0.65) {
      return { ok: false, errors: ['limited sample confidence cannot exceed 0.65'] };
    }
  }
