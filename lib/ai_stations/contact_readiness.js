  ...
  for (const row of fitRows) {
    ...
    const transaction = db.transaction(() => {
      ...
      const readiness = jobs.enqueue({
        ...
        trigger: fitJob.workflowId ? {
          source: 'workflow',
          workflowId: fitJob.workflowId,
          actorId: fitJob.trigger.actorId,
          reason: 'customer_fit_completed',
        } : {
          source: 'schedule',
          actorId: fitJob.trigger.actorId,
          reason: 'completed_fit_successor_scan',
        },
        ...
        if (!existingLink?.ai_job_id) {
          ...
          const finalize = jobs.enqueue({
            ...
            trigger: fitJob.workflowId ? {
              ...
              workflowId: fitJob.workflowId,
              actorId: fitJob.trigger.actorId,
              reason: 'customer_enrichment_finalize',
            } : {
              source: 'schedule',
              actorId: fitJob.trigger.actorId,
              reason: 'completed_fit_enrichment_finalize',
            },
