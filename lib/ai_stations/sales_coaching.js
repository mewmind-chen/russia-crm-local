...
function loadSalesCoachingScope(db, accessContext) {
  ...
  const orderSql = effectiveCommerceSql(db, 'order', { commerce: 'o', activity: 'oa' });
  ...
  const activityTestClause = db.prepare('PRAGMA table_info(crm_activities)').all()
    .some(column => column.name === 'is_test_data') ? 'AND COALESCE(x.is_test_data,0)=0' : '';
  ...
  return {
    users,
    accounts,
    activities: db.prepare(`SELECT x.customer_id,x.activity_type,x.channel,x.occurred_at
      FROM crm_activities x WHERE x.customer_id IN (${placeholders})
        ${activityTestClause} AND ${effectiveActivityWhereClause(db, 'x')}
      ORDER BY x.customer_id,x.occurred_at,x.id`).all(...accountIds),
    rfqs: db.prepare(`SELECT r.customer_id,r.completeness,r.received_at
      FROM crm_rfqs r ${rfqSql.join}
      WHERE r.customer_id IN (${placeholders}) AND ${rfqSql.condition}
      ORDER BY r.customer_id,r.received_at,r.id`).all(...accountIds),
    quotes: db.prepare(`SELECT q.customer_id,q.sent_at
      FROM crm_quotes q ${quoteSql.join}
      WHERE q.customer_id IN (${placeholders}) AND ${quoteSql.condition}
      ORDER BY q.customer_id,q.sent_at,q.id`).all(...accountIds),
    orders: db.prepare(`SELECT o.customer_id,o.amount,o.gross_margin,o.is_repeat,o.ordered_at
      FROM crm_orders o ${orderSql.join}
      WHERE o.customer_id IN (${placeholders}) AND ${orderSql.condition}
      ORDER BY o.customer_id,o.ordered_at,o.id`).all(...accountIds),
  };
}
