...
const TERMINAL_STAGES = new Set(['won', 'repeat', 'lost']);
...
function buildSalesCoachingSnapshot(input = {}) {
  ...
  const overdue = accounts.filter(row => clean(row.next_action_at)
    ...
    && !TERMINAL_STAGES.has(clean(row.stage))).length;
...
function loadSalesCoachingScope(db, accessContext) {
  ...
  return {
    ...
    activities: db.prepare(`SELECT customer_id,activity_type,channel,occurred_at
      FROM crm_activities WHERE customer_id IN (${placeholders}) ORDER BY customer_id,occurred_at,id`).all(...accountIds),
...
function buildSalesCoachingContext(db, accessContext, salesUserId, options = {}) {
  ...
  if (snapshot.sampleStatus === 'insufficient' && !options.allowInsufficient) {
    throw coachingError(
...
function enqueueSalesCoaching(db, accessContext, actor, salesUserId, options = {}) {
  ...
  return Object.freeze({
    ...
    job: jobs.enqueue({
      ...
      createdBy: actor.id,
      trigger: { source: 'manual', actorId: actor.id, reason: 'sales_coaching_requested' },
