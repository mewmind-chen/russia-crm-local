...
function createAIStationWorker(options = {}) {
  ...
  const executors = {
    ...
    sales_coaching: options.executeSalesCoachingJob || executeSalesCoachingJob,
    ...(options.executors || {}),
  };
  ...
  function executionIdentity(db, job) {
    ...
    const accessContext = buildAccessContext(db, actor, {
      includeTestData: job.triggerSource === 'release_validation'
        && job.triggerReason.startsWith('production-smoke:'),
    });
  ...
  async function runOnce() {
    ...
    try {
      ...
      if (typeof options.isWorkerEnabled === 'function'
          && !options.isWorkerEnabled({ db, workerId: id })) {
        const queue = jobs.queueHealth(options.queueHealthOptions || {});
        return Object.freeze({ status: 'idle', workerId: id, job: null, queue, disabled: true });
      }
      ...
      const job = jobs.claimNext(id, {
        stationAllowed: candidate => candidate.executionMode !== 'batch_eligible'
          && (typeof options.isJobEnabled !== 'function'
