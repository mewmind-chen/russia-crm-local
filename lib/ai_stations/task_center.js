...
function columnExists(db, table, name) {
  return exists(db, table)
    && db.prepare(`PRAGMA table_info(${table})`).all().some(column => column.name === name);
}
...
function createAITaskCenterStore(db, options = {}) {
  ...
  const accountTestClause = columnExists(db, 'crm_accounts', 'is_test_data')
    ? 'AND COALESCE(a.is_test_data,0)=0' : '';
  const smokeJobClause = columnExists(db, 'crm_ai_jobs', 'trigger_source')
    && columnExists(db, 'crm_ai_jobs', 'trigger_reason')
    ? "AND NOT (j.trigger_source='release_validation' AND j.trigger_reason LIKE 'production-smoke:%')" : '';
  ...
  function aiStationTasks() {
    return db.prepare(`
      SELECT j.id source_id,j.customer_id,j.crm_account_id,j.station task_type,
        ...
        EXISTS(SELECT 1 FROM crm_ai_enrichment_node_links enrichment_link
          WHERE enrichment_link.ai_job_id=j.id) enrichment_task
      FROM crm_ai_jobs j
      LEFT JOIN crm_ai_station_results r ON r.job_id=j.id
      LEFT JOIN crm_accounts a ON a.id=j.crm_account_id
      WHERE 1=1 ${smokeJobClause} ${accountTestClause}
    `).all().map(row => ({
      source: 'ai_station',
      ...
      errorSummary: row.error_summary,
      enrichmentTask: Boolean(row.enrichment_task),
    }));
  }
  ...
  function legacyTasks() {
    ...
    if (exists(db, 'recon_jobs')) {
      rows.push(...db.prepare(`SELECT r.*,COALESCE(a.id,'') crm_account_id,
        ...
        enrichmentTask: Boolean(db.prepare(`SELECT 1 FROM crm_ai_enrichment_node_links
          WHERE legacy_task_type='recon' AND legacy_task_id=? LIMIT 1`).get(row.job_id)),
      })));
    }
    if (exists(db, 'contact_recon_jobs')) {
      rows.push(...db.prepare(`SELECT r.*,COALESCE(a.id,'') crm_account_id,
        ...
        enrichmentTask: Boolean(db.prepare(`SELECT 1 FROM crm_ai_enrichment_node_links
          WHERE legacy_task_type='contact_recon' AND legacy_task_id=? LIMIT 1`).get(row.job_id)),
      })));
    }
    ...
    if (exists(db, 'crm_manager_evaluations')) {
      rows.push(...db.prepare(`SELECT e.*,a.external_customer_id,a.company_name
        FROM crm_manager_evaluations e JOIN crm_accounts a ON a.id=e.customer_id
        WHERE 1=1 ${accountTestClause}`).all().map(row => ({
  ...
  function visible(item, accessContext, actor) {
    if (typeof options.visibleTask === 'function' && !options.visibleTask(item)) return false;
  ...
  function list({ accessContext, actor, query = {} }) {
    ...
    const visibleItems = [...aiStationTasks(), ...interactionTasks(), ...legacyTasks()]
    ...
    return {
      ...
      pageSize,
      total: items.length,
      overview: actor?.role === 'admin' ? overview(visibleItems) : null,
    };
  }
  ...
  function stationDetail(sourceId, accessContext, actor) {
    ...
    if (!job) return null;
    const enrichmentTask = Boolean(db.prepare(`SELECT 1 FROM crm_ai_enrichment_node_links
      WHERE ai_job_id=? LIMIT 1`).get(job.id));
    if (typeof options.visibleTask === 'function'
        && !options.visibleTask({ source: 'ai_station', taskType: job.station, enrichmentTask })) return null;
    ...
    const snapshot = job.decisionSnapshot || {};
    const resultEvidenceIds = Array.isArray(result?.value?.evidenceIds)
      ? [...new Set(result.value.evidenceIds.map(value => String(value || '').trim()).filter(Boolean))]
      : [];
    const lastAttempt = attempts.at(-1);
    const decisionTrace = {
      stationVersion: String(snapshot.stationVersion || job.input.stationVersion || 'v1'),
      model: String(result?.model || lastAttempt?.model || ''),
      promptVersion: String(result?.promptVersion || snapshot.promptVersion || job.input.promptVersion || 'v1'),
      schemaVersion: String(result?.schemaVersion || snapshot.schemaVersion || job.input.schemaVersion || 'v1'),
      ruleVersion: String(snapshot.ruleVersion || job.input.ruleVersion || 'v1'),
      strategyVersion: String(snapshot.strategyVersion || job.input.strategyVersion || ''),
      contextHash: job.contextHash,
      evidenceIds: resultEvidenceIds,
      generatedAt: String(result?.generatedAt || ''),
      stale: Boolean(result?.stale),
      staleAt: String(result?.staleAt || ''),
      staleReason: String(result?.staleReason || ''),
    };
    const timeline = [
  ...
  function overview(visibleItems) {
    const at = now();
    ...
    const scoped = Array.isArray(visibleItems);
    const jobIds = scoped ? visibleItems
      .filter(item => item.source === 'ai_station')
      .map(item => item.sourceId) : [];
    const placeholders = jobIds.map(() => '?').join(',');
    const jobWhere = !scoped ? '' : jobIds.length ? `WHERE id IN (${placeholders})` : 'WHERE 0';
    const childWhere = !scoped ? '' : jobIds.length ? `AND job_id IN (${placeholders})` : 'AND 0';
    const states = db.prepare(`SELECT CASE WHEN control_state!='' THEN control_state ELSE state END state,
      COUNT(*) count FROM crm_ai_jobs ${jobWhere} GROUP BY 1`).all(...jobIds);
    ...
    const failures = db.prepare(`SELECT COUNT(*) total,
      ...
      FROM crm_ai_model_runs WHERE finished_at>=? ${childWhere}`).get(dayStart, ...jobIds);
    const costs = db.prepare(`SELECT
      ...
      FROM crm_ai_usage_ledger WHERE accounted_at>=? ${childWhere}`)
      .get(dayStart, monthStart, ...jobIds);
