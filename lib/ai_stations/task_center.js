...
function createAITaskCenterStore(db, options = {}) {
  ...
  function stationDetail(sourceId, accessContext) {
    ...
    return {
      ...
      canReview: job.station !== 'action_proposal'
  ...
  function review({ jobId, accessContext, actor, decision, summary = '' }) {
    ...
    if (!job) return null;
    assertExternalCustomerAccess(accessContext, job.customerId);
    if (job.station === 'action_proposal') {
      const error = new Error('活动提案必须在记录跟进表单确认');
      error.statusCode = 409;
      error.code = 'AI_ACTION_PROPOSAL_REQUIRES_ACTIVITY_CONFIRMATION';
      throw error;
    }
