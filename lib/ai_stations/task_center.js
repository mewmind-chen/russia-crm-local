...
function createAITaskCenterStore(db, options = {}) {
  ...
  function stationDetail(sourceId, accessContext) {
    ...
    const reviews = db.prepare(`SELECT decision,summary,reviewer_id reviewerId,created_at createdAt
      FROM crm_ai_task_reviews WHERE job_id=? ORDER BY created_at,id`).all(job.id);
    let evidence = [];
    if (result) {
      const context = buildCustomerContext(db, accessContext, job.customerId);
    ...
    return {
      ...
      canCancel: ['queued', 'running', 'retry_wait'].includes(job.state),
      canRetry: ['dead_letter', 'blocked', 'cancelled', 'retry_wait'].includes(job.state),
      canReview: job.state === 'needs_review',
    };
  }

  function genericDetail(source, sourceId, accessContext, actor) {
    const item = [...interactionTasks(), ...legacyTasks()].find(row => row.source === source && row.sourceId === sourceId);
    if (!item || !visible(item, accessContext, actor)) return null;
    ...
    if (source === 'assistant_chat') {
      ...
      item.attempts = parseJson(row.attempts_json, []).map(value => ({
        ...
        error: value.error || value.code || '',
      }));
      item.fallbackReason = row.fallback_reason;
    }
  ...
  function detail({ taskId: selected, accessContext, actor }) {
    ...
    if (!raw.includes(':')) return stationDetail(raw, accessContext);
  ...
  function overview() {
    ...
    const failures = db.prepare(`SELECT COUNT(*) total,
      ...
      FROM crm_ai_model_runs WHERE finished_at>=?`).get(new Date(now().getTime() - 86_400_000).toISOString());
    const costs = db.prepare(`SELECT COALESCE(SUM(charged_cost_micros),0) dailyMicros
      FROM crm_ai_usage_ledger WHERE accounted_at>=?`).get(new Date(now().getTime() - 86_400_000).toISOString());
    return {
  ...
  function review({ jobId, accessContext, actor, decision, summary = '' }) {
    ...
    return stationDetail(job.id, accessContext);
  }
