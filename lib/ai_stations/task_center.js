...
const { CAPABILITIES, presentAIResult } = require('./presentation');
...
const LEGACY_TRIGGER = Object.freeze({
  source: 'legacy_unknown', eventType: '', eventId: '', actorId: '',
  workflowId: '', reason: '', triggeredAt: '',
});
...
function taskPresentation({ db, job, result, evidence, accessContext, stale }) {
  const capability = ENRICHMENT_STATIONS.has(job.station) ? 'enrichment' : job.station;
  if (!result || !CAPABILITIES[capability]) return null;
  let coverage = {
    state: evidence.length ? 'partial' : 'none',
    analyzed: evidence.length ? 1 : 0,
    total: 1,
  };
  if (job.station === 'sales_coaching') {
    try {
      const snapshot = buildSalesCoachingContext(
        db,
        accessContext,
        job.input.salesUserId,
      ).context.snapshot;
      coverage = {
        state: snapshot.sampleStatus,
        sampleSize: snapshot.sampleSize,
        minimumSample: 10,
        analyzed: snapshot.sampleSize,
        total: snapshot.sampleSize,
        rules: ['少于 10 个已观察客户时不调用模型'],
      };
    } catch (_error) {
      coverage = { state: 'none', sampleSize: 0, minimumSample: 10 };
    }
  } else if (job.station === 'manager_anomaly') {
    coverage.rules = ['异常由确定性业务规则发现，AI 只负责解释和建议'];
  }
  return presentAIResult({
    job,
    result: { ...result, stale: Boolean(stale || result.stale) },
    evidence,
    coverage,
  ...
  function aiStationTasks() {
    return db.prepare(`
      SELECT j.id source_id,j.customer_id,j.crm_account_id,j.station task_type,
        ...
        j.trigger_source,j.trigger_actor_id,j.trigger_reason,j.triggered_at,
        j.event_type,j.event_id,j.workflow_id,
    ...
    `).all().map(row => ({
      ...
      trigger: {
        source: row.trigger_source || 'legacy_unknown',
        eventType: row.event_type || '',
        eventId: row.event_id || '',
        actorId: row.trigger_actor_id || '',
