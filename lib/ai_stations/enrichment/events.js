...
function consumePendingEnrichmentEvent(db, consumerId, options = {}) {
  ...
  if (stations.nextDispatch) {
    ...
    const finalize = jobs.enqueue({
      ...
      createdBy: run.triggeredBy,
      payload: { enrichmentRunId: run.id, pipelineVersion: run.pipelineVersion },
    }, `enrichment:${run.id}:enrichment_finalize:${run.pipelineVersion}`);
    store.linkNode({ runId: run.id, nodeKey: 'enrichment_finalize', aiJobId: finalize.id });
    created.push(fit, finalize);
  }
