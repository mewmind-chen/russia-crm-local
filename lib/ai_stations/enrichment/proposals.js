...
function createEnrichmentProposalStore(db, options = {}) {
  ...
  function finalize(runId) {
    ...
    const readinessLink = db.prepare(`SELECT ai_job_id FROM crm_ai_enrichment_node_links
      WHERE run_id=? AND node_key='contact_readiness'`).get(selectedRun.id);
    const readinessResult = readinessLink?.ai_job_id
      ? db.prepare(`SELECT value_json,stale_at FROM crm_ai_station_results
        WHERE job_id=?`).get(readinessLink.ai_job_id)
      : null;
    const readinessValue = parse(readinessResult?.value_json, {});
    const readinessBlocked = Boolean(readinessLink)
      && (!readinessResult || readinessResult.stale_at || readinessValue.readiness !== 'ready');
    if (readinessBlocked && !missingItems.includes('contact_readiness')) {
      missingItems.push('contact_readiness');
    }
    const routeState = readinessBlocked ? 'missing_info'
      : pendingReview ? 'needs_review'
        : missingItems.length ? 'missing_info' : 'pending_assignment';
    let routeReason = routeState === 'pending_assignment' ? '' : routeState;
    if (readinessBlocked) {
      if (readinessResult?.stale_at) routeReason = 'contact_readiness_stale';
      else if (readinessValue.readiness === 'partial') routeReason = 'contact_readiness_partial';
      else routeReason = 'contact_not_ready';
    }
    const tags = [...new Set([
