...
async function executeIdentityVerifyJob({
  ...
  const city = accountColumns.has('city') ? 'city' : "'' city";
  const industry = accountColumns.has('industry') ? 'industry' : "'' industry";
  const customerType = accountColumns.has('customer_type') ? 'customer_type' : "'' customer_type";
  const account = db.prepare(`SELECT id,external_customer_id,company_name,country,website,
      ${city},${industry},${customerType}
