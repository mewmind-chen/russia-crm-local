...
function createCustomerEnrichmentStore(db, options = {}) {
  ...
  function linkNode(input) {
    ...
    if (existing) {
      if (!existing.legacy_task_id && legacyTaskId
          && (!existing.ai_job_id || existing.ai_job_id === aiJobId)) {
        const changed = db.prepare(`UPDATE crm_ai_enrichment_node_links
          SET ai_job_id=COALESCE(ai_job_id,?),legacy_task_type=?,legacy_task_id=?,adapter_state='dispatched',updated_at=?
          WHERE id=? AND legacy_task_id=''`).run(
          aiJobId, legacyTaskType, legacyTaskId, timestamp(), existing.id,
        );
        const current = findLinkByNode.get(runId, nodeKey);
        if (changed.changes !== 1
            || (current.ai_job_id || null) !== aiJobId
            || current.legacy_task_type !== legacyTaskType
            || current.legacy_task_id !== legacyTaskId) {
          throw new Error('enrichment node link collision');
        }
        return mapLink(current);
      }
      if (!existing.ai_job_id && aiJobId) {
        const changed = db.prepare(`UPDATE crm_ai_enrichment_node_links SET ai_job_id=?,updated_at=?
          WHERE id=? AND ai_job_id IS NULL`).run(aiJobId, timestamp(), existing.id);
        const current = findLinkByNode.get(runId, nodeKey);
        if (changed.changes !== 1 || current.ai_job_id !== aiJobId) {
          throw new Error('enrichment node link collision');
        }
        return mapLink(current);
      }
