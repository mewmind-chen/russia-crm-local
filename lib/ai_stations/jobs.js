...
function createAIJobStore(db, options = {}) {
  ...
  const jobColumns = new Set(db.prepare('PRAGMA table_info(crm_ai_jobs)').all().map(column => column.name));
  const supportsTriggerAudit = jobColumns.has('trigger_source') && jobColumns.has('trigger_reason');
  ...
  function enqueue(input, idempotencyKey) {
    ...
    const triggerSource = String(input.triggerSource || '').trim().slice(0, 160);
    const triggerReason = String(input.triggerReason || '').trim().slice(0, 500);
    ...
    if (existing) {
      ...
      if (existing.customer_id !== customerId || existing.crm_account_id !== crmAccountId
          ...
          || (supportsTriggerAudit
            && (String(existing.trigger_source || '') !== triggerSource
              || String(existing.trigger_reason || '') !== triggerReason))
    ...
    const transaction = db.transaction(() => {
      ...
      if (supportsTriggerAudit) {
        db.prepare('UPDATE crm_ai_jobs SET trigger_source=?,trigger_reason=? WHERE id=?')
          .run(triggerSource, triggerReason, id);
      }
