...
const TRIGGER_SOURCES = new Set([
  'manual', 'business_event', 'workflow', 'schedule', 'api', 'migration', 'release_validation',
]);
...
function normalizeTrigger(input, at) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new Error('AI job trigger is required');
  }
  const source = nonempty(input.source, 'trigger.source');
  if (!TRIGGER_SOURCES.has(source)) throw new Error(`invalid AI job trigger source: ${source}`);
  const eventType = String(input.eventType || '').trim();
  const eventId = String(input.eventId || '').trim();
  if (Boolean(eventType) !== Boolean(eventId)) {
    throw new Error('trigger.eventType and trigger.eventId must be provided together');
  }
  if (source === 'business_event' && (!eventType || !eventId)) {
    throw new Error('business_event trigger requires eventType and eventId');
  }
  const workflowId = String(input.workflowId || '').trim();
  if (source === 'workflow' && !workflowId) {
    throw new Error('workflow trigger requires workflowId');
  }
  return Object.freeze({
    source,
    eventType,
    ...
    triggeredAt: input.triggeredAt ? asIso(input.triggeredAt) : at,
  });
}
...
function createAIJobStore(db, options = {}) {
  ...
  function enqueue(input, idempotencyKey) {
    ...
    const at = timestamp();
    const trigger = normalizeTrigger(input.trigger, at);
    const workflowId = String(input.workflowId || trigger.workflowId || '').trim();
    if (input.workflowId && trigger.workflowId && workflowId !== trigger.workflowId) {
      throw new Error('workflowId conflicts with trigger.workflowId');
    }
    const parentJobId = input.parentJobId ? nonempty(input.parentJobId, 'parentJobId') : null;
    const eventType = String(input.eventType || trigger.eventType || '').trim();
    const eventId = String(input.eventId || trigger.eventId || '').trim();
    if ((input.eventType && trigger.eventType && eventType !== trigger.eventType)
        || (input.eventId && trigger.eventId && eventId !== trigger.eventId)) {
      throw new Error('event fields conflict with trigger event');
    }
    ...
    if (existing) {
      ...
      if (existing.customer_id !== customerId || existing.crm_account_id !== crmAccountId
          ...
          || existing.trigger_source !== trigger.source
          || existing.trigger_actor_id !== trigger.actorId
    ...
    const transaction = db.transaction(() => {
      ...
      db.prepare(`UPDATE crm_ai_jobs SET trigger_source=?,trigger_actor_id=?,trigger_reason=?,triggered_at=?
        WHERE id=?`).run(trigger.source, trigger.actorId, trigger.reason, trigger.triggeredAt, id);
