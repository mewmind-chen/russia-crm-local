...
function loadManagerScope(db, accessContext) {
  ...
  const accountTestClause = db.prepare('PRAGMA table_info(crm_accounts)').all()
    .some(column => column.name === 'is_test_data') ? 'AND COALESCE(a.is_test_data,0)=0' : '';
  ...
  return {
    users,
    accounts,
    activities: db.prepare(`SELECT * FROM crm_activities x
      WHERE x.customer_id IN (${placeholders}) ${activityTestClause}
        AND ${effectiveActivityWhereClause(db, 'x')}
      ORDER BY x.customer_id,x.occurred_at DESC,x.id`).all(...accountIds),
    rfqs: db.prepare(`SELECT r.* FROM crm_rfqs r ${rfqSql.join}
      WHERE r.customer_id IN (${placeholders}) AND ${rfqSql.condition}
      ORDER BY r.customer_id,r.received_at DESC,r.id`).all(...accountIds),
    quotes: db.prepare(`SELECT q.* FROM crm_quotes q ${quoteSql.join}
      WHERE q.customer_id IN (${placeholders}) AND ${quoteSql.condition}
      ORDER BY q.customer_id,q.sent_at DESC,q.id`).all(...accountIds),
  };
}
...
function buildManagerAnomalyContext(db, accessContext, anomalyId, options = {}) {
  ...
  if (!anomalyValue) {
    const error = new Error('需主管关注事项已失效，请重新扫描');
