...
const ACTIVE_STAGES = new Set(['new', 'qualified', 'contacted', 'replied', 'meeting', 'manager', 'rfq', 'quoted', 'negotiating']);
...
function buildManagerAnomalies(input = {}) {
  ...
  for (const account of accounts) {
    if (!ACTIVE_STAGES.has(clean(account.stage))) continue;
  ...
  if (sales.length >= 2) {
    const activeAccounts = accounts.filter(account => ACTIVE_STAGES.has(clean(account.stage)) && clean(account.owner_id));
...
function enqueueManagerAnomalies(db, accessContext, actor, options = {}) {
  ...
  for (const item of anomalies) {
    ...
    enqueued.push(jobs.enqueue({
      ...
      createdBy: actor.id,
      trigger: { source: 'manual', actorId: actor.id, reason: 'manager_anomaly_scan_requested' },
