...
function loadManagerScope(db, accessContext) {
  ...
  return {
    users,
    accounts,
    activities: db.prepare(`SELECT * FROM crm_activities
      WHERE customer_id IN (${placeholders}) ${activityTestClause}
      ORDER BY customer_id,occurred_at DESC,id`).all(...accountIds),
