...
function assertGovernanceAccess(actor, accessContext) {
  if (!['admin', 'manager'].includes(actor?.role)
      || (actor.role !== 'admin' && !accessContext?.permissions?.view_team)) {
    throw routeError('只有授权经理或管理员可以管理 AI 版本', 403, 'AI_GOVERNANCE_FORBIDDEN');
  }
}
...
function createAIStationService(options = {}) {
  ...
  function jobFeatureKey(db, job) {
    if (job?.station === 'sales_pack') return 'sales_pack';
    const enrichmentJob = job?.id && db.prepare(`SELECT 1 FROM crm_ai_enrichment_node_links
      WHERE ai_job_id=? LIMIT 1`).get(job.id);
    return enrichmentJob ? 'customer_enrichment' : 'ai_stations';
  }

  function assertJobFeatureEnabled(db, job) {
    assertFeatureEnabled(db, jobFeatureKey(db, job), hardFlags);
  }

  function taskCenter(db) {
    const features = featureState(db, hardFlags);
    return createAITaskCenterStore(db, {
      visibleTask: task => (task.taskType !== 'sales_pack'
          || features.sales_pack.effectiveEnabled)
        && (!task.enrichmentTask || features.customer_enrichment.effectiveEnabled),
    });
  }
  ...
  function getCustomerResults({ db, accessContext, customerId }) {
    ...
    const salesPackEnabled = isFeatureEnabled(db, 'sales_pack', hardFlags);
    const salesPackContext = salesPackEnabled
      ? buildCustomerContext(db, accessContext, customerId, { station: 'sales_pack' })
      : null;
    const salesPackResult = salesPackEnabled
      ? results.latestForCustomer(customerId, 'sales_pack')
      : null;
  ...
  function retryJob({ db, accessContext, actor, jobId }) {
    ...
    assertJobFeatureEnabled(db, current);
  ...
  function listTasks({ db, accessContext, actor, query }) {
    return taskCenter(db).list({ accessContext, actor, query });
  }

  function getTask({ db, accessContext, actor, taskId }) {
    const task = taskCenter(db).detail({ accessContext, actor, taskId });
  ...
  function reviewJob({ db, accessContext, actor, jobId, body }) {
    ...
    assertExternalCustomerAccess(accessContext, job.customerId);
    assertJobFeatureEnabled(db, job);
  ...
  function getGovernance({ db, accessContext, actor }) {
    assertGovernanceAccess(actor, accessContext);
...
function registerAIStationRoutes(app, options = {}) {
  ...
  app.use('/api/sales-crm/ai', (_req, res, next) => {
    const db = openDb();
    try {
      assertFeatureEnabled(db, 'ai_stations', options.hardFlags || resolveAIHardFlags());
      next();
    } catch (error) {
      const response = { ok: false, error: error.message };
      if (error.code) response.code = error.code;
  ...
  app.get('/api/sales-crm/ai/customers/:customerId/enrichment', handle((_api, db, req) => {
    ...
    return enrichment.getEnrichment({
