...
function createAIStationService(options = {}) {
  ...
  function getCustomerResults({ db, accessContext, customerId }) {
    ...
    return {
      ...
      salesPack: {
        ...
        stale: Boolean(salesPackResult && salesPackResult.contextHash !== salesPackContext.contextHash),
      },
    };
  }
  ...
  function retryJob({ db, accessContext, jobId }) {
    ...
    if (!['customer_fit', 'sales_pack'].includes(current.station)) {
  ...
  return Object.freeze({
    getCustomerResults, runCustomerFit, runSalesPack, retryJob, cancelJob, listTasks, getTask, reviewJob,
