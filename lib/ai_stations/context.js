...
function buildCustomerContext(db, accessContext, customerId, options = {}) {
  ...
  const activities = accountRow && canViewCustomers
    ? db.prepare(`SELECT * FROM crm_activities
        WHERE customer_id=? ${columnExists(db, 'crm_activities', 'is_test_data') ? 'AND COALESCE(is_test_data,0)=0' : ''}
        ORDER BY occurred_at DESC,id DESC LIMIT 30`).all(accountRow.id)
