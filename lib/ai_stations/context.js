...
const { effectiveActivityWhereClause } = require('../crm_activity_effective');
...
function buildCustomerContext(db, accessContext, customerId, options = {}) {
  ...
  const activities = accountRow && canViewCustomers
    ? db.prepare(`SELECT * FROM crm_activities x
        WHERE x.customer_id=? AND ${effectiveActivityWhereClause(db, 'x')}
          ${columnExists(db, 'crm_activities', 'is_test_data') ? 'AND COALESCE(x.is_test_data,0)=0' : ''}
        ORDER BY x.occurred_at DESC,x.id DESC LIMIT 30`).all(accountRow.id)
