...
function applyAIStationSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS crm_ai_jobs (
      ...
      trigger_source TEXT NOT NULL DEFAULT 'legacy_unknown' CHECK (trigger_source IN (
  ...
  ensureColumn(db, 'crm_ai_jobs', 'execution_mode',
    "TEXT NOT NULL DEFAULT 'online' CHECK (execution_mode IN ('online','batch_eligible'))");
  ensureColumn(db, 'crm_ai_jobs', 'batch_not_before', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_jobs', 'stale_requeue_count', 'INTEGER NOT NULL DEFAULT 0 CHECK (stale_requeue_count >= 0)');
  ensureColumn(db, 'crm_ai_jobs', 'decision_trace_json',
    "TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(decision_trace_json))");
  ensureColumn(db, 'crm_ai_jobs', 'trigger_source', `TEXT NOT NULL DEFAULT 'legacy_unknown' CHECK (
    trigger_source IN (
      'manual','business_event','workflow','schedule','api','migration','release_validation','legacy_unknown'
    )
  )`);
  ensureColumn(db, 'crm_ai_jobs', 'trigger_actor_id', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_jobs', 'trigger_reason', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_jobs', 'triggered_at', "TEXT NOT NULL DEFAULT ''");
  db.exec(`UPDATE crm_ai_jobs SET trigger_source='legacy_unknown'
    WHERE trim(trigger_source)=''`);
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS crm_ai_next_action_consumptions (
      job_id TEXT PRIMARY KEY,
      customer_id TEXT NOT NULL,
      confirmed_by TEXT NOT NULL,
      proposal_json TEXT NOT NULL CHECK (json_valid(proposal_json)),
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_next_action_customer_idx
      ON crm_ai_next_action_consumptions(customer_id,confirmed_at DESC);
  `);
  ...
  ensureColumn(db, 'crm_ai_model_runs', 'request_id', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_model_runs', 'trace_id', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_model_runs', 'finish_reason', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_model_runs', 'original_cost', 'REAL');
  ensureColumn(db, 'crm_ai_model_runs', 'original_currency', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_model_runs', 'pricing_version', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_model_runs', 'fx_version', "TEXT NOT NULL DEFAULT ''");
  ...
  db.exec(`UPDATE crm_ai_jobs SET fairness_at=COALESCE(NULLIF(fairness_at,''),queued_at,created_at)
    WHERE fairness_at=''`);
  db.exec(`
    CREATE TABLE IF NOT EXISTS crm_ai_pricing_catalog (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_pricing_catalog_active_idx
      ON crm_ai_pricing_catalog(provider,model,execution_type,effective_from,effective_to);

    CREATE TABLE IF NOT EXISTS crm_ai_fx_rates (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_fx_rates_active_idx
      ON crm_ai_fx_rates(base_currency,quote_currency,effective_from,effective_to);

    CREATE TABLE IF NOT EXISTS crm_ai_batch_runs (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_batch_runs_state_idx
      ON crm_ai_batch_runs(state,created_at,id);

    CREATE TABLE IF NOT EXISTS crm_ai_batch_items (
      id TEXT PRIMARY KEY,
      run_id TEXT NOT NULL,
      job_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_batch_items_job_idx
      ON crm_ai_batch_items(job_id,created_at,id);
    CREATE INDEX IF NOT EXISTS crm_ai_batch_items_state_idx
      ON crm_ai_batch_items(run_id,state,id);

    CREATE TABLE IF NOT EXISTS crm_ai_feedback_labels (
      id TEXT PRIMARY KEY,
      job_id TEXT NOT NULL,
      result_id TEXT,
      customer_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_feedback_metrics_idx
      ON crm_ai_feedback_labels(station,model,prompt_version,rule_version,created_at);
    CREATE INDEX IF NOT EXISTS crm_ai_feedback_customer_idx
      ON crm_ai_feedback_labels(customer_id,created_at,id);

    CREATE TABLE IF NOT EXISTS crm_ai_strategy_versions (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_strategy_versions_status_idx
      ON crm_ai_strategy_versions(strategy_key,status,created_at,id);

    CREATE TABLE IF NOT EXISTS crm_ai_shadow_evaluations (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS crm_ai_shadow_evaluations_strategy_idx
      ON crm_ai_shadow_evaluations(strategy_version_id,created_at,id);
  `);
  ensureColumn(db, 'crm_ai_batch_runs', 'provider_input_file_id', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_batch_runs', 'provider_output_file_id', "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, 'crm_ai_batch_runs', 'provider_error_file_id', "TEXT NOT NULL DEFAULT ''");
