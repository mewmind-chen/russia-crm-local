...
function authorizedSalesUsers(db) {
  ...
  const archiveClause = db.prepare('PRAGMA table_info(sales_users)').all().some(row => row.name === 'archived_at')
    ? " AND COALESCE(archived_at,'')=''"
    : '';
  return db.prepare(`SELECT * FROM sales_users WHERE role='sales' AND active=1${archiveClause} ORDER BY name,id`).all()
    .filter(row => permissionsForUser(db, row).view_intake);
}

function authorizedSalesUser(db, userId) {
  if (!db || typeof db.prepare !== 'function') throw new Error('database is required');
  const archiveClause = db.prepare('PRAGMA table_info(sales_users)').all().some(candidate => candidate.name === 'archived_at')
    ? " AND COALESCE(archived_at,'')=''"
    : '';
  const row = db.prepare(`SELECT * FROM sales_users WHERE id=? AND role='sales' AND active=1${archiveClause}`)
...
function arbitrateIntakeOwner(input = {}) {
  ...
  if (risk.blocked) {
    return decision({
      ...
      reason: '风险规则要求管理者手动分配，AI 排名不能越过阻断',
  ...
  if (!recommendation.available) {
    if (highValue || risk.elevated || crossTeam) {
      return decision({
        ...
        reason: highValue ? '高价值客户建议管理者手动分配' : risk.elevated ? '高风险客户建议管理者手动分配' : '跨团队客户建议管理者手动分配',
      });
    }
    return decision({
      disposition: 'assign',
  ...
  if (aiUserId !== deterministicUserId) {
    return decision({
      ...
      reason: 'AI 排名与确定性规则结果冲突，建议管理者手动分配',
      aiConfidence: recommendation.confidence,
    });
  }
  return decision({
