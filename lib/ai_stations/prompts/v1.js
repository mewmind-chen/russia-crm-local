...
const STATION_INSTRUCTIONS = Object.freeze({
  ...
  action_proposal: 'Parse the salesperson statement into an editable activity proposal. Write nextActionAt in Asia/Shanghai business local time using exactly YYYY-MM-DD HH:mm:ss, without Z or an offset. Never create an activity or change CRM state. Treat outcome as an optional free-form customer-reaction hint because administrators manage the selectable vocabulary separately. Use empty values and missingFields when the statement does not support a required field. Always require human review.',
  next_action: 'Recommend one concrete next action and due time. Write nextActionAt in Asia/Shanghai business local time using exactly YYYY-MM-DD HH:mm:ss, without Z or an offset. Never update CRM state or schedule reminders. Use empty values and missingFields when evidence is insufficient. Always require human review.',
