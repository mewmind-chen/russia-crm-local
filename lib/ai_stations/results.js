...
function createAIResultStore(db, options = {}) {
  ...
  function saveResult(input, idempotencyKey) {
    ...
    const validated = validateStationOutput(job.station, schemaVersion, input.value, {
      ...
      ...(input.candidateEmployeeIds ? { candidateEmployeeIds: input.candidateEmployeeIds } : {}),
      ...(input.anomalyIds ? {
        anomalyIds: input.anomalyIds,
        anomalyCodes: input.anomalyCodes,
        customerIds: input.customerIds,
      } : {}),
      ...(input.salesUserIds ? {
        salesUserIds: input.salesUserIds,
        sampleSizes: input.sampleSizes,
        sampleStatuses: input.sampleStatuses,
      } : {}),
    });
  ...
  function recordModelRun(input, idempotencyKey) {
    ...
    db.prepare(`INSERT INTO crm_ai_model_runs
      (id,job_id,attempt,station,engine,model,status,duration_ms,usage_json,cost,error_summary,
       idempotency_key,started_at,finished_at,request_id,trace_id,finish_reason,original_cost,
       original_currency,pricing_version,fx_version) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      ...
      String(input.requestId || ''), String(input.traceId || ''), String(input.finishReason || ''),
      input.originalCost === undefined ? null : safeCost(input.originalCost),
      String(input.originalCurrency || ''), String(input.pricingVersion || ''), String(input.fxVersion || ''),
    );
