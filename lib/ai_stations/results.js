...
function createAIResultStore(db, options = {}) {
  ...
  function saveResult(input, idempotencyKey) {
    ...
    if (job.state !== 'running' || job.lease_owner !== workerId) throw new Error('AI job lease is not owned by this worker');
    ...
    const validated = validateStationOutput(job.station, schemaVersion, input.value, { evidenceIds: input.evidenceIds });
