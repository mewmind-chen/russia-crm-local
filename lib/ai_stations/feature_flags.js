...
const FEATURE_DEFINITIONS = Object.freeze({
  ...
  qwen_online: Object.freeze({
    label: '通义千问在线路由',
    env: 'CRM_AI_QWEN_ONLINE_ENABLED',
    defaultOutsideProduction: false,
    requires: Object.freeze(['ai_stations']),
  }),
  qwen_batch: Object.freeze({
    label: '通义千问夜间 Batch',
    env: 'CRM_AI_QWEN_BATCH_ENABLED',
    defaultOutsideProduction: false,
...
function installAIFeatureFlagSchema(db) {
  ...
  const insert = db.prepare(`INSERT OR IGNORE INTO crm_ai_feature_flags
    (feature_key,enabled,updated_by,updated_at) VALUES (?,?,'','')`);
  for (const [key, definition] of Object.entries(FEATURE_DEFINITIONS)) {
    insert.run(key, definition.defaultRuntime === false ? 0 : 1);
  }
}
