...
function activateProtectedCustomer(db, user, externalCustomerId, payload = {}) {
  ...
  return db.transaction(() => {
    ...
    const owner = ownerId && db.prepare(`SELECT id FROM sales_users
      WHERE id=? AND role='sales' AND active=1 AND COALESCE(archived_at,'')=''`).get(ownerId);
    if (!owner) {
    ...
    for (const row of protectedRows) {
      db.prepare(`DELETE FROM crm_customer_identity_registry
        WHERE external_customer_id=? AND source IN (
          ...
          'protected_profile_company_name'
        )`)
...
function listProtectedCustomers(db, user, options = {}) {
  ...
  const customFields = {
    ...
    alpha_nickname: 'p.alpha_nickname',
    crm_nickname: "LOWER(COALESCE(NULLIF(a.nickname,''),NULLIF(m.nickname,''),''))",
    company_name: "LOWER(COALESCE(NULLIF(a.company_name,''),NULLIF(m.company_name,''),NULLIF(p.alpha_nickname,''),''))",
    country: "LOWER(COALESCE(a.country,m.country,''))",
    city: "LOWER(COALESCE(a.city,m.city,''))",
    website: "LOWER(COALESCE(a.website,m.website,''))",
    industry: "LOWER(COALESCE(a.industry,m.industry,''))",
    customer_type: "LOWER(COALESCE(a.customer_type,m.customer_type,''))",
    product_focus: "LOWER(COALESCE(a.product_focus,m.products,''))",
    status: 'p.status',
    batch_id: 'p.batch_id',
    created_at: 'p.created_at',
    activated_at: "COALESCE(p.activated_at,'')",
    updated_at: "COALESCE(p.updated_at,'')",
  };
  const customSort = parseSortDescriptors(sort, customFields);
  const sortSql = {
    created_desc: 'p.created_at DESC,p.external_customer_id',
    created_asc: 'p.created_at ASC,p.external_customer_id',
    company_asc: "LOWER(COALESCE(NULLIF(a.company_name,''),NULLIF(m.company_name,''),NULLIF(p.alpha_nickname,''),'')) COLLATE NOCASE ASC,p.external_customer_id",
    status_priority: "CASE p.status WHEN 'protected' THEN 1 WHEN 'activated' THEN 2 WHEN 'withdrawn' THEN 3 ELSE 9 END,p.created_at DESC,p.external_customer_id",
    country_asc: 'COALESCE(a.country,m.country) COLLATE NOCASE ASC,p.external_customer_id',
  }[sort] || (customSort.length
    ? orderByForSort(sort, customFields, { tieBreaker: 'p.external_customer_id ASC' }).orderBy
    : '');
  if (!sortSql) throw lifecycleError('排序条件未获授权', 'SORT_NOT_AUTHORIZED', 403);
  const conditions = [];
  const params = [];
  if (['protected', 'activated', 'withdrawn'].includes(status)) {
    conditions.push('p.status=?');
    params.push(status);
  }
  if (!['all', 'protected', 'activated', 'withdrawn'].includes(status)) {
    throw lifecycleError('保护状态筛选无效', 'PROTECTED_CUSTOMER_STATUS_INVALID');
  }
  ...
  const rows = db.prepare(`SELECT p.external_customer_id externalCustomerId,
      ...
      p.batch_id batchId,p.created_at createdAt,p.activated_at activatedAt,
      p.updated_at updatedAt,m.company_name masterCompanyName,m.nickname masterNickname,
      m.country masterCountry,m.city masterCity,m.website masterWebsite,m.industry masterIndustry,
      m.customer_type masterCustomerType,m.products masterProductFocus,
      a.company_name accountCompanyName,a.nickname accountNickname,a.country accountCountry,
      a.city accountCity,a.website accountWebsite,a.industry accountIndustry,
      a.customer_type accountCustomerType,a.product_focus accountProductFocus,
      br.payload_json payloadJson
    FROM crm_protected_customers p
    LEFT JOIN customer_pool m ON m.customer_id=p.external_customer_id
    LEFT JOIN crm_accounts a ON a.id=p.activated_account_id
    LEFT JOIN crm_protected_customer_batch_rows br
      ON br.batch_id=p.batch_id AND br.external_customer_id=p.external_customer_id
    ${where} ORDER BY ${sortSql}`).all(...params)
    .map(protectedCustomerView)
    .filter(row => !query || [
      row.alphaNickname,
      row.crmNickname,
      row.companyName,
      row.externalCustomerId,
    ].join('\n').toLocaleLowerCase('und').includes(query));
  return {
    items: rows,
    total: rows.length,
    writeEnabled: protectedCustomerWritesEnabled(),
  };
}
...
function protectedCustomerView(row) {
  const payload = parsePayload(row.payloadJson);
  return {
    externalCustomerId: row.externalCustomerId,
    alphaNickname: String(row.alphaNickname || ''),
    crmNickname: firstText(row.accountNickname, row.masterNickname),
    companyName: firstText(row.accountCompanyName, payload.companyName, row.masterCompanyName),
    country: firstText(row.accountCountry, row.masterCountry, payload.country),
    city: firstText(row.accountCity, row.masterCity, payload.city),
    website: firstText(row.accountWebsite, row.masterWebsite, payload.website),
    industry: firstText(row.accountIndustry, row.masterIndustry, payload.industry),
    customerType: firstText(row.accountCustomerType, row.masterCustomerType, payload.customerType),
    productFocus: firstText(row.accountProductFocus, row.masterProductFocus, payload.productFocus),
    status: row.status,
...
function protectedCustomerQuery(db, externalCustomerId) {
  return db.prepare(`SELECT p.external_customer_id externalCustomerId,
      p.alpha_nickname alphaNickname,p.status,p.activated_account_id accountId,
      p.batch_id batchId,p.created_at createdAt,p.activated_at activatedAt,
      p.updated_at updatedAt,m.company_name masterCompanyName,m.nickname masterNickname,
      m.country masterCountry,m.city masterCity,m.website masterWebsite,m.industry masterIndustry,
    ...
    LEFT JOIN crm_protected_customer_batch_rows br
      ON br.batch_id=p.batch_id AND br.external_customer_id=p.external_customer_id
    WHERE p.external_customer_id=?`).get(externalCustomerId);
}

function getProtectedCustomer(db, user, externalCustomerId) {
  assertProtectedCustomerAdmin(user);
  installProtectedCustomers(db);
  const cleanId = String(externalCustomerId || '').trim();
  const row = protectedCustomerQuery(db, cleanId);
  if (!row) throw lifecycleError('保护客户不存在', 'PROTECTED_CUSTOMER_NOT_FOUND', 404);
  const audit = db.prepare(`SELECT action,actor_id actorId,detail_json detailJson,
      created_at createdAt FROM crm_protected_customer_audit
    WHERE external_customer_id=? ORDER BY created_at DESC,rowid DESC`).all(cleanId)
    .map(item => ({
      action: item.action,
      actorId: item.actorId,
      details: parsePayload(item.detailJson),
      createdAt: item.createdAt,
    }));
  return {
    customer: protectedCustomerView(row),
    audit,
    writeEnabled: protectedCustomerWritesEnabled(),
  };
}
...
const PROFILE_FIELDS = Object.freeze({
  companyName: ['company_name', 300],
  country: ['country', 120],
  city: ['city', 160],
  website: ['website', 1000],
  industry: ['industry', 300],
  customerType: ['customer_type', 160],
  productFocus: ['products', 2000],
});

function profileValue(value, maxLength) {
  const text = String(value ?? '').normalize('NFKC').trim();
  if (text.length > maxLength) {
    throw lifecycleError('保护客户资料字段过长', 'PROTECTED_CUSTOMER_PROFILE_FIELD_TOO_LONG', 422);
  }
  return text;
}

function updateProtectedCustomer(db, user, externalCustomerId, input = {}) {
  assertProtectedCustomerAdmin(user);
  assertProtectedCustomerWritesEnabled();
  installProtectedCustomers(db);
  const cleanId = String(externalCustomerId || '').trim();
  return db.transaction(() => {
    ...
    if (protectedRow.status !== 'protected') {
      throw lifecycleError(
        '已激活客户请在 CRM 主档中维护资料',
        'PROTECTED_CUSTOMER_ALREADY_ACTIVATED',
        409,
      );
    }
    const master = db.prepare('SELECT * FROM customer_pool WHERE customer_id=?').get(cleanId);
    const changes = {};
    for (const [field, [column, maxLength]] of Object.entries(PROFILE_FIELDS)) {
      if (Object.prototype.hasOwnProperty.call(input, field)) {
        changes[column] = profileValue(input[field], maxLength);
      }
    }
    if (!Object.keys(changes).length) {
      throw lifecycleError('没有可更新的保护客户资料', 'PROTECTED_CUSTOMER_PROFILE_EMPTY');
    }
    if (Object.prototype.hasOwnProperty.call(changes, 'company_name')) {
      const oldName = String(master?.company_name || '').trim();
      const newName = changes.company_name;
      const oldNormalized = normalizeCustomerName(oldName);
      const newNormalized = normalizeCustomerName(newName);
      if (newNormalized && identityOwnersForName(db, newNormalized, cleanId).length) {
        throw lifecycleError(
          '客户名称需要管理员核验后才能使用',
          'CUSTOMER_IDENTITY_REVIEW_REQUIRED',
          409,
      ...
      if (oldNormalized && oldNormalized !== newNormalized) {
        db.prepare(`DELETE FROM crm_customer_identity_registry
          WHERE normalized_name=? AND external_customer_id=? AND source='protected_profile_company_name'`)
          .run(oldNormalized, cleanId);
      }
      if (newName) {
        reserveCustomerIdentity(db, {
          externalCustomerId: cleanId,
          name: newName,
          source: 'protected_profile_company_name',
          actorId: user.id,
        });
      }
    }
    const timestamp = nowText();
    const assignments = [...Object.keys(changes).map(column => `${column}=?`), 'updated_at=?'];
    db.prepare(`UPDATE customer_pool SET ${assignments.join(',')} WHERE customer_id=?`)
      .run(...Object.values(changes), timestamp, cleanId);
    const batchRow = db.prepare(`SELECT row_id,payload_json FROM crm_protected_customer_batch_rows
      WHERE batch_id=? AND external_customer_id=?`).get(protectedRow.batch_id, cleanId);
    if (batchRow) {
      const payload = parsePayload(batchRow.payload_json);
      for (const [field, [column]] of Object.entries(PROFILE_FIELDS)) {
        if (Object.prototype.hasOwnProperty.call(changes, column)) payload[field] = changes[column];
      }
      db.prepare(`UPDATE crm_protected_customer_batch_rows SET payload_json=?,updated_at=?
        WHERE row_id=?`).run(JSON.stringify(payload), timestamp, batchRow.row_id);
    }
    db.prepare(`UPDATE crm_protected_customers SET updated_at=? WHERE external_customer_id=?`)
      .run(timestamp, cleanId);
    db.prepare(`INSERT INTO crm_protected_customer_audit
      (id,action,batch_id,external_customer_id,actor_id,detail_json,created_at)
      VALUES (?,?,?,?,?,?,?)`).run(
      createId('PCA'), 'protected_customer_profile_updated', protectedRow.batch_id,
      cleanId, user.id, JSON.stringify({ fields: Object.keys(changes) }), timestamp,
    );
    return getProtectedCustomer(db, user, cleanId);
  }).immediate();
}
