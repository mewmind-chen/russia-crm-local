...
const MANAGER_METRIC_FILTERS = new Set([
  'search', 'owner', 'stage', 'created_at', 'task_status', 'task_reason',
  'task_resolved_at', 'metric_window',
]);
...
function listManagerTaskRows(db, user, ast = { page: 'manager_tasks', filters: [] }, input = {}) {
  ...
  const total = Number(db.prepare(`SELECT COUNT(*) total ${managerTaskFrom()} ${where}`)
    .get(...base.params).total || 0);
...
function listManagerMetricRows(db, user, ast = { page: 'manager_metrics', filters: [] }, input = {}, options = {}) {
  ...
  const taskFilters = filters.filter(filter => filter.key !== 'metric_window');
  const scope = managerTaskScope(user, 'manager_metrics', {
    page: 'manager_metrics', filters: taskFilters,
  });
  const matchingOwners = new Set(db.prepare(`SELECT DISTINCT
      COALESCE(NULLIF(t.owner_id_snapshot,''),a.owner_id) owner_id
    ${managerTaskFrom()} WHERE ${scope.conditions.join(' AND ')}`)
    .all(...scope.params).map(row => String(row.owner_id || '')).filter(Boolean));
  const { buildManagerMetrics } = require('./manager_metrics');
  const all = windows.flatMap(rangeDays => buildManagerMetrics(db, {
    user,
    rangeDays,
    now: options.now,
    settings: options.settings,
  }).sales.map(row => ({ ...row, rangeDays })));
  const needsTaskMatch = taskFilters.length > 0;
  const filtered = needsTaskMatch ? all.filter(row => matchingOwners.has(String(row.actorId))) : all;
  const pageInfo = pagination(input);
...
function notificationScope(user, ast) {
...
function listNotificationRows(db, user, ast = { page: 'notifications', filters: [] }, input = {}) {
  const pageInfo = pagination(input);
  const scope = notificationScope(user, ast);
  const unfiltered = notificationScope(user, { page: 'notifications', filters: [] });
  ...
  const authorizedTotal = Number(db.prepare(`SELECT COUNT(*) total FROM crm_notifications n
    WHERE ${unfiltered.conditions.join(' AND ')}`).get(...unfiltered.params).total || 0);
  let rows = db.prepare(`SELECT n.*,recipient.name recipient_name FROM crm_notifications n
...
function businessFilterOptions(db, user, pageKey, authorizedFields, options = {}) {
  ...
  if (pageKey === 'pipeline') rows = collectPaged(input =>
    ...
    listNotificationRows(db, user, { page: pageKey, filters: [] }, input));
  const property = {
