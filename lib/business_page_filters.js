...
const { hasPermission, accountVisibilityScope, contactSafePipelineRecord, contactSafeInsightsRecord, contactSafeAlertsRecord, contactSafeNotificationRecord } = require('./access_control');
const { astWithoutField, overlayOptionCounts } = require('./filter_option_linkage');
const { STAGE_LABELS } = require('./customer_stages');
const { projectNextAction, projectManagerState } = require('./domains/lifecycle/state_projection');
...
const { compareSortValues, orderByForSort, parseSortDescriptors } = require('./list_sort');
...
const PIPELINE_SORT_VALUES = Object.freeze([
  'pending_action', 'recent_activity', 'stage_asc', 'company_asc',
]);
const PIPELINE_SORT_FIELDS = Object.freeze({
  company_name: row => row.company_name,
  stage: row => row.stageLabel || row.stage,
  next_action_at: row => row.next_action_at,
  owner_name: row => row.owner_name,
  priority: row => row.priority,
  last_activity_at: row => row.last_activity_at,
  updated_at: row => row.updated_at,
});
const ALERT_SORT_VALUES = Object.freeze([
  'urgency_priority', 'due_at_asc', 'recent_update', 'company_asc',
]);
const ALERT_SORT_FIELDS = Object.freeze({
  urgency: row => ({ immediate: 0, today: 1, attention: 2 }[row.urgency] ?? 9),
  companyName: row => row.companyName,
  reasonCount: row => row.reasonCount,
  dueAt: row => row.dueAt,
  ownerName: row => row.ownerName,
  updatedAt: row => row.updatedAt,
});
const NOTIFICATION_SORT_VALUES = Object.freeze([
  'unread_priority', 'recent_update', 'severity_priority', 'title_asc',
]);
const NOTIFICATION_SORT_FIELDS = Object.freeze({
  status: "CASE n.status WHEN 'unread' THEN 0 ELSE 1 END",
  title: "LOWER(COALESCE(n.title,''))",
  customerName: "LOWER(COALESCE((SELECT company_name FROM crm_accounts WHERE id=n.customer_id),(SELECT company_name FROM customer_pool WHERE customer_id=n.customer_id),''))",
  detail: "LOWER(COALESCE(n.detail,''))",
  createdAt: "COALESCE(n.created_at,'')",
  deliveryStatus: "CASE WHEN n.wecom_status='failed' OR EXISTS (SELECT 1 FROM crm_notification_deliveries d WHERE d.notification_id=n.id AND d.channel='wecom' AND d.status='failed') THEN 0 ELSE 1 END",
});
const INSIGHT_SORT_VALUES = Object.freeze([
...
const INSIGHT_SORT_FIELDS = Object.freeze({
  company_name: 'LOWER(COALESCE(NULLIF(p.company_name,\'\'),a.company_name,\'\'))',
  stage: 'LOWER(COALESCE(a.stage,\'\'))',
  country: 'LOWER(COALESCE(a.country,\'\'))',
  owner_name: 'LOWER(COALESCE(owner.name,\'\'))',
  evaluation_status: 'CASE WHEN COALESCE(er.evaluation_count,0)=0 THEN 0 ELSE 1 END',
  evaluation_text: "LOWER(COALESCE(latest.evaluation_text,''))",
  evaluation_count: 'COALESCE(er.evaluation_count,0)',
  evaluated_at: "COALESCE(latest.updated_at,'')",
});
...
const RECYCLE_SORT_VALUES = Object.freeze([
  'recycled_desc', 'recycled_asc', 'company_asc', 'reason_asc',
]);
const RECYCLE_SORT_FIELDS = Object.freeze({
  company_name: row => row.companyName,
  previous_owner_name: row => row.previousOwnerName,
  reason: row => row.reason,
  recycled_at: row => row.recycledAt,
});
const MANAGER_TASK_SORT_FIELDS = Object.freeze({
  company_name: "LOWER(COALESCE(NULLIF(p.company_name,''),a.company_name,''))",
  companyName: "LOWER(COALESCE(NULLIF(p.company_name,''),a.company_name,''))",
  customer_id: 't.customer_id',
  customerId: 't.customer_id',
  status: 't.status',
  owner_name: "LOWER(COALESCE(owner.name,''))",
  ownerName: "LOWER(COALESCE(owner.name,''))",
  reason: "LOWER(COALESCE(t.reason,''))",
  due_at: "COALESCE(t.due_at,'')",
  dueAt: "COALESCE(t.due_at,'')",
  triggered_at: "COALESCE(t.triggered_at,'')",
  triggeredAt: "COALESCE(t.triggered_at,'')",
});
...
function sortRowsByDescriptors(rows, descriptors, tieBreaker = row => row?.id || row?.recordKey || '') {
  return rows.map((row, index) => ({ row, index })).sort((left, right) => {
    for (const descriptor of descriptors) {
      const getter = typeof descriptor.expression === 'function'
        ? descriptor.expression
        : row => row?.[descriptor.field];
      const compared = compareSortValues(getter(left.row), getter(right.row));
      if (compared) return descriptor.direction === 'desc' ? -compared : compared;
    }
    const tied = compareSortValues(tieBreaker(left.row), tieBreaker(right.row));
...
function accountScope(user, alias = 'a') {
  return accountVisibilityScope(user, alias);
}
...
function pipelineActionKeys(row) {
  const keys = new Set();
  ...
  if (projectNextAction(row).overdue
      && !['won', 'repeat', 'lost', 'disqualified'].includes(String(row.stage || ''))) {
    keys.add('due_followup');
  }
  const manager = projectManagerState(row);
  if (manager.required && manager.status !== '已完成') {
    keys.add('manager_assistance');
  }
  return [...keys];
}

function publicPipelineActionRow(row) {
  const actionQueueKeys = pipelineActionKeys(row);
  return {
    ...row,
...
function listPipelineRows(db, user, ast = { page: 'pipeline', filters: [] }, input = {}) {
  ...
  if (!hasPermission(user, 'view_contacts')) rawRows = contactSafePipelineRecord(rawRows);
  ...
  const matchingRows = actionQueue === 'need_decision'
    ...
    : actionQueue === 'worth_deepening'
      ...
      : starRows;
  const customSort = parseSortDescriptors(input.sort, PIPELINE_SORT_FIELDS);
  if (customSort.length) {
    const sortedRows = sortRowsByDescriptors(matchingRows, customSort, row => row?.id || '');
    const total = sortedRows.length;
    const rows = sortedRows.slice(pageInfo.offset, pageInfo.offset + pageInfo.pageSize);
    return {
      rows, ...pageInfo, total, authorizedTotal, summary, actionQueue, starView,
      hasMore: pageInfo.offset + rows.length < total,
    };
  }
  const requestedSort = String(input.sort || 'pending_action').trim() || 'pending_action';
  const compareText = (left, right) => String(left || '').localeCompare(String(right || ''), 'zh-CN');
  const compareDate = (left, right, direction = 'asc') => {
  ...
  const comparators = {
    pending_action: (left, right) => compareDate(left.next_action_at, right.next_action_at)
      || Number(left.priority || 0) - Number(right.priority || 0)
      || compareDate(left.updated_at, right.updated_at, 'desc'),
    recent_activity: (left, right) => compareDate(left.last_activity_at, right.last_activity_at, 'desc')
      || compareDate(left.updated_at, right.updated_at, 'desc'),
    stage_asc: (left, right) => compareText(left.stageLabel || left.stage, right.stageLabel || right.stage)
      || compareDate(left.updated_at, right.updated_at, 'desc'),
    company_asc: (left, right) => compareText(left.company_name, right.company_name),
  };
  const compare = comparators[requestedSort] || comparators.pending_action;
  matchingRows.sort((left, right) => compare(left, right) || String(left.id).localeCompare(String(right.id)));
...
function listTodayTasks(db, user, ast = { page: 'alerts', filters: [] }, input = {}, options = {}) {
  ...
  const visible = urgency
    ...
    : rows;
  const customSort = parseSortDescriptors(input.sort, ALERT_SORT_FIELDS);
  if (customSort.length) {
    const sortedRows = sortRowsByDescriptors(visible, customSort, row => row?.id || '');
    const paged = sortedRows.slice(pageInfo.offset, pageInfo.offset + pageInfo.pageSize);
    const summary = {
      objects: rows.length,
    ...
    const safeRows = hasPermission(user, 'view_contacts') ? paged : contactSafeAlertsRecord(paged);
    return {
      rows: safeRows,
      ...pageInfo,
      ...
      summary,
      hasMore: pageInfo.offset + safeRows.length < sortedRows.length,
    };
  }
  const requestedSort = String(input.sort || 'urgency_priority').trim() || 'urgency_priority';
  const compareText = (left, right) => String(left || '').localeCompare(String(right || ''), 'zh-CN');
  const compareDate = (left, right, direction = 'asc') => {
    const leftValue = String(left || '');
    const rightValue = String(right || '');
  ...
  const comparators = {
    urgency_priority: (left, right) => (urgencyOrder[left.urgency] ?? 9) - (urgencyOrder[right.urgency] ?? 9)
      || Number(right.maxOverdueHours || 0) - Number(left.maxOverdueHours || 0)
      || compareDate(left.updatedAt, right.updatedAt, 'desc'),
    due_at_asc: (left, right) => compareDate(left.dueAt, right.dueAt)
      || (urgencyOrder[left.urgency] ?? 9) - (urgencyOrder[right.urgency] ?? 9),
    recent_update: (left, right) => compareDate(left.updatedAt, right.updatedAt, 'desc')
      || (urgencyOrder[left.urgency] ?? 9) - (urgencyOrder[right.urgency] ?? 9),
    company_asc: (left, right) => compareText(left.companyName, right.companyName)
      || (urgencyOrder[left.urgency] ?? 9) - (urgencyOrder[right.urgency] ?? 9),
  };
  const compare = comparators[requestedSort] || comparators.urgency_priority;
  visible.sort((left, right) => {
    const result = compare(left, right);
    return result || String(left.id || '').localeCompare(String(right.id || ''));
  });
  const summary = {
    objects: rows.length,
    reasons: rows.reduce((sum, item) => sum + Number(item.reasonCount || 0), 0),
  ...
  if (!hasPermission(user, 'view_contacts')) {
    paged = contactSafeAlertsRecord(paged);
  }
  return {
    rows: paged,
    ...pageInfo,
...
function listManagerEvaluationCustomers(db, user, ast = { page: 'insights', filters: [] }, input = {}, options = {}) {
  ...
  const pageInfo = pagination(input);
  const customSort = parseSortDescriptors(input.sort, INSIGHT_SORT_FIELDS);
  const sortPreset = String(input.sort || 'evaluation_updated_desc').trim() || 'evaluation_updated_desc';
  const scope = accountScope(user);
  ...
  const presetOrderBy = {
    evaluation_updated_desc: "CASE WHEN latest.updated_at IS NULL THEN 1 ELSE 0 END,latest.updated_at DESC",
    evaluation_updated_asc: "CASE WHEN latest.updated_at IS NULL THEN 0 ELSE 1 END,latest.updated_at ASC",
    evaluation_status_priority: "CASE WHEN COALESCE(er.evaluation_count,0)=0 THEN 0 ELSE 1 END,latest.updated_at DESC",
    evaluation_count_desc: "COALESCE(er.evaluation_count,0) DESC,latest.updated_at DESC",
    company_asc: "LOWER(COALESCE(NULLIF(p.company_name,''),a.company_name,'')) ASC",
    owner_asc: "LOWER(COALESCE(owner.name,'')) ASC",
    stage_asc: "LOWER(COALESCE(a.stage,'')) ASC",
  }[sortPreset] || "CASE WHEN latest.updated_at IS NULL THEN 1 ELSE 0 END,latest.updated_at DESC";
  const orderBy = customSort.length
    ? orderByForSort(input.sort, INSIGHT_SORT_FIELDS, { tieBreaker: 'a.id ASC' }).orderBy
    : presetOrderBy;
  ...
  if (!hasPermission(user, 'view_contacts')) rows = contactSafeInsightsRecord(rows);
...
function listRecycleRows(db, user, ast = { page: 'recycle_bin', filters: [] }, input = {}) {
  ...
  const customSort = parseSortDescriptors(input.sort, RECYCLE_SORT_FIELDS);
  if (customSort.length) {
    const sortedRows = sortRowsByDescriptors(filteredRows, customSort, row => row?.recordKey || '');
    const rows = sortedRows.slice(pageInfo.offset, pageInfo.offset + pageInfo.pageSize);
    return {
      rows,
      ...pageInfo,
      ...
      hasMore: pageInfo.offset + rows.length < sortedRows.length,
    };
  }
  const requestedSort = String(input.sort || 'recycled_desc').trim() || 'recycled_desc';
  const compareText = (left, right) => String(left || '').localeCompare(String(right || ''), 'zh-CN');
  const comparators = {
    recycled_desc: (left, right) => String(right.recycledAt || '').localeCompare(String(left.recycledAt || '')),
    recycled_asc: (left, right) => String(left.recycledAt || '').localeCompare(String(right.recycledAt || '')),
    company_asc: (left, right) => compareText(left.companyName, right.companyName),
    reason_asc: (left, right) => compareText(left.reason, right.reason),
  };
  const compare = comparators[requestedSort] || comparators.recycled_desc;
  filteredRows.sort((left, right) => compare(left, right) || left.recordKey.localeCompare(right.recordKey));
...
function listManagerTaskRows(db, user, ast = { page: 'manager_tasks', filters: [] }, input = {}) {
  ...
  const customSort = parseSortDescriptors(input.sort, MANAGER_TASK_SORT_FIELDS);
  const orderBy = customSort.length
    ? orderByForSort(input.sort, MANAGER_TASK_SORT_FIELDS, { tieBreaker: 't.id ASC' }).orderBy
    : `CASE t.status WHEN 'escalated' THEN 0 WHEN 'overdue' THEN 1 WHEN 'open' THEN 2 ELSE 3 END,
      t.due_at,t.triggered_at DESC,t.id`;
...
function listManagerMetricRows(db, user, ast = { page: 'manager_metrics', filters: [] }, input = {}, options = {}) {
  ...
  const filtered = hasMetricScope
    ...
    : authorized;
  const customSort = parseSortDescriptors(input.sort, {
    actorName: row => row.actorName,
    rangeDays: row => row.rangeDays,
    activeCustomers: row => row.counts?.activeCustomers,
    deferredCustomers: row => row.counts?.deferredCustomers,
    thresholdCustomers: row => row.counts?.thresholdCustomers,
    plannedAfterDeferredCustomers: row => row.counts?.plannedAfterDeferredCustomers,
    onTimeActionCustomers: row => row.counts?.onTimeActionCustomers,
    firstTouchSilentCustomers: row => row.counts?.firstTouchSilentCustomers,
    unimprovedAfterInterventionCustomers: row => row.counts?.unimprovedAfterInterventionCustomers,
    needsManagerReview: row => row.needsManagerReview ? 1 : 0,
  });
  const sorted = customSort.length
    ? sortRowsByDescriptors(filtered, customSort, row => `${row.actorId || row.actorName || ''}:${row.rangeDays || ''}`)
    : filtered;
  const summaries = Object.fromEntries(windows.map(rangeDays => {
    const rangeRows = sorted.filter(row => Number(row.rangeDays) === rangeDays);
    return [String(rangeDays), managerMetricSummary(rangeRows, rangeDays)];
  }));
  const pageInfo = pagination(input);
  const rows = sorted.slice(pageInfo.offset, pageInfo.offset + pageInfo.pageSize);
  return {
    rows,
    ...pageInfo,
    total: sorted.length,
    ...
    hasMore: pageInfo.offset + rows.length < sorted.length,
  };
}
...
function listNotificationRows(
  ...
  const customSort = parseSortDescriptors(input.sort, NOTIFICATION_SORT_FIELDS);
  const requestedSort = String(input.sort || 'unread_priority').trim() || 'unread_priority';
  const notificationOrderBy = {
    unread_priority: "CASE n.status WHEN 'unread' THEN 0 ELSE 1 END,n.created_at DESC,n.id",
    recent_update: 'n.created_at DESC,n.id',
    severity_priority: "CASE n.severity WHEN 'critical' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END,n.created_at DESC,n.id",
    title_asc: "LOWER(COALESCE(n.title,'')) ASC,n.created_at DESC,n.id",
  };
  const orderBy = customSort.length
    ? orderByForSort(input.sort, NOTIFICATION_SORT_FIELDS, { tieBreaker: 'n.id ASC' }).orderBy
    : notificationOrderBy[requestedSort] || notificationOrderBy.unread_priority;
  ...
  if (!hasPermission(user, 'view_contacts')) {
    rows = contactSafeNotificationRecord(rows);
    if (user?.role === 'sales') {
...
function businessFilterOptions(db, user, pageKey, authorizedFields, options = {}) {
  ...
  const linkageAst = options.linkageAst && typeof options.linkageAst === 'object'
    ? options.linkageAst
    : { page: pageKey, filters: [] };
  const rowCache = new Map();
  function rowsFor(ast) {
    const filters = Array.isArray(ast?.filters) ? ast.filters : [];
    const cacheKey = JSON.stringify(filters);
    if (rowCache.has(cacheKey)) return rowCache.get(cacheKey);
    const pageAst = { page: pageKey, filters };
    let rows;
    if (pageKey === 'pipeline') rows = collectPaged(input =>
      listPipelineRows(db, user, pageAst, input));
    else if (pageKey === 'alerts') {
      const all = allTodayTasks(db, user, options);
      rows = filters.length
        ? all.filter(item => filters.every(filter => alertMatches(user, item, filter)))
        : all;
    } else if (pageKey === 'insights') rows = collectPaged(input =>
      listManagerEvaluationCustomers(db, user, pageAst, input, options));
    else if (pageKey === 'recycle_bin') rows = collectPaged(input =>
      listRecycleRows(db, user, pageAst, input));
    else if (pageKey === 'manager_tasks') rows = collectPaged(input =>
      listManagerTaskRows(db, user, pageAst, input));
    else if (pageKey === 'manager_risks') rows = collectPaged(input =>
      listManagerRiskRows(db, user, pageAst, input));
    else if (pageKey === 'manager_metrics') rows = collectPaged(input =>
      listManagerMetricRows(db, user, pageAst, input, options));
    else rows = collectPaged(input =>
      listNotificationRows(db, user, pageAst, input, options));
    rowCache.set(cacheKey, rows);
    return rows;
  }
  const catalogRows = rowsFor({ page: pageKey, filters: [] });
  const property = {
  ...
  for (const key of keys) {
    ...
    if (TAG_CATEGORIES[key]) {
      const catalog = tagOptions(db, user, TAG_CATEGORIES[key]);
      const remaining = astWithoutField(linkageAst, key);
      result[key] = remaining.filters?.length
        ? overlayOptionCounts(catalog, tagOptions(db, user, TAG_CATEGORIES[key], remaining))
        : catalog;
      continue;
    }
    const catalog = countsFrom(catalogRows, key);
    const remaining = astWithoutField(linkageAst, key);
    result[key] = remaining.filters?.length
      ? overlayOptionCounts(catalog, countsFrom(rowsFor(remaining), key))
      : catalog;
  }
  return result;
}

function tagOptions(db, user, category, extraAst = { filters: [] }) {
  const scope = accountScope(user);
  const conditions = [...scope.conditions];
  const params = [...scope.params];
  for (const filter of extraAst.filters || []) {
    addAccountFilter(conditions, params, filter, { user });
  }
  return db.prepare(`SELECT CAST(t.id AS TEXT) value,t.name label,COUNT(DISTINCT a.id) count
    ...
    WHERE ${conditions.join(' AND ')} AND t.category=?
    GROUP BY t.id,t.name ORDER BY t.name COLLATE NOCASE`).all(...params, category).map(row => ({
