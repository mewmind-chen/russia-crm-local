...
function listRecycleRows(db, user, ast = { page: 'recycle_bin', filters: [] }, input = {}) {
  ...
  const accountRows = db.prepare(`SELECT a.id,a.external_customer_id,
    ...
    .map(row => ({
      ...
      actions: row.recycle_kind === 'manual_delete' && canRestore ? ['restore'] : [],
    }));
  const intakeConditions = [
    "i.status='rejected'",
    "COALESCE(i.crm_customer_id,'')=''",
    "COALESCE(i.rejected_at,'')!=''",
  ];
  const intakeParams = [];
  if (!hasPermission(user, 'view_all_customers')) {
    intakeConditions.push('(i.previous_owner_id=? OR i.rejected_by=?)');
    intakeParams.push(String(user?.id || ''), String(user?.id || ''));
  }
  const intakeRows = db.prepare(`SELECT i.id,i.external_customer_id,i.company_name,i.country,
    ...
    WHERE ${intakeConditions.join(' AND ')}`)
    .all(...intakeParams)
    .map(row => ({
