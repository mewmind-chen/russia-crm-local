...
function listPipelineRows(db, user, ast = { page: 'pipeline', filters: [] }, input = {}) {
  ...
  const rawRows = db.prepare(`SELECT DISTINCT a.*,
  ...
  const summary = pipelineActionSummary(rawRows);
  const matchingRows = actionQueue === 'need_decision'
    ? rawRows.filter(row => row.actionQueueKeys.some(key =>
    ...
    : actionQueue === 'worth_deepening'
      ...
      : actionQueue
        ? rawRows.filter(row => row.actionQueueKeys.includes(actionQueue))
        : rawRows;
  const total = matchingRows.length;
  let rows = matchingRows.slice(pageInfo.offset, pageInfo.offset + pageInfo.pageSize);
  if (!hasPermission(user, 'view_contacts')) rows = redactContactFields(rows);
  return {
    rows, ...pageInfo, total, authorizedTotal, summary, actionQueue,
