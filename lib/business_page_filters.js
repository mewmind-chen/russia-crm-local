...
function addAccountFilter(conditions, params, filter, options = {}) {
  ...
  if (filter.key === 'search') {
    ...
    const searchable = [
      'a.id', 'a.external_customer_id', 'a.nickname',
...
function listManagerEvaluationCustomers(db, user, ast = { page: 'insights', filters: [] }, input = {}) {
  ...
  for (const filter of filters) {
    if (filter.key === 'search') {
      ...
      conditions.push(`(LOWER(COALESCE(a.company_name,'')) LIKE LOWER(?) ESCAPE '\\'
        OR LOWER(COALESCE(a.nickname,'')) LIKE LOWER(?) ESCAPE '\\'
  ...
  let rows = db.prepare(`${cte} SELECT DISTINCT a.id customerId,a.external_customer_id externalCustomerId,
    COALESCE(NULLIF(p.company_name,''),a.company_name) companyName,a.nickname,a.country,a.city,a.stage,a.priority,
...
function listRecycleRows(db, user, ast = { page: 'recycle_bin', filters: [] }, input = {}) {
  ...
  for (const filter of filters) {
    if (filter.key === 'search') {
      const like = `%${escapeLike(textFor(filter))}%`;
      const searchable = [
        'a.id', 'a.external_customer_id', 'a.nickname', 'a.company_name', 'a.country',
      ];
  ...
  const where = `WHERE ${conditions.join(' AND ')}`;
  const total = Number(db.prepare(`SELECT COUNT(*) total FROM crm_accounts a ${where}`)
    .get(...params).total || 0);
  ...
  const rows = db.prepare(`SELECT a.id,a.external_customer_id,a.nickname,a.company_name,a.country,a.stage,
