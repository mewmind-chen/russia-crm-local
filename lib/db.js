...
// Templates are a legacy development resource, not a generic row transport.
// Keep the established camelCase contract while dropping future/unknown
// columns introduced by schema migrations. This closes the `SELECT *` shape
// without changing the six fields consumed by the legacy template renderer.
...
function getInitialData(accessContext) {
  ...
  try {
    ...
    const customers = permissions.view_customers
      ...
      : [];
    const visibleCustomers = permissions.view_contacts
      ? customers
      : customers.map(contactSafeCustomerRecord);
    ...
    const templates = permissions.view_development
      ? db.prepare('SELECT * FROM templates ORDER BY id').all().map(buildTemplate)
      : [];
    ...
    const payload = {
      ...
      customers: visibleCustomers, customerPool, stats, templates,
...
function getCustomerProfileData(accessContext, customerId, options = {}) {
  ...
  try {
    ...
    const adminMasterAccess = accessContext?.user?.role === 'admin';
    const recycleReadOnly = Boolean(options.recycleReadOnly);
    const intakeReadOnly = Boolean(options.intakeReadOnly && !adminMasterAccess);
    if (!intakeReadOnly) assertExternalCustomerAccess(accessContext, cleanId);
    const permissions = accessContext?.permissions || {};
    const linkedAccount = value.prepare(`SELECT a.id,a.source,a.created_by,a.created_at,a.updated_at,
        creator.name creator_name
      FROM crm_accounts a
      LEFT JOIN sales_users creator ON creator.id=a.created_by
      WHERE a.external_customer_id=?
      ORDER BY a.created_at DESC,a.id LIMIT 1`).get(cleanId);
    ...
    const includeProfileAI = options.includeAI !== false && !intakeReadOnly;
    const profileTags = [
      ...(tagMap[cleanId] || []),
      ...(includeProfileAI ? profileEvaluationTags(value, cleanId) : []),
    ];
    const pool = { ...buildPoolCustomer(poolRow), tags: profileTags };
    if (linkedAccount && (crmAccessible || adminMasterAccess)) {
      pool.customerSource = linkedAccount.source || '';
      pool.recordCreatedAt = linkedAccount.created_at || pool.createdAt;
      pool.recordUpdatedAt = linkedAccount.updated_at || pool.updatedAt;
      if (permissions.view_all_customers) {
        pool.creatorName = String(linkedAccount.created_by || '') === 'system'
          ? '系统导入'
          : (linkedAccount.creator_name || '历史数据/未知');
      }
    }
    const legacyCustomers = intakeReadOnly
      ? []
    ...
    const customers = permissions.view_contacts
      ? legacyCustomers
      : legacyCustomers.map(contactSafeCustomerRecord);
    const reconJobs = permissions.view_recon
    ...
    const payload = {
      ...
      profileAccess: recycleReadOnly
        ? {
          ...
          inCrm: true,
          crmAccessible: false,
          accountId: String(options.recycleAccountId || ''),
          canEditNickname: Boolean(options.canEditNickname),
        }
        : intakeReadOnly
          ? {
            readOnly: true,
          ...
          : {
            readOnly: false,
            source: linkedAccount ? 'crm' : 'master',
            inCrm: Boolean(linkedAccount),
            accountId: linkedAccount?.id || '',
            crmAccessible: Boolean(linkedAccount && crmAccessible),
            status: linkedAccount ? 'crm_accessible' : 'admin_master',
            adminMasterAccess: Boolean(adminMasterAccess && !linkedAccount),
            canEditNickname: Boolean(options.canEditNickname),
          },
      updatedAt: nowText(),
    };
...
function recordTagIdentityAudit(db, identity, action, customerId, diff) {
  if (!identity || !identity.realUserId) return;
  const table = db.prepare(
    "SELECT 1 FROM sqlite_master WHERE type='table' AND name='crm_audit_log'",
  ).get();
  if (!table) return;
  db.prepare(`INSERT INTO crm_audit_log
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)`).run(
    `AUD-TAG-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
    identity.effectiveUserId || identity.realUserId,
    action,
    'customer_master',
    customerId,
    JSON.stringify({ added: diff.added, removed: diff.removed }),
    nowText(),
    identity.realUserId,
    identity.effectiveUserId || identity.realUserId,
    identity.contextId || '',
...
function setCustomerTags(customerId, tagIds = [], options = {}) {
  ...
  try {
    ...
    if (diff.added.length || diff.removed.length) {
      recordTagIdentityAudit(db, options.identity, 'customer_tags_updated', clean, diff);
    }
...
function removeCustomerTag(customerId, tagId, options = {}) {
  ...
  try {
    ...
    if (diff.removed.length) {
      recordTagIdentityAudit(db, options.identity, 'customer_tag_removed', clean, diff);
    }
    const lifecycle = db.prepare('SELECT updated_at FROM customer_pool WHERE customer_id=?').get(clean);
    return {
      customerId: clean,
