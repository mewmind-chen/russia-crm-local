...
function normalizeSanctionStatus(row = {}) {
  ...
  const hasSanctionEvidence = Boolean(
    String(row.recon_evidence_url || '').trim()
    || String(row.recon_sanction_source || '').trim()
    || (Array.isArray(normalizedMatches) && normalizedMatches.length)
  );

  const normalizedResult = String(row.sanction_check_result || '').trim().toLowerCase();
  if (normalizedResult === 'confirmed_match' && normalizedCheckedAt && hasSanctionEvidence) return '受制裁';
  ...
  if (sanctionStatus === 'HIT' && reconCheckedAt && hasSanctionEvidence) return '受制裁';
