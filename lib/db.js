...
function getCustomerProfileData(accessContext, customerId) {
  const value = getDb();
  try {
    const cleanId = String(customerId || '').trim();
    assertExternalCustomerAccess(accessContext, cleanId);
    const permissions = accessContext?.permissions || {};
    const poolRow = value.prepare('SELECT * FROM customer_pool WHERE customer_id=?').get(cleanId);
    if (!poolRow) { const error = new Error('未找到对应客户资料'); error.statusCode = 404; throw error; }
    const tagMap = getCustomerTagMap(value);
    const profileTags = [...(tagMap[cleanId] || []), ...profileEvaluationTags(value, cleanId)];
    const pool = { ...buildPoolCustomer(poolRow), tags: profileTags };
    const customers = value.prepare('SELECT * FROM customers WHERE customer_id=? ORDER BY follow_id').all(cleanId)
    ...
    const payload = {
      ...
      prospectCandidates: [],
      prospectSources: [],
      tagCategories: [...TAG_CATEGORY_ORDER, 'AI评价标签'],
      statusOptions: STATUS_OPTIONS,
      statusGroups: Object.keys(STATUS_GROUPS),
      updatedAt: nowText(),
    };
...
module.exports = {
  ensureTables, getInitialData, getCustomerProfileData, updateCustomer, createTag, setCustomerTags, createReconJob,
