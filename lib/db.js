...
const { redactContactFields } = require('./access_control');
...
function getInitialData(accessContext) {
  ...
  try {
    ...
    const customers = permissions.view_customers
      ...
      : [];
    const customerPool = permissions.view_pool
      ? attachTags(db.prepare(`SELECT * FROM customer_pool WHERE ${scopeSql}
          ORDER BY CAST(SUBSTR(customer_id, 4, 4) AS INTEGER) DESC`).all(...allowedIds).map(buildPoolCustomer), tagMap)
      : [];
    ...
    const reconJobs = permissions.view_recon
      ...
      : [];
    const reconResults = permissions.view_recon
