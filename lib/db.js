...
function listProspectState(db) {
  const tasks = db.prepare('SELECT * FROM prospect_tasks ORDER BY updated_at DESC LIMIT 30').all().map(buildProspectTask);
  const candidates = db.prepare('SELECT * FROM prospect_candidates ORDER BY score DESC, updated_at DESC LIMIT 200').all().map(buildProspectCandidate);
  const sources = db.prepare('SELECT * FROM prospect_sources ORDER BY id DESC LIMIT 400').all().map(buildProspectSource);
...
function getInitialData() {
  ...
  const customers = attachTags(db.prepare('SELECT * FROM customers ORDER BY follow_id').all().map(buildCustomer), tagMap);
  const customerPool = attachTags(db.prepare(`
    SELECT * FROM customer_pool
    ORDER BY
      CAST(SUBSTR(customer_id, 4, 4) AS INTEGER) DESC
  `).all().map(buildPoolCustomer), tagMap);
  const tags = getTags(db);
  const templates = db.prepare('SELECT * FROM templates ORDER BY id').all();
  const reconJobs = db.prepare('SELECT * FROM recon_jobs ORDER BY updated_at DESC').all();
  const reconResults = db.prepare('SELECT * FROM recon_results ORDER BY updated_at DESC').all();
  const contactReconJobs = db.prepare('SELECT * FROM contact_recon_jobs ORDER BY updated_at DESC LIMIT 100').all();
  const people = db.prepare(`SELECT pc.*,
    ...
    FROM person_candidates pc ORDER BY sales_ready DESC, contact_level DESC, updated_at DESC LIMIT 500`).all();
  const contactQualityStats = getContactQualityStats(db);
  const prospect = listProspectState(db);
...
function setCustomerTags(customerId, tagIds = []) {
  const clean = String(customerId || '').trim();
  if (!clean) throw new Error('缺少客户ID');

  const ids = Array.from(new Set((Array.isArray(tagIds) ? tagIds : []).map(id => Number(id)).filter(Number.isInteger)));
  const db = getDb();
  ...
  `).get(clean, clean);
  if (!exists) { db.close(); throw new Error(`未找到客户：${clean}`); }

  const valid = ids.length
    ? db.prepare(`SELECT id FROM tags WHERE id IN (${ids.map(() => '?').join(',')})`).all(...ids).map(r => r.id)
    : [];
  if (valid.length !== ids.length) {
    db.close();
    throw new Error('包含不存在的标签');
  }

  const now = nowText();
  const insert = db.prepare('INSERT OR IGNORE INTO customer_tags (customer_id, tag_id, created_at) VALUES (?, ?, ?)');
  const save = db.transaction(() => {
    db.prepare('DELETE FROM customer_tags WHERE customer_id = ?').run(clean);
    valid.forEach(id => insert.run(clean, id, now));
  });
  save();

  const tags = getTagsForCustomer(db, clean);
  db.close();
  return { customerId: clean, tags, message: '客户标签已保存', updatedAt: nowText() };
}

function createProspectTask(payload = {}) {
  ...
  db.prepare(`
    INSERT INTO prospect_tasks (
      task_id, query, market, industry_focus, product_focus, status, source_mix,
      candidate_count, promoted_count, recon_count, error, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(...Object.values(task));
  const state = listProspectState(db);
  db.close();
  return { task: buildProspectTask(task), prospectState: state, message: '外贸智能体任务已创建' };
}

function getProspectTask(taskId) {
  const clean = String(taskId || '').trim();
  if (!clean) throw new Error('缺少任务ID');
  const db = getDb();
  const task = db.prepare('SELECT * FROM prospect_tasks WHERE task_id = ?').get(clean);
  db.close();
  if (!task) throw new Error(`未找到任务：${clean}`);
...
function localProspectSearch(payload = {}) {
  const query = cleanIncoming(payload.query);
  ...
  const poolRows = db.prepare(`
    SELECT * FROM customer_pool
    WHERE ${poolClauses.map(c => `(${c})`).join(' OR ')}
    LIMIT 60
  `).all(...poolParams);
  ...
  const reconRows = db.prepare(`
    ...
    WHERE ${reconClauses.map(c => `(${c})`).join(' OR ')}
    LIMIT 60
  `).all(...reconParams);
  db.close();
...
function saveProspectTaskResults(taskId, candidates = [], meta = {}) {
  const cleanTaskId = String(taskId || '').trim();
  if (!cleanTaskId) throw new Error('缺少任务ID');
  const db = getDb();
  const task = db.prepare('SELECT * FROM prospect_tasks WHERE task_id = ?').get(cleanTaskId);
...
function markProspectTaskRunning(taskId) {
  const clean = String(taskId || '').trim();
  if (!clean) throw new Error('缺少任务ID');
  const db = getDb();
  const now = nowText();
  db.prepare("UPDATE prospect_tasks SET status = 'running', error = '', updated_at = ? WHERE task_id = ?").run(now, clean);
  const state = listProspectState(db);
  db.close();
  return { prospectState: state };
}

function markProspectTaskFailed(taskId, error) {
  const clean = String(taskId || '').trim();
  if (!clean) throw new Error('缺少任务ID');
  const db = getDb();
  const now = nowText();
  db.prepare("UPDATE prospect_tasks SET status = 'failed', error = ?, updated_at = ? WHERE task_id = ?")
    .run(String(error || '').slice(0, 1000), now, clean);
  const state = listProspectState(db);
  db.close();
  return { prospectState: state };
}
...
function promoteProspectCandidate(candidateId, options = {}) {
  ...
  const db = getDb();
  const candidate = db.prepare('SELECT * FROM prospect_candidates WHERE candidate_id = ?').get(clean);
  if (!candidate) { db.close(); throw new Error(`未找到候选：${clean}`); }
...
function createReconJob(customerId, source) {
  const clean = String(customerId || '').trim();
  if (!clean) throw new Error('缺少客户ID');
  ...
  if (!customer) { db.close(); throw new Error(`未找到客户：${clean}`); }
...
function listQueuedJobs(payload = {}) {
  ...
  const jobs = db.prepare("SELECT * FROM recon_jobs WHERE status = 'queued' OR (status = 'running' AND updated_at < ?) ORDER BY updated_at ASC LIMIT ?")
...
function submitReconResult(payload = {}) {
  const jobId = String(payload.job_id || '').trim();
  if (!jobId) throw new Error('缺少 job_id');
  const db = getDb();
  const job = db.prepare('SELECT * FROM recon_jobs WHERE job_id = ?').get(jobId);
  ...
  if (!validEvidence.length) { db.close(); throw new Error('缺少证据记录'); }
  ...
  if (resultV3) {
    ...
    if (contractErrors.length) { db.close(); throw new Error(`Recon V3校验失败：${contractErrors.join('；')}`); }
  }
...
function submitContactReconResult(payload = {}) {
  const jobId = String(payload.job_id || '').trim();
  ...
  const commit = db.transaction(() => {
    ...
    ratedPeople.forEach((person, personIndex) => {
      ...
      addPerson.run(personId, job.customer_id, jobId, person.full_name || '', person.full_name_local || '', String(person.full_name || '').toLowerCase(), job.company_name,
      ...
      person.methods.forEach((method, methodIndex) => addMethod.run(`${personId}-${method.type}-${methodIndex + 1}`, personId, job.customer_id, method.type, method.value, method.normalized_value, method.verification_status,
    ...
    const addEvidence = db.prepare(`INSERT INTO person_evidence
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    evidence.forEach(item => addEvidence.run(item.evidence_id, item.person_id || '', job.customer_id, jobId, item.evidence_type || 'unknown', item.field_name || '', item.value || '', item.source_url,
    ...
    (value.company_entry_points || []).forEach(item => addEntry.run(jobId, job.customer_id, item.type || '', item.value || '', item.discovery_type || 'company_generic', item.verification_status || 'unverified', item.source_url || '', item.checked_at || now));
    ...
    db.prepare(`UPDATE customer_pool SET best_contact_level=?,best_person_id=?,sales_ready_contact_count=?,contact_recon_status='done',contact_last_checked_at=?,contact_next_action=? WHERE customer_id=?`)
      .run(bestLevel, best?.person_id || '', salesReady, now, nextAction, job.customer_id);
    db.prepare(`INSERT INTO contact_recon_audit (job_id,customer_id,person_count,l2_count,l3_count,validation_status,created_at) VALUES (?,?,?,?,?,'accepted',?)`)
      .run(jobId, job.customer_id, ratedPeople.length, l2, l3, now);
    return { bestLevel, l2, l3, personCount: ratedPeople.length };
  });
  try {
    const summary = commit();
    return { job: db.prepare('SELECT * FROM contact_recon_jobs WHERE job_id=?').get(jobId), people: ratedPeople, summary };
  } finally { db.close(); }
}
...
function getContactReconState(options = {}) {
  ...
  try {
    ...
    return {
      jobs: db.prepare('SELECT * FROM contact_recon_jobs ORDER BY updated_at DESC LIMIT ?').all(limit),
      people: db.prepare(`SELECT pc.*,
        (SELECT group_concat(cm.method_type || ':' || cm.value, ' / ') FROM contact_methods cm WHERE cm.person_id=pc.person_id) methods_summary
        FROM person_candidates pc ORDER BY sales_ready DESC, contact_level DESC, updated_at DESC LIMIT ?`).all(limit),
      stats: getContactQualityStats(db),
    };
  } finally { db.close(); }
}
...
module.exports = {
  ensureTables, getInitialData, updateCustomer, createTag, setCustomerTags, createReconJob,
