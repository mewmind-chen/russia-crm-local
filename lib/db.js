...
function setCustomerTags(customerId, tagIds = [], options = {}) {
  ...
  try {
    ...
    const protectedIds = db.prepare(`SELECT t.id
      FROM customer_tags ct JOIN tags t ON t.id=ct.tag_id
      WHERE ct.customer_id=? AND t.is_preset=1
        AND t.category IN ('客户类型','需确认属性')`).all(clean).map(row => row.id);
    const desiredIds = [...new Set([...ids, ...protectedIds])];
    const save = db.transaction(() => applyCustomerTagDiff(
      db, clean, desiredIds, String(options.actorId || ''), nowText(),
    ));
...
function removeCustomerTag(customerId, tagId, options = {}) {
  ...
  try {
    ...
    if (boundTag.is_preset && ['客户类型', '需确认属性'].includes(boundTag.category)) {
      throw new Error('结构化或风险标签不能手动移除');
    }
...
module.exports = {
  ensureTables, getInitialData, getCustomerProfileData, getCustomerTagHistory,
