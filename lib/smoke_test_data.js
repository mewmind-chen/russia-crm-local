    ...
    if (!current) {
      db.prepare(`INSERT INTO crm_accounts
        (id,external_customer_id,company_name,country,source,priority,stage,owner_id,created_by,
         last_activity_at,next_action,next_action_at,created_at,updated_at,is_test_data,test_run_id)
        VALUES (?,?,?,'','production_smoke','C','contacted',?,?, ?,?,?,?, ?,1,?)`).run(
        SMOKE_ACCOUNT_ID,
        SMOKE_EXTERNAL_CUSTOMER_ID,
        '[SMOKE TEST] Next Action 专用测试客户',
        actor.id,
      ...
      db.prepare(`UPDATE crm_accounts SET external_customer_id=?,company_name=?,stage='contacted',
        owner_id=?,last_activity_at=?,next_action=?,next_action_at=?,updated_at=?,
...
function cleanupNextActionSmoke(db, options = {}) {
  ...
  db.transaction(() => {
    db.prepare(`UPDATE crm_accounts SET next_action=?,next_action_at=?,last_activity_at=?,stage=?,
...
function cleanupLegacyA303Smoke(db, options = {}) {
  ...
  const at = isoNow(options);
  db.transaction(() => {
    const updated = db.prepare(`UPDATE crm_accounts SET next_action='',next_action_at='',updated_at=?
