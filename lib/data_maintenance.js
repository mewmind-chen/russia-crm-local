...
function resolveResetAssignmentTargets(db, filtersInput) {
  ...
  const counts = {
    ...
    notifications: countByCustomer('crm_notifications'),
  ...
  return { filters, items, accounts, accountIds, counts, skippedByStatus, conflicts, targetFingerprint };
}
...
function backupDirectory() {
  return path.resolve(process.env.CRM_BACKUP_DIR || path.join(__dirname, '..', 'backups', 'data-maintenance'));
}
  ...
  try {
    ...
    const tx = db.transaction(() => {
      ...
      if (target.accountIds.length) {
        db.prepare(`DELETE FROM crm_notifications WHERE customer_id IN (${placeholders(target.accountIds)})`).run(...target.accountIds);
