...
const { evaluateAssignmentRules } = require('./intake_assignment_engine');
const {
...
} = require('./intake_assignment_rules');
...
function anonymousSalesRoute(method, requestPath) {
  ...
  route = route.replace(/^\/accounts\/bulk-assign$/, '/accounts/bulk-assign')
    ...
    .replace(/^\/intake\/assignment-rules\/versions\/[^/]+\/restore$/, '/intake/assignment-rules/versions/:versionId/restore')
    .replace(/^\/intake\/assignment-rules\/(?!draft$|reorder$|publish$|simulate$|versions$)[^/]+$/, '/intake/assignment-rules/:ruleId')
...
function serializeArbitrationDecision(decision = {}) {
  return {
    ...
    selectedUserId: decision.selectedUserId || decision.userId || '',
    ruleId: decision.ruleId || decision.matchedRule?.id || '',
    ruleName: decision.ruleName || decision.matchedRule?.name || '',
    ruleVersionId: decision.ruleVersionId || decision.matchedRule?.versionId || '',
    strategy: decision.strategy || decision.matchedRule?.strategy || '',
    matchedRule: decision.matchedRule || null,
    candidateUserIds: Array.isArray(decision.candidateUserIds)
      ? decision.candidateUserIds.slice(0, 200) : [],
    eligibleUserIds: Array.isArray(decision.eligibleUserIds)
      ? decision.eligibleUserIds.slice(0, 200) : [],
    excludedCandidates: Array.isArray(decision.excludedCandidates)
      ? decision.excludedCandidates.slice(0, 200) : [],
  };
}
...
function installSalesCrm() {
  ...
  try {
    ...
    installIntakeAssignmentRules(value);
    seedUsers(value);
    installPermissionGroups(value);
...
function filterAstToCustomerQuery(ast, query = {}) {
  const result = {
    ...
    sort: String(query.sort || 'next_urgent'),
    tagFilters: [],
  };
...
function filterOptionsFor(value, user, definition) {
  ...
  const grouped = (expression, labelExpression = expression, joins = '') => value.prepare(`SELECT
      ...
      ORDER BY label COLLATE NOCASE`).all(...scope.params).map(row => ({
  ...
  if (definition.key === 'creator') {
    return grouped(
      ...
      "COALESCE(NULLIF(creator.name,''),'历史数据')",
      'LEFT JOIN sales_users creator ON creator.id=a.created_by',
    );
  }
  if (definition.key === 'stage') {
    return grouped('a.stage').map(option => ({
      ...option,
      label: STAGE_LABELS[option.value] || option.value,
    }));
  }
  ...
  if (definition.type === 'tag_multi') {
    return value.prepare(`SELECT CAST(t.id AS TEXT) value,t.name label,COUNT(DISTINCT a.id) count
      ...
      ${scope.sql} AND t.category=?
      GROUP BY t.id,t.name ORDER BY t.name COLLATE NOCASE`)
      .all(...scope.params, definition.tagCategory)
      .map(row => ({
        value: String(row.value),
        label: String(row.label || row.value),
        count: Number(row.count || 0),
...
function authorizedFilterSchema(value, user, page) {
  const effective = effectiveFilterSchemaFor(value, user, page);
  const researchOptions = ['contacts', 'recon'].includes(page)
    ? researchFilterOptions(value, user, page, effective.filters)
    : null;
  const intakeOptions = ['intake', 'lead_flow'].includes(page)
    ? intakeFlowFilterOptions(value, user, page, effective.filters)
    : null;
  const businessOptions = ['pipeline', 'alerts', 'insights', 'recycle_bin'].includes(page)
    ? businessFilterOptions(value, user, page, effective.filters)
    : null;
  const pageOptions = researchOptions || intakeOptions || businessOptions;
  const fields = effective.filters.map(definition => {
    ...
    const field = {
      ...
      options: pageOptions
        ...
        : filterOptionsFor(value, user, definition),
    };
...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  if (!['intake', 'lead_flow', 'pipeline', 'alerts', 'insights', 'recycle_bin'].includes(page)) {
  ...
  const value = db();
  try {
    const ast = authorizedFilterAst(value, user, page, query);
    let result;
    if (page === 'intake' || page === 'lead_flow') {
      const intake = queryIntakeFlowPage(value, user, page, ast, query);
      result = { ...intake, rows: intake.items };
      delete result.items;
    } else if (page === 'pipeline') {
      result = listPipelineRows(value, user, ast, query);
    } else if (page === 'alerts') {
      result = listTodayTasks(value, user, ast, query);
    } else if (page === 'insights') {
      result = listManagerEvaluationCustomers(value, user, ast, query);
    } else {
    ...
    return {
      ...result,
      schema: authorizedFilterSchema(value, user, page),
    };
  } finally { value.close(); }
}
...
function listCustomerAccounts(user, query = {}) {
  ...
  try {
    ...
    const pageSize = Math.max(20, Math.min(100, Number.parseInt(query.pageSize || query.page_size, 10) || 50));
    ...
    const sort = String(customerInput.sort || 'next_urgent');
    if (!['all', 'mine', 'unassigned', 'today', 'overdue', 'no_next', 'disqualified'].includes(quickView)
        || !['next_urgent', 'last_activity', 'newest', 'potential_desc', 'company'].includes(sort)) {
      throw badRequest('筛选条件格式无效');
    }
    ...
    const rows = addStageLabels(value.prepare(`SELECT DISTINCT a.*,
      ...
      ${from} ${where}
      ORDER BY ${customerQuery.orderBy} LIMIT ? OFFSET ?`).all(...params, pageSize, offset));
    ...
    const safeRows = hasPermission(user, 'view_contacts')
      ? rows
...
function assignmentRuleUsers(value) {
  return hydrateUsersPermissions(
    value,
    value.prepare("SELECT * FROM sales_users WHERE role='sales' ORDER BY name,id").all(),
  );
}

function assignmentRiskSignal(input) {
  const value = String(input || '').trim();
  if (!value) return '';
  const explicitlyClear = /\bclear\b|未发现.{0,8}制裁|制裁.{0,8}未命中|未命中/i.test(value);
  const explicitBlock = /blocked|high[\s_-]?risk|高风险|军工|军事|国防|导弹|weapon|military|defen[cs]e/i
    .test(value);
  return explicitlyClear && !explicitBlock ? '' : value;
}

function assignmentCandidateContext(value, candidate) {
  const externalCustomerId = String(
    candidate.external_customer_id || candidate.customer_id || '',
  );
  const tagIds = externalCustomerId && hasTable(value, 'customer_tags')
    ? value.prepare('SELECT tag_id FROM customer_tags WHERE customer_id=? ORDER BY tag_id')
      .all(externalCustomerId).map(row => row.tag_id)
    : [];
  const screening = externalCustomerId && hasTable(value, 'company_screening')
    ? value.prepare(`SELECT risk_level FROM company_screening
      WHERE customer_id=? ORDER BY updated_at DESC LIMIT 1`).get(externalCustomerId)
    : null;
  const poolHasRiskStatus = externalCustomerId && hasTable(value, 'customer_pool')
    && value.prepare('PRAGMA table_info(customer_pool)').all()
      .some(column => column.name === 'risk_status');
  const pool = poolHasRiskStatus
    ? value.prepare('SELECT risk_status FROM customer_pool WHERE customer_id=?')
  ...
  return {
    ...
    ].map(assignmentRiskSignal).filter(Boolean).join(' | '),
  };
}

function arbitrateCandidate(value, candidate, users, load, daily, quota, options = {}) {
  const store = options.assignmentRuleStore || createIntakeAssignmentRuleStore(value);
  const published = options.publishedRules || store.getPublishedRules();
  const usageDate = options.usageDate || nowText().slice(0, 10);
  const runtime = store.loadRuntimeState({
    versionId: published.publishedVersionId,
    usageDate,
  });
  const ruleUsers = options.ruleUsers || assignmentRuleUsers(value);
  const enrichedCandidate = assignmentCandidateContext(value, candidate);
  const externalCustomerId = String(
    enrichedCandidate.external_customer_id || enrichedCandidate.customer_id || '',
  );
  const duplicate = Boolean(
    enrichedCandidate.crm_customer_id
    || (externalCustomerId && value.prepare(
      'SELECT 1 FROM crm_accounts WHERE external_customer_id=? LIMIT 1',
    ).get(externalCustomerId)),
  );
  const ruleDecision = evaluateAssignmentRules({
    candidate: enrichedCandidate,
    rules: published.rules,
    versionId: published.publishedVersionId,
    versionNumber: published.publishedVersionNumber,
    users: ruleUsers,
    workloadByOwner: load,
    dailyByOwner: daily,
    dailyByRule: runtime.dailyByRule,
    roundRobinState: runtime.roundRobinState,
    defaultDailyQuota: quota,
    duplicate,
    riskBlocked: options.riskBlocked,
  });
  const expectedRoundRobinCursor = Number(
    runtime.roundRobinState[ruleDecision.ruleId]?.cursor || 0,
  );
  const noRecommendation = {
    available: false,
    reasonCode: options.aiEnabled === false ? 'ai_disabled' : 'not_evaluated',
    confidence: 0,
    reviewRequired: false,
    rankedCandidates: [],
  };
  if (!ruleDecision.assignable) {
    return {
      ...ruleDecision,
      recommendation: serializeRecommendation(noRecommendation),
      deterministicMatch: null,
      expectedRoundRobinCursor,
    };
  }
  const deterministicMatch = {
    userId: ruleDecision.selectedUserId,
    reason: ruleDecision.reason,
  };
  const loadedRecommendation = options.aiEnabled === false
    ? {
      available: false,
    ...
    : loadSalesMatchRecommendation(value, externalCustomerId, {
      now: options.now,
    });
  const eligibleIds = new Set(ruleDecision.eligibleUserIds);
  const recommendation = {
    ...loadedRecommendation,
    rankedCandidates: (loadedRecommendation.rankedCandidates || [])
      .filter(item => eligibleIds.has(item.userId)),
  };
  const arbitration = arbitrateIntakeOwner({
    candidate: enrichedCandidate,
    users: ruleUsers.filter(user => eligibleIds.has(user.id)),
    deterministicMatch,
    recommendation,
    riskBlocked: options.riskBlocked,
    duplicate,
  ...
  return {
    ...ruleDecision,
    ...arbitration,
    selectedUserId: arbitration.assignable ? arbitration.userId : '',
    matchedRule: ruleDecision.matchedRule,
    ruleId: ruleDecision.ruleId,
    ruleName: ruleDecision.ruleName,
    ruleVersionId: ruleDecision.ruleVersionId,
    strategy: ruleDecision.strategy,
    candidateUserIds: ruleDecision.candidateUserIds,
    eligibleUserIds: ruleDecision.eligibleUserIds,
    excludedCandidates: ruleDecision.excludedCandidates,
    nextRoundRobinCursor: ruleDecision.nextRoundRobinCursor,
    expectedRoundRobinCursor,
...
function reserveAssignmentRuleDecision(store, decision, usageDate) {
  if (!decision?.assignable || !decision.ruleId || !decision.ruleVersionId || !decision.userId) return;
  const rotation = decision.nextRoundRobinCursor === null
    || decision.nextRoundRobinCursor === undefined
    ? {}
    : {
      expectedRoundRobinCursor: Number(decision.expectedRoundRobinCursor || 0),
      nextRoundRobinCursor: Number(decision.nextRoundRobinCursor),
    };
  store.reserveRuntime({
    ruleId: decision.ruleId,
    versionId: decision.ruleVersionId,
    salesUserId: decision.userId,
    usageDate,
    dailyQuota: decision.matchedRule?.dailyQuota ?? null,
    ...rotation,
  });
}
...
function isRestrictedIntakeViewer(user) {
  return user?.role === 'sales' || !hasPermission(user, 'manage_intake');
}
...
function buildIntakeQueryScope(user, query = {}, options = {}) {
  ...
  const params = [];
  if (isRestrictedIntakeViewer(user)) {
...
function loadIntakeState(value, user, query = {}, options = {}) {
  ...
  const scoped = isRestrictedIntakeViewer(user);
  ...
  const ownerNames = hasTable(value, 'sales_users')
    ...
    : new Map();
  const scopedSales = isRestrictedIntakeViewer(user);
  const ruleRestrictedViewer = isRestrictedIntakeViewer(user);
  const managerViewer = user.role === 'manager' && hasPermission(user, 'manage_intake');
  for (const item of items) {
    ...
    if (managerViewer) {
      item.arbitration.ruleDecision = assignmentDecisionForActor(
        item.arbitration.ruleDecision,
        assignmentRuleActor(user),
      );
      if (item.arbitration.aiRecommendation) {
        item.arbitration.aiRecommendation.rankedCandidates = [];
      }
      history.forEach(entry => {
        entry.ruleDecision = assignmentDecisionForActor(
          entry.ruleDecision,
          assignmentRuleActor(user),
        );
        if (entry.aiRecommendation) entry.aiRecommendation.rankedCandidates = [];
      });
    }
    ...
    if (ruleRestrictedViewer) {
      delete item.arbitration;
      delete item.assignmentAudit;
...
function eligibleIntakeCandidates(value, settings, options = {}) {
  const publishedRulesActive = Boolean(
    options.publishedRules?.publishedVersionId
    && Array.isArray(options.publishedRules?.rules),
  );
  const groups = publishedRulesActive
    ? []
    : (settings.matchGroups.length ? settings.matchGroups : ['A', 'B']);
  const countries = publishedRulesActive ? [] : settings.countries.map(normalizeCountry);
  ...
  return rows.filter(row => {
    ...
    if (groups.length && !groups.includes(row.match_group || '')) return false;
...
function scanDailyIntake(actor = { id: 'system', role: 'admin' }, options = {}) {
  ...
  try {
    ...
    const assignmentRuleStore = createIntakeAssignmentRuleStore(value);
    const publishedRules = assignmentRuleStore.getPublishedRules();
    const ruleUsers = assignmentRuleUsers(value);
    if (!settings.enabled && !options.force) throw new Error('每日自动入库已停用');
    const candidates = eligibleIntakeCandidates(value, settings, { publishedRules });
      ...
      if (settings.approvalMode === 'automatic') {
        ...
        for (const item of backlog) {
          const decision = arbitrateCandidate(
            value, item, users, load, daily, Number(settings.dailyPerSales || 5), {
              aiEnabled,
              assignmentRuleStore,
              publishedRules,
              ruleUsers,
          ...
          if (!decision.assignable) {
            const status = decision.reasonCode === 'duplicate_customer' ? 'duplicate' : 'pending';
            value.prepare("UPDATE crm_intake_items SET status=?,suggested_owner_id=?,decision_reason=?,updated_at=? WHERE id=?")
              .run(status, decision.suggestedUserId || '', decision.reason, nowText(), item.id);
            continue;
          }
          ...
          if (result.assigned) {
            reserveAssignmentRuleDecision(assignmentRuleStore, decision, batchDate);
            assigned += 1;
    ...
    if (action === 'bulk_assign') {
      ...
      const transaction = value.transaction(() => {
        ...
        for (const candidate of items) {
          const decision = arbitrateCandidate(
            value, candidate, users, load, daily, settings.dailyPerSales, {
              aiEnabled,
              assignmentRuleStore,
              publishedRules,
              ruleUsers,
          ...
          if (!decision.assignable) {
            ...
            if (status === 'duplicate') {
              value.prepare("UPDATE crm_intake_items SET status='duplicate',updated_at=? WHERE id=?")
                .run(nowText(), candidate.id);
            }
            continue;
          }
          const match = { userId: decision.userId, reason: decision.reason };
...
function assignmentRuleActor(user, isImpersonating = false) {
  return { ...user, isImpersonating: Boolean(isImpersonating) };
}

function assignmentRuleOptions(value, actor) {
  const distinct = (table, column) => hasTable(value, table)
    ? value.prepare(`SELECT DISTINCT ${column} value FROM ${table}
      WHERE TRIM(COALESCE(${column},''))!='' ORDER BY ${column} LIMIT 300`)
      .all().map(row => row.value)
    : [];
  const tags = hasTable(value, 'tags')
    ? value.prepare('SELECT id,name,category FROM tags ORDER BY category,name,id LIMIT 500').all()
      .map(row => ({ id: row.id, name: `${row.category} · ${row.name}` }))
    : [];
  const sales = actor.role === 'admin'
    ? assignmentRuleUsers(value).map(user => ({
      id: user.id,
      name: user.name,
      active: Boolean(user.active) && !user.archived_at,
      authorized: Boolean(user.permissions?.view_intake),
    }))
    : [];
  return {
    countries: distinct('customer_pool', 'country'),
    industries: distinct('customer_pool', 'industry'),
    products: distinct('customer_pool', 'products'),
    customerTypes: distinct('customer_pool', 'customer_type'),
    tags,
    matchGroups: ['A', 'B', 'C', 'D'],
    sales,
  };
...
function assignmentRulesResponse(value, actor) {
  const store = createIntakeAssignmentRuleStore(value);
  const config = store.getConfig(actor);
  const published = store.getPublishedRules();
  const summary = actor.role === 'admin'
    ? summarizeRuleChanges(published.rules, store.getDraftRules())
    : { added: [], removed: [], changed: [], reordered: false };
  const hasDraftChanges = Boolean(
    summary.added.length || summary.removed.length || summary.changed.length || summary.reordered,
  );
  const userNames = new Map(value.prepare('SELECT id,name FROM sales_users').all()
    .map(row => [row.id, row.name]));
  const tagNames = hasTable(value, 'tags')
    ? new Map(value.prepare('SELECT id,name FROM tags').all().map(row => [row.id, row.name]))
    : new Map();
  const rules = config.rules.map(rule => ({
    ...rule,
    conditions: {
      ...(rule.conditions || {}),
      tagNames: (rule.conditions?.tagIds || []).map(tagId => tagNames.get(tagId) || tagId),
    },
    ...(Array.isArray(rule.salesUserIds) ? {
      salesUserNames: rule.salesUserIds.map(userId => userNames.get(userId) || userId),
    } : {}),
    updatedByName: userNames.get(rule.updatedBy) || rule.updatedBy || '系统',
  }));
  return {
    ...config,
    rules,
    publishedVersion: published.publishedVersionId ? {
      id: published.publishedVersionId,
      versionNumber: published.publishedVersionNumber,
    ...
    publishedAt: published.publishedAt || '',
    hasDraftChanges,
    changeSummary: actor.role === 'admin' ? summary : null,
    options: assignmentRuleOptions(value, actor),
  };
}

function simulateAssignmentRule(value, actor, payload, options = {}) {
  assertAssignmentRuleSimulation(actor);
  const itemId = String(payload?.itemId || '').trim();
  if (!itemId) {
    const error = new Error('请选择需要试算的线索');
  ...
  const settingsRow = value.prepare("SELECT * FROM crm_intake_settings WHERE id='default'").get();
  const usageDate = nowText().slice(0, 10);
  const store = createIntakeAssignmentRuleStore(value);
  const published = store.getPublishedRules();
  const users = assignmentRuleUsers(value);
  const load = activeWorkloadByOwner(value);
  const daily = Object.fromEntries(value.prepare(`SELECT assigned_owner_id,COUNT(*) n
    FROM crm_intake_items WHERE assigned_at>=? AND assigned_owner_id!=''
    GROUP BY assigned_owner_id`).all(`${usageDate} 00:00:00`)
    .map(row => [row.assigned_owner_id, Number(row.n)]));
  const aiEnabled = featureState(value, options.hardFlags || resolveAIHardFlags())
    .ai_stations.effectiveEnabled;
  const decision = arbitrateCandidate(
    value,
    ...
    load,
    daily,
    Number(settingsRow.daily_per_sales || 5),
    {
      aiEnabled,
      assignmentRuleStore: store,
  ...
  const detailed = {
    ...serializeArbitrationDecision(decision),
    selectedUserName: userNames.get(decision.selectedUserId || decision.userId) || '',
    eligibleCandidateNames: (decision.eligibleUserIds || [])
      .map(userId => userNames.get(userId) || userId),
    excludedCandidates: (decision.excludedCandidates || []).map(excluded => ({
      ...excluded,
      userName: userNames.get(excluded.userId) || excluded.userId,
    })),
  };
  const visible = assignmentDecisionForActor(detailed, actor);
  return {
    item: {
      id: item.id,
      companyName: item.company_name,
    ...
    decision: {
      ...visible,
      selectedUserName: userNames.get(visible.selectedUserId || visible.userId) || '',
    },
    simulation: true,
  };
}
...
function researchTotals(value, user, permissions) {
  ...
  return {
    pool: count(
      'customer_pool', 'p', canSeePool, 'COALESCE(p.is_test_data,0)=0', researchPoolCondition,
    ),
    poolAvailable: count(
      'customer_pool',
      'p',
      canSeePool,
      'COALESCE(p.is_test_data,0)=0 AND NOT EXISTS(SELECT 1 FROM crm_accounts linked_account WHERE linked_account.external_customer_id=p.customer_id)',
  ...
  try {
    if (kind === 'people' || kind === 'recon') {
      ...
      const orderBy = kind === 'people'
        ? 'pc.sales_ready DESC,pc.contact_level DESC,pc.updated_at DESC'
        : 'r.updated_at DESC';
    ...
    if (kind === 'pool') {
      ...
      addLike(permissions.view_contacts
        ? ['p.customer_id','p.company_name','p.country','p.city','p.website','p.industry','p.customer_type','p.products']
        : ['p.customer_id','p.company_name','p.country','p.city','p.website','p.industry','p.customer_type']);
...
function updateAccount(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    if (payload.nickname !== undefined) {
      ...
      if (nickname !== String(account.nickname || '')) {
        fields.push('nickname=?');
        params.push(nickname);
        nicknameChange = { oldNickname: account.nickname || '', newNickname: nickname };
      }
    }
    const allowed = {
    ...
    const masterAllowed = { country: 'country', city: 'city', website: 'website', industry: 'industry', productFocus: 'products' };
    const masterFields = [], masterParams = [];
    for (const [key, column] of Object.entries(masterAllowed)) {
      if (payload[key] === undefined) continue;
      masterFields.push(`${column}=?`);
      masterParams.push(payload[key]);
    }
    ...
    if (!fields.length && !masterFields.length) {
    ...
    const transaction = value.transaction(() => {
      ...
      if (nicknameChange) {
        recordRecycleAudit(value, user, identity, 'customer_nickname_updated', account.id, nicknameChange);
      }
...
function updateCustomerMaster(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    for (const [key, column] of Object.entries(allowed)) {
      ...
      if (next === String(existing[column] || '')) continue;
      fields.push(`${column}=?`);
      params.push(next);
      changed[key] = { from: String(existing[column] || ''), to: next };
    }
...
function listRecycleBin(user, query = {}, options = {}) {
  ...
  try {
    ...
    if (search) {
      const like = `%${search}%`;
      conditions.push('(a.id LIKE ? OR a.external_customer_id LIKE ? OR a.nickname LIKE ? OR a.company_name LIKE ? OR a.country LIKE ?)');
      params.push(like, like, like, like, like);
    }
    const where = conditions.join(' AND ');
    const total = Number(value.prepare(`SELECT COUNT(*) total FROM crm_accounts a WHERE ${where}`).get(...params).total || 0);
    const canReassign = !options.isImpersonating;
    const canRestore = user.role === 'admin'
      && hasPermission(user, 'manage_manual_customer_deletion')
      && !options.isImpersonating;
    const rows = value.prepare(`SELECT a.id,a.external_customer_id,a.nickname,a.company_name,a.country,a.stage,
...
function loadRecycleProfile(user, customerId, options = {}) {
  ...
  try {
    const scopeClause = fullScope
      ? ''
      : 'AND (a.previous_owner_id=? OR a.recycled_by=?)';
    const scopeParams = fullScope ? [] : [user.id, user.id];
    let account = value.prepare(`SELECT a.*,u.name owner_name,m.name manager_name,
      creator.name creator_name,
      COALESCE(NULLIF(p.company_name,''),a.company_name) company_name,
      COALESCE(NULLIF(p.country,''),a.country) country,
      COALESCE(NULLIF(p.city,''),a.city) city,
      ...
      WHERE a.id=? AND COALESCE(a.lifecycle_status,'active')='recycled'
        ${scopeClause} LIMIT 1`).get(cleanId, ...scopeParams);
    if (!account) {
      throw fullScope
        ? recycleError(404, '回收客户不存在', 'RECYCLED_CUSTOMER_NOT_FOUND')
        : recycleError(403, '无权访问该回收客户', 'RECYCLED_CUSTOMER_FORBIDDEN');
    }
    account = addStageLabels([account])[0];
    ...
    const activities = value.prepare(`SELECT x.*,u.name user_name
    ...
    const canRestore = account.recycle_kind === 'manual_delete'
      ...
      && hasPermission(user, 'manage_manual_customer_deletion')
      && !options.isImpersonating;
    const canReassign = account.recycle_kind === 'sales_return'
      && !options.isImpersonating;
    const payload = {
      ...masterProfile,
      ok: true,
...
function exportCrmCsv(user, query = {}, options = {}) {
  ...
  const headers = ['昵称', '正式名称', '客户编码', '国家', '行业', '阶段', '负责人', '优先级', '潜在金额', '最近动作', '下一步'];
  const rows = value.customers.map(row => [
    row.nickname, row.company_name, row.external_customer_id, row.country, row.industry, row.stageLabel,
...
function registerSalesCrm(app, options = {}) {
  ...
  app.get('/api/sales-crm/filter-schema/:pageKey', (req, res) => {
    ...
    try {
      ...
      res.json({
        ok: true,
        schema: authorizedFilterSchema(value, req.salesUser, String(req.params.pageKey || '')),
      });
    } catch (error) {
  ...
  app.get('/api/sales-crm/intake/assignment-rules', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      res.json({ ok: true, ...assignmentRulesResponse(value, actor) });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/intake/assignment-rules/draft', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const result = createIntakeAssignmentRuleStore(value).createDraft(req.body || {}, actor);
      res.json({ ok: true, ...result });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.patch('/api/sales-crm/intake/assignment-rules/:ruleId', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const result = createIntakeAssignmentRuleStore(value)
        .updateDraft(req.params.ruleId, req.body || {}, actor);
      res.json({ ok: true, ...result });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/intake/assignment-rules/reorder', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const body = req.body || {};
      const result = createIntakeAssignmentRuleStore(value).reorderDraft(
        body.ruleIds || body.orderedRuleIds || [],
  ...
  app.post('/api/sales-crm/intake/assignment-rules/publish', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const result = createIntakeAssignmentRuleStore(value).publish({
  ...
  app.get('/api/sales-crm/intake/assignment-rules/versions', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const versions = createIntakeAssignmentRuleStore(value).listVersions(actor);
      res.json({ ok: true, versions });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/intake/assignment-rules/versions/:versionId/restore', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      const result = createIntakeAssignmentRuleStore(value).restore(
  ...
  app.post('/api/sales-crm/intake/assignment-rules/simulate', (req, res) => {
    const value = db();
    try {
      const actor = assignmentRuleActor(req.salesUser, Boolean(req.impersonation));
      res.json({
        ok: true,
        ...simulateAssignmentRule(value, actor, req.body || {}, { hardFlags: hardFeatureFlags }),
      });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });
