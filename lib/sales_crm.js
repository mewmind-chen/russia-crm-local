...
function installIntakeCrmStatusSync(value) {
  value.exec(`
    CREATE TRIGGER IF NOT EXISTS crm_accounts_sync_intake_insert
    AFTER INSERT ON crm_accounts
    ...
    CREATE TRIGGER IF NOT EXISTS crm_accounts_sync_intake_external_update
    AFTER UPDATE OF external_customer_id ON crm_accounts
    WHEN TRIM(NEW.external_customer_id)!=''
    BEGIN
      UPDATE crm_intake_items
  ...
  value.prepare(`
    ...
    SET status='duplicate',
        crm_customer_id=(
          ...
          FROM crm_accounts linked_account
          WHERE linked_account.external_customer_id=crm_intake_items.external_customer_id
          LIMIT 1
        ),
        decision_reason='客户已在CRM',
        updated_at=?
    WHERE status IN ('pending','approved')
      AND EXISTS (
        SELECT 1 FROM crm_accounts linked_account
        WHERE linked_account.external_customer_id=crm_intake_items.external_customer_id
      )
  `).run(nowText());
}
