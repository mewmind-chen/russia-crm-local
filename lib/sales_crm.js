  ...
  try {
    ...
    for (let offset = 0; offset < externalIds.length; offset += 500) {
      ...
      const tagRows = value.prepare(`SELECT ct.customer_id,t.id,t.name,t.category,t.color,t.is_preset
        ...
        ORDER BY t.category,t.name,t.id`).all(...batch);
      for (const row of tagRows) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      if (hasCustomerType && account.external_customer_id) {
        ...
        if (!customerType || CUSTOMER_TYPE_OPTIONS.includes(customerType)) {
          customerTypeTag = syncCustomerTypeTag(value, {
            customerId: account.external_customer_id,
            customerType,
            actorId: identity.realUserId || user.id,
          });
        }
      }
      if (changedCustomerType) {
        ...
        value.prepare(`INSERT INTO crm_audit_log
          ...
          JSON.stringify({
            ...
            tagId: customerTypeTag?.tagId || null,
          }),
  ...
  app.get('/api/sales-crm/profile/:customerId/tag-history', (req, res) => {
    try {
      res.json({
        ...
        history: getCustomerTagHistory(req.accessContext, req.params.customerId, req.query || {}),
      });
    } catch (error) {
