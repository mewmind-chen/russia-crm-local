...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  try {
    ...
    if (page === 'intake' || page === 'lead_flow') {
      ...
      result = listPipelineRows(value, user, ast, listQuery);
    } else if (page === 'alerts') {
...
function applyCustomerReturn(value, user, identity, account, reason, action, batchSize = null) {
  ...
  const updated = value.prepare(`UPDATE crm_accounts SET lifecycle_status='recycled',recycle_kind='sales_return',
    ...
    AND COALESCE(lifecycle_status,'active')='active'
    AND COALESCE(assignment_status,'')!='returned'`).run(
...
function returnCustomer(user, customerId, payload = {}, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      applyCustomerReturn(value, user, identity, account, reason, 'customer_returned');
