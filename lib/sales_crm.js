    ...
    if (page === 'intake' || page === 'lead_flow') {
      ...
      intake.items.forEach(item => {
        ...
        if (item.identityWarning && !reusesReturnedAccount) {
          ...
          item.assignmentBlockReason = '待管理员确认客户身份';
          item.claimBlocked = true;
          if (user.role === 'sales' || !hasPermission(user, 'manage_intake')) {
            item.reviewVagueHint = '该客户需要管理员确认，确认后可继续领取。';
          }
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET stage=?,last_activity_at=?,next_action=?,next_action_at=?,
        ...
        .run(nextStage, occurredAt,
          managerRequired ? String(account.next_action || '') : (terminal ? '' : nextAction),
          managerRequired ? String(account.next_action_at || '') : (terminal ? '' : nextActionAt),
          managerRequired ? String(account.next_action_time_basis || '')
