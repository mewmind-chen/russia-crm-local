...
const TEAM_STATUS_READ_KEYS = new Set(['range', 'filters', 'permissionVersion']);
    ...
    const field = {
      ...
      options: pageOptions
        ? pageOptions[definition.key]
          || (teamStatusOptions ? filterOptionsFor(value, user, definition) : [])
...
function registerSalesCrm(app, options = {}) {
  ...
  app.get('/api/sales-crm/team-status', (req, res) => {
    ...
    try {
      ...
      const result = buildTeamStatus(value, req.salesUser, input, teamStatusOptions(value, req));
      res.setHeader('Cache-Control', 'private, no-store');
      res.json({
        ok: true,
        ...result,
