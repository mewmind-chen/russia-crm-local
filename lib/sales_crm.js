...
const { registerSalesCrmPageEntrypoints } = require('./page_entrypoints');
...
const {
  ...
  buildAccessContext,
  redactContactFields,
  redactContactDynamicFields,
  redactExportCredentials,
...
const {
  ...
  INTAKE_SORT_VALUES,
  INTAKE_SORT_FIELDS,
  CUSTOMER_POOL_LIST_FIELDS,
  redactCustomerPoolListFields,
...
const {
  ...
  INSIGHT_SORT_FIELDS,
  RECYCLE_SORT_FIELDS,
  MANAGER_TASK_SORT_FIELDS,
} = require('./business_page_filters');
const { parseSortDescriptors } = require('./list_sort');
const { applyAccountStatePatch, assertAccountStateContract, assertQuoteTransition, assertFirstOrderTransition } = require('./domains/lifecycle/state_write');
const { applyAccountPlanPatch, applyManagerStatusPatch, PLAN_TIME_BASIS } = require('./domains/lifecycle/collaboration_write');
const { projectNextAction } = require('./domains/lifecycle/state_projection');
const { json, parseJsonObject } = require('./domains/json/parse');
const { normalizeListQuery, listPage } = require('./domains/list/pagination');
const { redactAuditPayload } = require('./domains/audit/redact');
const {
...
const { csvCell, csvSerialize } = require('./domains/reporting/csv');
const { safeEvaluationLabel } = require('./domains/insights/labels');
const { publicActivityRecord, publicActivityRecords } = require('./domains/activity/serialize');
const { reasonOrder, urgencyFor, groupAlerts } = require('./domains/planning/alerts');
const { emptyCustomerPlanRisk } = require('./domains/planning/risk');
const { noPlanStreakForActivities } = require('./domains/planning/streak');
const {
  intakeActionIdempotencyKey,
  manualAssignmentRequestHash,
  manualAssignmentRequiresPreview,
} = require('./domains/intake/assignment');
const {
  serializeArbitrationDecision,
  withoutArbitrationAI,
  serializeRecommendation,
} = require('./domains/intake/decision');
const { intakeQueryValues, intakeQueryBoolean, intakeQueryDate } = require('./domains/intake/query');
const {
  isCurrentIntakeAccount,
  isReturnedAccountForIntake,
  reusableReturnedAccountForIntake,
} = require('./domains/assignment/link');
const { inaccessibleOrMissing } = require('./domains/auth/access');
const { hashPassword } = require('./domains/auth/credentials');
const { parseCookies } = require('./domains/auth/session');
const { safeUser } = require('./domains/auth/user');
const { chooseIntakeOwner } = require('./domains/intake/owner');
const {
  ACTIVITY_STAGE,
  PROGRESS_TYPE_MAP,
...
} = require('./domains/activity/progress');
const { resolveActivityReaction } = require('./domains/activity/request');
const {
  normalizeCountry,
  normalizeEstablishedYear,
  normalizeAccountNickname,
  normalizeCustomerStarReason,
} = require('./domains/customer/normalize');
const {
  normalizeEvaluation,
  withoutEvaluationAI,
  withoutEvaluationAIRow,
  aiFeatureDisabled,
} = require('./domains/insights/evaluation');
const { cleanContactFields, publicAccountContact } = require('./domains/customer/contacts');
const { identityConflictNote } = require('./domains/customer/identity');
const { creatorDisplayName, historyAccountSummary, changedFieldLabels } = require('./domains/customer/summary');
const { buildCountryReport, buildCohortReport, buildTeamReport, rate } = require('./domains/reporting/builders');
const { advanceStage } = require('./domains/commerce/rules');
const {
  insertRfqRow,
  commitQuote,
  commitOrder,
} = require('./domains/commerce/write');
const {
  ...
  manualReturnBatchId,
} = require('./domains/customer/recycle');
const {
  duplicateFingerprint,
  hydrateDuplicateCandidate,
  reviewCandidateRows,
  reviewHasProtectedExact,
} = require('./domains/customer/dedupe');
const {
  ...
  escapeActivitySearchLike,
} = require('./domains/activity/present');
const { customerCreateRequestHash } = require('./domains/customer/create');
const { filterVersionError } = require('./domains/filter/errors');
const { todayTaskError, normalizeTodayTaskDate } = require('./domains/planning/today_task');
const {
...
const { registerTeamStatusRoutes } = require('./sales_crm_team_status_routes');
const { registerSalesCrmContactRoutes } = require('./sales_crm_contacts_routes');
const { registerSalesAuthRoutes } = require('./sales_auth_routes');
const {
  registerSalesCrmAccountReadRoutes,
  registerSalesCrmAccountRecycleRoutes,
} = require('./sales_crm_account_routes');
const {
  registerSalesCrmListRoutes,
  registerSalesCrmIntakeResearchRoutes,
} = require('./sales_crm_read_routes');
const { registerSalesCrmProtectedRoutes } = require('./sales_crm_protected_routes');
const { registerSalesCrmManagerRoutes } = require('./sales_crm_manager_routes');
const { registerSalesCrmActivityRoutes } = require('./sales_crm_activity_routes');
const { registerSalesCrmBootstrapRoutes } = require('./sales_crm_bootstrap_routes');
const { registerSalesCrmBusinessWriteRoutes } = require('./sales_crm_business_write_routes');
const { registerSalesCrmAdminRoutes } = require('./sales_crm_admin_routes');
const { createManagerWorkflowServices } = require('./manager_workflows');
const {
...
function ensureActivityProgressSchema(value) {
  ...
  INITIAL_ACTIVITY_REACTIONS.forEach(([reactionId, name, actionQueueKey], index) => {
    insert.run(
      reactionId,
      name,
      activityReactionNameKey(name, { badRequest }),
      index,
      actionQueueKey,
      installedAt,
      installedAt,
  ...
  if (actionQueueColumnAdded) {
    ...
    INITIAL_ACTIVITY_REACTIONS.forEach(([reactionId, name, actionQueueKey]) => {
      migrateDefault.run(actionQueueKey, installedAt, reactionId, activityReactionNameKey(name, { badRequest }));
    });
  }
}
...
function authorizedTeamStatusAst(value, user, page, input = {}) {
  ...
  if (input.permissionVersion !== undefined
    ...
    throw filterVersionError({ httpError });
  }
...
function customerLinkageClause(value, user, ast, fieldKey) {
  ...
  const customerQuery = buildCustomerQuery(customerInput, {
    ...
    canViewRecon: hasPermission(user, 'view_recon'),
  });
...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  if (page === 'alerts') {
    allowedQueryKeys.add('urgency');
    allowedQueryKeys.add('sort');
  }
  if (page === 'notifications') allowedQueryKeys.add('sort');
  if (page === 'insights') allowedQueryKeys.add('sort');
  if (['intake', 'lead_flow'].includes(page)) allowedQueryKeys.add('sort');
  if (page === 'pipeline') {
    allowedQueryKeys.add('actionQueue');
    allowedQueryKeys.add('starView');
  }
  if (['manager_tasks', 'manager_risks', 'manager_metrics'].includes(page)) {
    allowedQueryKeys.add('sort');
  }
  if (['intake', 'lead_flow'].includes(page)) {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, INTAKE_SORT_FIELDS);
    else if (requestedSort && !INTAKE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'alerts') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, ALERT_SORT_FIELDS);
    else if (requestedSort && !ALERT_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'notifications') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, NOTIFICATION_SORT_FIELDS);
    else if (requestedSort && !NOTIFICATION_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'insights') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, INSIGHT_SORT_FIELDS);
    else if (requestedSort && !INSIGHT_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'recycle_bin') allowedQueryKeys.add('sort');
  if (page === 'pipeline') allowedQueryKeys.add('sort');
  if (Object.keys(query).some(key => !allowedQueryKeys.has(key))) {
    throw httpError(403, '筛选条件未获授权', 'FILTER_NOT_AUTHORIZED');
  }
  if (page === 'recycle_bin') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, RECYCLE_SORT_FIELDS);
    else if (requestedSort && !RECYCLE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'pipeline') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort.startsWith('[')) parseSortDescriptors(requestedSort, PIPELINE_SORT_FIELDS);
    else if (requestedSort && !PIPELINE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  const listQuery = {
...
// Profile routes append route-owned shapes (account contacts, insight access,
// and evaluations) after getCustomerProfileData has applied its shared
// aggregate boundary. Re-run the contact boundary after those additions so a
// caller with view_insights but without view_contacts cannot reintroduce
// narrative/contact descendants through the route wrapper.
function redactProfileResponse(value, user, payload) {
  const tagged = redactUnauthorizedProfileTags(value, user, payload);
  return hasPermission(user, 'view_contacts') ? tagged : redactContactDynamicFields(tagged);
}
...
function customerPoolListProjection(alias = 'p') {
  return CUSTOMER_POOL_LIST_FIELDS
    .map(([, key]) => `${alias}.\"${key.replace(/^pool_/, '')}\" AS \"${key}\"`)
    .join(',');
}

function customerPoolListProjectionDynamic(alias = 'p', value = null) {
  const available = value
    ? new Set(value.prepare('PRAGMA table_info(customer_pool)').all()
      .map(column => String(column.name || '')))
    : null;
  return CUSTOMER_POOL_LIST_FIELDS
    .map(([source, key]) => available && !available.has(source)
      ? 'NULL AS "' + key + '"'
      : alias + '."' + source + '" AS "' + key + '"')
    .join(',');
}
...
function listCustomerAccounts(user, query = {}) {
  ...
  try {
    ...
    let rows = addStageLabels(value.prepare(`SELECT DISTINCT ${crmAccountProjection(value)},
      ${customerPoolListProjectionDynamic('p', value)},
    ...
    rows = redactCustomerPoolListFields(rows, user);
...
function setCustomerStar(user, customerId, payload = {}, identity = {}) {
  ...
  const reason = payload.active ? normalizeCustomerStarReason(payload.reason, { badRequest }) : '';
  const value = db();
  try {
...
function buildAlerts(accounts, activities, rfqs, quotes, planEvents = [], managerTasks = []) {
  ...
  for (const account of accounts) {
    ...
    const nextActionState = projectNextAction(account);
    const nextAt = nextActionState.overdue ? Date.parse(`${String(account.next_action_at).replace(' ', 'T')}Z`) : 0;
    ...
    if (nextActionState.overdue) add(account, 'critical', 'OVERDUE', '跟进任务已超期', `${account.next_action} 已超过计划时间`, '今天完成跟进', (now - nextAt) / 3600000);
...
function assertTodayTaskManager(user, permissions) {
  if (!['admin', 'manager'].includes(String(user?.role || ''))) {
    throw todayTaskError(403, '只有管理员或经理可以执行此操作', 'TODAY_TASK_FORBIDDEN', { error: httpError });
  }
  for (const permission of permissions) {
    if (!hasPermission(user, permission)) {
      throw todayTaskError(403, `没有权限：${PERMISSION_DEFINITIONS[permission] || permission}`, 'TODAY_TASK_FORBIDDEN', { error: httpError });
    }
  }
}

// Deferred-plan and manager-intervention application workflows are injected
// from `manager_workflows.js`; the composition root only supplies dependencies.
function todayTaskRequestSpec(payload = {}) {
  ...
  if (!TODAY_TASK_ACTION_TYPES.has(actionType)) {
    throw todayTaskError(400, '不支持的今日待办操作', 'TODAY_TASK_INVALID', { error: httpError });
  }
  ...
  if (!targetId) throw todayTaskError(400, '今日待办目标不能为空', 'TODAY_TASK_INVALID', { error: httpError });
  const idempotencyKey = String(payload.idempotencyKey || '').trim();
  if (!idempotencyKey || idempotencyKey.length > 240) {
    throw todayTaskError(400, '必须提供有效的幂等键', 'TODAY_TASK_INVALID', { error: httpError });
  }
...
function completeTodayTaskAction(value, spec, response) {
  ...
  if (updated.changes !== 1) {
    throw todayTaskError(409, '今日待办操作状态已变化', 'TODAY_TASK_ACTION_IN_PROGRESS', { error: httpError });
  }
  return response;
}
...
function resolveOverdueLeadTodayTask(value, user, payload, spec, identity) {
  ...
  if (!['reassign', 'return_to_pool'].includes(resolution)) {
    throw todayTaskError(400, '请选择重新分配或退回线索池', 'TODAY_TASK_INVALID', { error: httpError });
  }
  ...
  if (!item
    ...
    throw todayTaskError(409, '该超时线索已处理或状态已变化', 'TODAY_TASK_STALE', { error: httpError });
  }
  ...
  if (resolution === 'reassign') {
    ...
    const owner = authorizedSalesUser(value, ownerId);
    if (!owner) {
      throw todayTaskError(400, '请选择有效的在职销售负责人', 'TODAY_TASK_INVALID', { error: httpError });
    }
    if (ownerId === previousOwnerId) {
      throw todayTaskError(400, '请重新分配给其他在职销售', 'TODAY_TASK_INVALID', { error: httpError });
    }
...
function addNextPlanTodayTask(value, user, payload, spec, identity) {
  ...
  if (String(account.next_action || '').trim() && String(account.next_action_at || '').trim()) {
    throw todayTaskError(409, '该客户已补充下一步计划', 'TODAY_TASK_STALE', { error: httpError });
  }
  const nextAction = String(payload.nextAction || '').trim();
  if (!nextAction) throw todayTaskError(400, '下一步动作不能为空', 'TODAY_TASK_INVALID', { error: httpError });
  if (Array.from(nextAction).length > 1000) {
    throw todayTaskError(400, '下一步动作最多1000个字符', 'TODAY_TASK_INVALID', { error: httpError });
  }
  const nextActionAt = normalizeTodayTaskDate(payload.nextActionAt, { parseBusinessDateTime });
...
function completeManagerAssistanceTodayTask(value, user, payload, spec, identity) {
  ...
  if (!account.manager_required || account.manager_status !== '待介入') {
    throw todayTaskError(409, '该管理协助待办已完成或状态已变化', 'TODAY_TASK_STALE', { error: httpError });
  }
  const result = String(payload.result || '').trim();
  if (!result) throw todayTaskError(400, '主管处理意见不能为空', 'TODAY_TASK_INVALID', { error: httpError });
  if (Array.from(result).length > 2000) {
    throw todayTaskError(400, '主管处理意见最多2000个字符', 'TODAY_TASK_INVALID', { error: httpError });
  }
...
function confirmManagerAssistanceTodayTask(value, user, payload, spec, identity) {
  ...
  const account = getAccountForUser(value, user, spec.targetId);
  if (!account.manager_required || account.manager_status !== '已回复') {
    throw todayTaskError(409, '该主管协助回执已处理或状态已变化', 'TODAY_TASK_STALE', { error: httpError });
  }
  const nextAction = String(payload.nextAction || '').trim();
  if (!nextAction) throw todayTaskError(400, '下一步动作不能为空', 'TODAY_TASK_INVALID', { error: httpError });
  if (Array.from(nextAction).length > 1000) {
...
function loadResearchPage(user, kind, query = {}) {
  ...
  try {
    if (kind === 'people' || kind === 'recon') {
      const allowedQueryKeys = new Set([
        'page', 'pageSize', 'page_size', 'permissionVersion', 'filters', 'sort',
      ]);
      if (Object.keys(query).some(key => !allowedQueryKeys.has(key))) {
        throw httpError(403, '筛选条件未获授权', 'FILTER_NOT_AUTHORIZED');
      }
      ...
      const select = kind === 'people'
        ...
        : 'r.*';
      const requestedSort = String(query.sort || '').trim();
      const peopleSortFields = {
        company_name: 'company_name', full_name: 'pc.full_name', title: 'pc.title',
        contact_level: 'pc.contact_level', methods_summary: 'methods_summary', sales_ready: 'pc.sales_ready',
      };
      const reconSortFields = {
        company_name: 'r.company_name', score: 'r.score', customer_type: 'r.customer_type',
        opportunity_summary: 'r.opportunity_summary', contacts_summary: 'r.contacts_summary',
        updated_at: 'r.updated_at',
      };
      const customSort = requestedSort.startsWith('[')
        ? parseSortDescriptors(requestedSort, kind === 'people' ? peopleSortFields : reconSortFields)
        : [];
      const orderBy = kind === 'people'
        ? (customSort.length
          ? `${customSort.map(item => `${item.expression} ${item.direction.toUpperCase()}`).join(',')},pc.person_id ASC`
          : {
          sales_ready: 'pc.sales_ready DESC,pc.contact_level DESC,pc.updated_at DESC,pc.person_id ASC',
          contact_level: 'pc.contact_level DESC,pc.sales_ready DESC,pc.updated_at DESC,pc.person_id ASC',
          updated_desc: 'pc.updated_at DESC,pc.person_id ASC',
          company_asc: 'company_name COLLATE NOCASE ASC,pc.person_id ASC',
        }[requestedSort || 'sales_ready'])
        : (customSort.length
          ? `${customSort.map(item => `${item.expression} ${item.direction.toUpperCase()}`).join(',')},r.job_id ASC`
          : {
          updated_desc: 'r.updated_at DESC,r.job_id ASC',
          score_desc: 'r.score DESC,r.updated_at DESC,r.job_id ASC',
          company_asc: 'r.company_name COLLATE NOCASE ASC,r.job_id ASC',
        }[requestedSort || 'updated_desc']);
      if (!orderBy) throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
...
function loadPayload(user, options = {}) {
  ...
  try {
    ...
    const contactSafeIntake = payload => permissions.view_contacts ? payload : redactIntakeAggregate(payload);
    const contactSafeAlerts = payload => permissions.view_contacts
      ? payload
      : redactContactDynamicFields(payload, { preserveAlertCopy: true });
    ...
    const intake = permissions.view_intake
      ? contactSafeIntake(loadIntakeState(value, user, {}, { includeAI: aiEnabled }))
    ...
    return {
      ...
      insights: permissions.view_insights ? {
        ...
        evaluations: permissions.view_contacts
          ? visibleEvaluations
          : redactContactDynamicFields(visibleEvaluations.filter(row => row.subjectType === 'company')),
...
const managerWorkflow = createManagerWorkflowServices({
  db,
  assertPermission,
  badRequest,
  conflictError,
...
function updateCustomerNickname(user, externalCustomerId, payload, identity = {}) {
  ...
  const nickname = normalizeAccountNickname(payload?.nickname, { badRequest });
  const value = db();
  try {
...
function createActivityReaction(actor, payload, identity = {}) {
  ...
  const name = normalizeActivityReactionName(payload?.name, { badRequest });
  const nameKey = activityReactionNameKey(name, { badRequest });
  const actionQueueKey = normalizeActivityActionQueueKey(payload?.actionQueueKey, { badRequest });
  const value = db();
  try {
    return value.transaction(() => {
  ...
  try {
    return value.transaction(() => {
      ...
      const actionQueueKey = payload?.actionQueueKey === undefined
        ...
        : normalizeActivityActionQueueKey(payload.actionQueueKey, { badRequest });
...
function addActivity(user, payload, options = {}) {
  ...
  try {
    ...
    const spec = resolveActivityRequestSpec(payload, { badRequest });
    ...
    const transaction = value.transaction(() => {
      ...
      if (reservation?.replay) return reservation.replay;
      const reaction = resolveActivityReaction(payload, {
        badRequest,
        conflictError,
        findReactionById: reactionOptionId => value.prepare(`SELECT * FROM crm_activity_reaction_options
          WHERE id=? AND active=1`).get(reactionOptionId),
        findReactionByKey: nameKey => value.prepare(`SELECT * FROM crm_activity_reaction_options
      ...
      if (spec.activityType === 'rfq') {
        insertRfqRow(value, {
          ...
          reference: String(payload.reference || ''),
          status: 'open',
          bomLines: Number(payload.bomLines || 0),
          expectedValue: Number(payload.expectedValue || 0),
          productCategory: String(payload.productCategory || ''),
          completeness: Number(payload.completeness || 0),
...
function addQuote(user, payload, options = {}) {
  const value = db();
  try {
    return commitQuote(value, user, payload, {
      assertPermission,
      badRequest,
      conflictError,
      json,
...
function addOrder(user, payload) {
  const value = db();
  try {
    return commitOrder(value, user, payload, {
      assertPermission,
      badRequest,
      conflictError,
      json,
...
function addAccount(user, payload, options = {}) {
  ...
  try {
    ...
    const establishedYear = normalizeEstablishedYear(customerInput.establishedYear, { badRequest });
...
function updateAccount(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    const establishedYear = payload.establishedYear === undefined
      ? undefined
      : normalizeEstablishedYear(payload.establishedYear, { badRequest });
    let unassignReason = '';
    const statePatchInput = {};
    const planPatchInput = {};
    const managerPatchInput = {};
    for (const [key, column] of Object.entries(allowed)) {
      if (payload[key] === undefined) continue;
      fields.push(`${column}=?`);
      params.push(key === 'establishedYear' ? establishedYear : payload[key]);
    }
    if (payload.stage !== undefined) {
      statePatchInput.stage = payload.stage;
      if (stopsFollowUp) {
        planPatchInput.nextAction = '';
        planPatchInput.nextActionAt = '';
        planPatchInput.timeBasis = '';
      }
    }
    if (!stopsFollowUp && touchesPlan) {
      if (payload.nextAction !== undefined) planPatchInput.nextAction = nextAction;
      if (payload.nextActionAt !== undefined) planPatchInput.nextActionAt = nextActionAt;
      else if (nextActionAt !== String(account.next_action_at || '').trim()) {
        planPatchInput.nextActionAt = nextActionAt;
      }
      planPatchInput.timeBasis = nextAction && nextActionAt ? 'utc' : '';
    }
    if (payload.managerRequired !== undefined) {
      managerPatchInput.required = payload.managerRequired ? 1 : 0;
    }
    if (payload.managerStatus !== undefined) {
      managerPatchInput.status = payload.managerStatus;
    }
    if (payload.ownerId !== undefined) {
      ...
      if (ownerId !== String(account.owner_id || '')) {
        ...
        statePatchInput.ownerId = ownerId || null;
        statePatchInput.assignmentStatus = ownerId ? 'claimed' : 'unassigned';
        fields.push('assigned_at=?');
        params.push(ownerId ? nowText() : '');
    ...
    if (!fields.length && !masterFields.length && !nicknameChange
        && !Object.keys(statePatchInput).length && !Object.keys(planPatchInput).length
        && !Object.keys(managerPatchInput).length) {
    ...
    const transaction = value.transaction(() => {
      ...
      if (Object.keys(statePatchInput).length) {
        applyAccountStatePatch(value, account.id, { ...statePatchInput, updatedAt: nowText() });
      }
      if (Object.keys(planPatchInput).length) {
        applyAccountPlanPatch(value, account.id, { ...planPatchInput, updatedAt: nowText() });
      }
      if (Object.keys(managerPatchInput).length) {
        applyManagerStatusPatch(value, account.id, { ...managerPatchInput, updatedAt: nowText() });
      }
      if (fields.length) {
...
function updateCustomerMaster(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    for (const [key, column] of Object.entries(allowed)) {
      ...
      const normalized = key === 'establishedYear' ? normalizeEstablishedYear(next, { badRequest }) : next;
...
function applyCustomerReturn(value, user, identity, account, reason, action, batchSize = null) {
  assertCustomerReturnEligible(account, { httpError: recycleError });
  const updatedAt = nowText();
...
function returnCustomer(user, customerId, payload = {}, identity = {}) {
  const reason = validateRecycleReason(payload.reason, { httpError: recycleError });
  assertPermission(user, 'manage_customer_recycle');
  const value = db();
  try {
    return value.transaction(() => {
      const account = assertCustomerReturnEligible(accountForReturn(value, user, customerId), { httpError: recycleError });
  ...
  try {
    return value.transaction(() => {
      ...
      const accounts = customerIds.map(customerId =>
        assertCustomerReturnEligible(accountForReturn(value, user, customerId), { httpError: recycleError }));
      for (const account of accounts) {
        applyCustomerReturn(
...
function buildRecycleAccountProfile(value, user, sourceAccount, options = {}) {
    ...
    return hasPermission(user, 'view_contacts') ? payload : redactContactDynamicFields(payload);
}
...
function loadMismatchRecordProfile(user, recordKey, options = {}) {
  const parsed = parseMismatchRecordKey(recordKey, { httpError: recycleError });
  const value = db();
  try {
    if (parsed.sourceType === 'intake') {
      ...
      if (!item) {
        ...
        throw exists
          ...
          : mismatchRecordNotFound({ httpError: recycleError });
      }
  ...
  try {
    return value.transaction(() => {
      ...
      assertAccountStateContract({
        ...
        assignment_status: ownerId ? 'claimed' : 'unassigned',
        owner_id: ownerId,
      });
      applyAccountStatePatch(value, account.id, {
        lifecycleStatus: 'active',
...
function exportCrmData(user, query = {}, options = {}) {
  ...
  try {
    ...
    const contactSafePayload = contactsAllowed ? payload : redactContactDynamicFields(payload);
    return redactExportCredentials(contactSafePayload);
  } finally { value.close(); }
}
...
function exportCrmCsv(user, query = {}, options = {}) {
  ...
  if (String(query.dataset || '').toLowerCase() === 'activities') {
    ...
    return csvSerialize(headers, rows);
  }
...
function registerSalesCrm(app, options = {}) {
  ...
  registerSalesCrmPageEntrypoints(app, { rootDir: path.join(__dirname, '..') });
  ...
  app.get('/api/sales-crm/profile/:customerId', (req, res) => {
    ...
    try {
      ...
      res.json(redactProfileResponse(value, req.salesUser, payload));
    }
  ...
  // Unified non-AI profile mutations. These replace the legacy /api/app actions
  // while keeping the existing database writers and permission boundary.
  app.patch('/api/sales-crm/profile/:customerId/follow', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'edit_customer');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      const requestedFollowId = String(req.body?.followId || '').trim();
      const row = requestedFollowId
        ? value.prepare('SELECT follow_id FROM customers WHERE customer_id=? AND follow_id=? LIMIT 1').get(customerId, requestedFollowId)
        : value.prepare('SELECT follow_id FROM customers WHERE customer_id=? ORDER BY follow_id LIMIT 1').get(customerId);
      if (!row?.follow_id) throw new Error('该客户没有可编辑的跟进记录');
      const payload = { ...(req.body || {}) };
      delete payload.followId;
      res.json(updateCustomer(row.follow_id, payload));
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/tags', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'edit_customer');
      const category = String(req.body?.category || '').trim();
      if (!category || !allowedCustomerTagCategories(value, req.salesUser).has(category)) {
        throw forbidden('该标签分类未获授权');
      }
      res.json({ ok: true, ...createTag(req.body || {}) });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.put('/api/sales-crm/profile/:customerId/tags', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'edit_customer');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      const tagIds = Array.isArray(req.body?.tagIds) ? req.body.tagIds : req.body?.tags;
      res.json({ ok: true, ...setCustomerTags(customerId, tagIds || [], { actorId: req.salesUser.id, identity: auditIdentity(req) }) });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.delete('/api/sales-crm/profile/:customerId/tags/:tagId', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'edit_customer');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      res.json({ ok: true, ...removeCustomerTag(customerId, req.params.tagId, { actorId: req.salesUser.id, identity: auditIdentity(req) }) });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.get('/api/sales-crm/profile/:customerId/recon/:jobId', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'view_recon');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      const result = value.prepare('SELECT * FROM recon_results WHERE job_id=? AND customer_id=? LIMIT 1')
        .get(req.params.jobId, customerId);
      if (!result) throw inaccessibleOrMissing(req.salesUser, 'Recon 结果不存在');
      const evidence = hasPermission(req.salesUser, 'view_contacts')
        ? value.prepare('SELECT * FROM recon_evidence WHERE job_id=? ORDER BY id').all(req.params.jobId)
        : [];
      let resultV3 = null;
      try { resultV3 = result.result_json ? JSON.parse(result.result_json) : null; } catch (_error) {}
      res.json({ ok: true, result: hasPermission(req.salesUser, 'view_contacts') ? result : contactSafeReconRecord(result), resultV3: hasPermission(req.salesUser, 'view_contacts') ? resultV3 : null, evidence });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/profile/:customerId/recon', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'run_recon');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      res.json(createReconJob(customerId, req.body?.source || 'followup', { requestedBy: req.salesUser.id, db: value }));
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });

  app.post('/api/sales-crm/profile/:customerId/recon/:jobId/retry', (req, res) => {
    const value = db();
    try {
      assertPermission(req.salesUser, 'run_recon');
      const customerId = String(req.params.customerId || '').trim();
      if (!canAccessCustomerMaster(value, req.salesUser, customerId)) throw inaccessibleOrMissing(req.salesUser, '客户不存在');
      const job = value.prepare('SELECT job_id,customer_id FROM recon_jobs WHERE job_id=? AND customer_id=?').get(req.params.jobId, customerId);
      if (!job) throw inaccessibleOrMissing(req.salesUser, 'Recon 任务不存在');
      res.json(retryReconJob(req.params.jobId));
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });
  ...
  registerSalesCrmBusinessWriteRoutes(app, {
    ...
    addActivity,
    deferAccountPlan,
  }, { commerce: false, impersonation: false });
  ...
  registerSalesCrmBusinessWriteRoutes(app, {
    ...
    addQuote,
    addOrder,
  }, { exportActivity: false, impersonation: false });
  ...
  registerSalesCrmBusinessWriteRoutes(app, {
    ...
    hydrateUserPermissions,
    safeUser,
  }, { exportActivity: false, commerce: false });
  ...
  registerSalesCrmAdminRoutes(app, {
    db, sendApiError, maintenanceCapabilities, listMaintenanceRuns, previewDataMaintenance, executeDataMaintenance,
    createUser, resetUserPassword, updateUser, archiveUser, restoreUser, deleteArchivedUser,
    listPermissionGroups, createPermissionGroup, updatePermissionGroup, restoreUserPermissions, replaceUserPermissions,
    assertPermission, httpError, filterPermissionAdminState, createFilterDefinition, saveGroupFilterGrants,
    restoreUserExtraFilterGrants, saveUserExtraFilterGrants, updateFilterDefinition, badRequest,
  });
