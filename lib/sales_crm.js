...
function authorizedFilterSchema(value, user, page) {
  ...
  const fields = effective.filters.map(definition => {
    ...
    const field = {
      ...
      options: filterOptionsFor(value, user, definition),
    };
    if (definition.key === 'search') {
      const ranges = ['企业', '网站', '行业'];
      if (hasPermission(user, 'view_contacts')) ranges.push('产品', '联系人');
      if (hasPermission(user, 'view_insights')) ranges.push('评价');
...
function filterPermissionAdminState(value) {
  ...
  return {
    ...
    definitions: listFilterDefinitions(value),
