  ...
  try {
    ...
    value.transaction(() => {
      ...
      if (payload.permissions !== undefined) {
        const desired = payload.permissions && typeof payload.permissions === 'object' ? payload.permissions : {};
        const defaults = json(group.permissions_json, {});
        ...
        for (const [permission, allowed] of Object.entries(desired)) {
          if (!PERMISSION_DEFINITIONS[permission]) throw badRequest(`未知权限：${permission}`);
          if (typeof allowed !== 'boolean') throw badRequest(`权限值必须为布尔值：${permission}`);
          remove.run(userId, permission);
          if (Boolean(defaults[permission]) !== allowed) {
