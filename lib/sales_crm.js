...
function accountScope(user) {
  return hasPermission(user, 'view_all_customers')
    ? hasPermission(user, 'manage_intake')
      ? { sql: "WHERE COALESCE(a.lifecycle_status,'active')='active'", params: [] }
      : { sql: "WHERE a.owner_id IS NOT NULL AND COALESCE(a.lifecycle_status,'active')='active'", params: [] }
    : { sql: "WHERE a.owner_id=? AND COALESCE(a.assignment_status,'claimed')!='returned' AND COALESCE(a.lifecycle_status,'active')='active'", params: [user.id] };
}
...
function arbitrateCandidate(value, candidate, users, load, daily, quota, options = {}) {
  ...
  const recommendation = loadSalesMatchRecommendation(value, candidate.external_customer_id || candidate.customer_id, {
    now: options.now,
  });
...
function loadIntakeState(value, user, query = {}) {
  ...
  if (search) {
    filters.push('(i.company_name LIKE ? OR i.external_customer_id LIKE ? OR i.website LIKE ? OR i.industry LIKE ? OR i.product_focus LIKE ?)');
    const like = `%${search}%`;
  ...
  if (query.country) {
    filters.push('i.country=?');
    params.push(String(query.country));
  }
  if (query.owner) {
    if (query.owner === '__unassigned__') {
      filters.push("(i.assigned_owner_id='' AND i.suggested_owner_id='')");
    } else {
      filters.push('(i.assigned_owner_id=? OR i.suggested_owner_id=?)');
      params.push(String(query.owner), String(query.owner));
    }
  }
  ...
  for (const item of items) {
    ...
    item.signals = intakeSignals(value, item);
    ...
    item.arbitration = {
      ...
      aiRecommendation,
      ruleDecision: arbitration?.ruleDecision || {
  ...
  const countWhere = scoped ? 'WHERE assigned_owner_id=?' : '';
  const countParams = scoped ? [user.id] : [];
    ...
    const transaction = value.transaction(() => {
      ...
      if (settings.approvalMode === 'automatic') {
        ...
        for (const item of backlog) {
          const decision = arbitrateCandidate(value, item, users, load, daily, Number(settings.dailyPerSales || 5));
    ...
    if (action === 'bulk_assign') {
      ...
      const transaction = value.transaction(() => {
        ...
        for (const candidate of items) {
          const decision = arbitrateCandidate(value, candidate, users, load, daily, settings.dailyPerSales);
...
function buildCustomerTimeline(value, accounts, activities, rfqs, quotes, orders) {
  const events = [];
  const accountIds = accounts.map(row => row.id);
  ...
  if (accountIds.length && hasTable(value, 'crm_ai_jobs')) {
  ...
  if (accountIds.length && hasTable(value, 'crm_ai_next_action_consumptions')) {
...
function parseBootstrapSections(input) {
  if (input === undefined) return null;
  const raw = Array.isArray(input) ? input.join(',') : String(input);
  const sections = [...new Set(raw.split(',').map(item => item.trim()).filter(Boolean))];
  const unknown = sections.filter(section => !BOOTSTRAP_SECTIONS.has(section));
  if (!sections.length || unknown.length) {
    const error = new Error(unknown.length
      ? `未知 bootstrap section：${unknown.join(', ')}`
      : 'sections 至少需要一个有效值');
    error.statusCode = 400;
    error.code = 'INVALID_BOOTSTRAP_SECTIONS';
    throw error;
  }
  return new Set(sections);
}

function loadPayload(user, selectedSections = null) {
  const value = db();
  try {
    const fullPayload = selectedSections === null;
    const wants = section => fullPayload || selectedSections.has(section);
    const needsAccounts = wants('today') || wants('customers') || wants('team') || wants('intelligence');
    const needsActivities = needsAccounts;
    const needsRfqs = wants('today') || wants('customers') || wants('team');
    const needsQuotes = needsRfqs;
    const needsOrders = wants('customers') || wants('team') || wants('intelligence');
    const needsUsers = wants('team') || wants('administration');
    const needsInsights = wants('customers') || wants('intelligence');
    const scope = accountScope(user);
    const accounts = needsAccounts ? value.prepare(`SELECT a.*,u.name owner_name,m.name manager_name,
    ...
    const activities = needsActivities
      ? value.prepare(`SELECT x.*,u.name user_name FROM crm_activities x LEFT JOIN sales_users u ON u.id=x.user_id
        WHERE x.customer_id IN (${placeholders}) ORDER BY x.occurred_at DESC`).all(...customerIds)
      : [];
    const rfqs = needsRfqs
      ? value.prepare(`SELECT * FROM crm_rfqs WHERE customer_id IN (${placeholders}) ORDER BY received_at DESC`).all(...customerIds)
      : [];
    const quotes = needsQuotes
      ? value.prepare(`SELECT * FROM crm_quotes WHERE customer_id IN (${placeholders}) ORDER BY sent_at DESC`).all(...customerIds)
      : [];
    const orders = needsOrders
      ? value.prepare(`SELECT * FROM crm_orders WHERE customer_id IN (${placeholders}) ORDER BY ordered_at DESC`).all(...customerIds)
      : [];
    const allUsers = needsUsers
    ...
    const alerts = wants('today') || wants('customers')
      ? [...buildIntakeAlerts(value, user), ...buildAlerts(accounts, activities, rfqs, quotes)]
      : [];
    const countryReport = wants('intelligence') ? buildCountryReport(accounts, activities, orders) : [];
    const cohortReport = wants('intelligence') ? buildCohortReport(accounts, activities, orders) : [];
    const teamReport = wants('team')
      ? buildTeamReport(activeUsers, accounts, activities, rfqs, quotes, orders)
      : [];
    const insights = needsInsights ? loadInsights(value, accounts) : { contacts: [], evaluations: [] };
    const customerEvaluationTags = wants('customers') ? accounts.map(account => ({
    ...
    const auditLog = wants('administration') && user.role === 'admin'
      ...
      : [];
    const migrationReview = wants('administration') && user.role === 'admin'
      ? value.prepare("SELECT * FROM crm_migration_review WHERE resolved_at='' ORDER BY created_at,source_id LIMIT 200").all()
      : [];
    const notifications = wants('today') ? value.prepare(`SELECT n.*,recipient.name recipient_name,
    ...
    const funnel = wants('customers')
      ? STAGES.filter(([key]) => !['new', 'lost'].includes(key)).map(([key, label]) => ({ key, label, count: atLeast(key) }))
      : [];
    const wonAccounts = atLeast('won');
    const summary = wants('customers') ? {
    ...
    const timeline = wants('customers') && canSeeAccounts
      ? contactSafe(buildCustomerTimeline(value, accounts, activities, rfqs, quotes, orders))
      : [];
    const intake = wants('intake') && permissions.view_intake
      ? contactSafe(loadIntakeState(value, user))
      : { settings: {}, stats: {}, items: [], batches: [] };
    const payload = {};
    if (wants('core')) {
      payload.user = safeUser(user);
    }
    if (wants('administration')) {
      payload.users = permissions.view_users ? activeUsers.map(safeUser) : [safeUser(user)];
      payload.archivedUsers = permissions.view_users ? archivedUsers.map(safeUser) : [];
      if (permissions.view_users) payload.permissionGroups = listPermissionGroups(value);
    }
    if (wants('customers')) {
      payload.accounts = canSeeAccounts ? contactSafe(accounts) : [];
      payload.activities = canSeeAccounts ? contactSafe(activities) : [];
      payload.rfqs = canSeeAccounts ? contactSafe(rfqs) : [];
      payload.quotes = canSeeAccounts ? contactSafe(quotes) : [];
      payload.orders = canSeeAccounts ? contactSafe(orders) : [];
      payload.timeline = timeline;
    }
    if (wants('today')) payload.alerts = permissions.view_alerts ? contactSafe(alerts) : [];
    if (wants('intelligence')) {
      payload.countryReport = permissions.view_markets ? contactSafe(countryReport) : [];
      payload.cohortReport = permissions.view_markets ? contactSafe(cohortReport) : [];
    }
    if (wants('team')) payload.teamReport = permissions.view_team ? contactSafe(teamReport) : [];
    if (wants('customers')) {
      payload.funnel = permissions.view_dashboard || permissions.view_pipeline ? funnel : [];
      payload.summary = permissions.view_dashboard ? summary : {};
    }
    if (wants('intake')) payload.intake = intake;
    if (wants('intelligence')) {
      payload.insights = permissions.view_insights ? {
        contacts: permissions.view_contacts ? insights.contacts : [],
        evaluations: permissions.view_contacts
          ? insights.evaluations
          : redactContactFields(insights.evaluations.filter(row => row.subjectType === 'company')),
      } : { contacts: [], evaluations: [] };
    }
    if (wants('customers')) {
      payload.customerEvaluationTags = canSeeAccounts ? customerEvaluationTags : [];
    }
    if (wants('intelligence')) {
      payload.customerPool = [];
      payload.people = [];
      payload.reconResults = [];
      payload.researchTotals = researchTotals(value, user, permissions);
    }
    if (wants('administration')) {
      payload.auditLog = permissions.view_users ? contactSafe(auditLog) : [];
      payload.migrationReview = permissions.view_users ? contactSafe(migrationReview) : [];
    }
    if (wants('today')) payload.notifications = contactSafe(notifications);
    if (wants('core')) {
      payload.permissionDefinitions = PERMISSION_DEFINITIONS;
      payload.rolePermissions = ROLE_PERMISSIONS;
      payload.stages = STAGES.map(([key, label]) => ({ key, label }));
      payload.generatedAt = nowText();
    }
    return payload;
  } finally { value.close(); }
}
  ...
  try {
    ...
    const allowed = features.ai_stations.effectiveEnabled
      && ['use_ai_assistant', 'view_customers', 'view_contacts', 'record_activity']
        .every(permission => hasPermission(user, permission));
    if (!allowed || !account.external_customer_id || ['lost', 'won', 'repeat'].includes(account.stage)) return '';
...
function updateReturnedIntake(value, account, reason) {
  if (!account.intake_item_id) return;
  value.prepare(`UPDATE crm_intake_items SET status='returned',assigned_owner_id='',
    assigned_at='',claim_due_at='',claimed_at='',return_reason=?,updated_at=? WHERE id=?`)
    .run(reason, nowText(), account.intake_item_id);
}
...
function returnCustomer(user, customerId, payload = {}, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      if (String(account.lifecycle_status || 'active') !== 'active') {
        throw recycleError(409, '客户已经在回收站', 'CUSTOMER_ALREADY_RECYCLED');
      }
      ...
      value.prepare(`UPDATE crm_accounts SET lifecycle_status='recycled',recycle_kind='sales_return',
        ...
        assignment_status='returned',return_reason=?,updated_at=? WHERE id=?`).run(
...
function bulkReturnCustomers(user, payload = {}, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      const accounts = customerIds.map(customerId => activeAccountForRecycle(value, user, customerId));
      if (accounts.some(account => String(account.lifecycle_status || 'active') !== 'active')) {
        throw recycleError(409, '部分客户状态已变化，请刷新后重试', 'RECYCLE_STATE_CHANGED');
      }
      const recycledAt = nowText();
      for (const account of accounts) {
        value.prepare(`UPDATE crm_accounts SET lifecycle_status='recycled',recycle_kind='sales_return',
          recycle_reason=?,recycled_by=?,recycled_at=?,previous_owner_id=?,owner_id=NULL,
          assignment_status='returned',return_reason=?,updated_at=? WHERE id=? AND COALESCE(lifecycle_status,'active')='active'`).run(
          reason, user.id, recycledAt, account.owner_id || '', reason, recycledAt, account.id,
        );
        updateReturnedIntake(value, account, reason);
...
function exportCrmData(user, query = {}) {
  ...
  try {
    ...
    if (search) {
      const like = `%${search}%`;
      filters.push(`(a.company_name LIKE ? OR a.external_customer_id LIKE ? OR a.country LIKE ? OR a.industry LIKE ? OR a.product_focus LIKE ? OR EXISTS (
        SELECT 1 FROM crm_manager_evaluations e WHERE e.customer_id=a.id AND (e.evaluation_text LIKE ? OR e.ai_labels_json LIKE ?)
      ))`);
      params.push(like, like, like, like, like, like, like);
    }
    if (query.stage) { filters.push('a.stage=?'); params.push(String(query.stage)); }
    if (query.priority) { filters.push('a.priority=?'); params.push(String(query.priority)); }
    if (query.evaluationTag) {
      filters.push(`EXISTS (SELECT 1 FROM crm_manager_evaluations e WHERE e.customer_id=a.id AND e.ai_labels_json LIKE ?)`);
      params.push(`%${String(query.evaluationTag).replace(/[%_]/g, '\\$&')}%`);
    }
    if (String(query.onlyOverdue || '').toLowerCase() === 'true' || query.onlyOverdue === '1') {
      filters.push(`a.next_action_at!='' AND a.next_action_at<? AND a.stage NOT IN ('won','repeat','lost')`);
      params.push(nowText());
    }
    const accounts = value.prepare(`SELECT a.*,
      owner.name owner_name,creator.name creator_name
      FROM crm_accounts a
      LEFT JOIN sales_users owner ON owner.id=a.owner_id
      LEFT JOIN sales_users creator ON creator.id=a.created_by
      WHERE ${filters.join(' AND ')} ORDER BY a.id`).all(...params).map(row => ({
        ...row,
...
function exportCrmCsv(user, query = {}) {
  const value = exportCrmData(user, query);
  const headers = ['客户', '客户编码', '国家', '行业', '阶段', '负责人', '优先级', '潜在金额', '最近动作', '下一步'];
  const rows = value.customers.map(row => [
    row.company_name, row.external_customer_id, row.country, row.industry, row.stage,
...
function registerSalesCrm(app, options = {}) {
  ...
  app.get('/api/sales-crm/bootstrap', (req, res) => {
    ...
    try {
      ...
      if (sections === null || sections.has('core')) {
        ...
        response.realUser = safeUser(req.realUser);
  ...
  app.get('/api/sales-crm/profile/:customerId', (req, res) => {
    try { res.json(getCustomerProfileData(req.accessContext, req.params.customerId)); }
  ...
  app.get('/api/sales-crm/intake', (req, res) => {
    ...
    try {
      const value = db();
      try {
        const payload = loadIntakeState(value, req.salesUser, req.query || {});
  ...
  app.get('/api/sales-crm/export', (req, res) => {
    try {
      if (String(req.query.format || '').toLowerCase() === 'csv') {
        ...
        return res.send(exportCrmCsv(req.salesUser, req.query || {}));
      }
      const filename = `crm-data-${new Date().toISOString().slice(0, 10)}.json`;
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
      return res.send(JSON.stringify(exportCrmData(req.salesUser, req.query || {}), null, 2));
    } catch (error) { sendApiError(res, error); }
  });
