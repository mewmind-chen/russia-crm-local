...
function anonymousSalesRoute(method, requestPath) {
  let route = String(requestPath || '').split('?')[0];
  route = route.replace(/^\/accounts\/[^/]+$/, '/accounts/:customerId')
...
function manageIntake(user, payload) {
  ...
  try {
    ...
    if (['assign', 'reassign'].includes(action)) {
      ...
      const owner = value.prepare("SELECT 1 FROM sales_users WHERE id=? AND role='sales' AND active=1").get(ownerId);
...
function researchOwnerCondition(user, alias, params) {
  if (hasPermission(user, 'view_all_customers')) return '';
  params.push(user.id);
...
function loadPayload(user) {
  ...
  try {
    ...
    return {
      user: safeUser(user),
      users: permissions.view_users ? allUsers.map(safeUser) : [safeUser(user)],
...
function addAccount(user, payload) {
  ...
  const value = db();
  try {
    const ownerId = String(payload.ownerId || '') || (user.role === 'sales' ? user.id : '');
...
function updateAccount(user, customerId, payload) {
  ...
  try {
    ...
    if (payload.ownerId !== undefined) {
      ...
      if (!value.prepare("SELECT 1 FROM sales_users WHERE id=? AND role='sales' AND active=1").get(ownerId)) {
        throw new Error('请选择有效的销售负责人');
      }
...
function systemDefaultPermissionGroup(value, role) {
  const group = value.prepare(`SELECT id,permissions_json FROM permission_groups
    WHERE role_key=? AND system_key=?`).get(role, `${role}-default`);
  if (!group) throw new Error(`缺少${role}默认权限组`);
  return group;
}

function replaceUserPermissionOverrides(value, userId, permissions, group, now) {
  const defaults = normalizePermissions(json(group.permissions_json, {}));
  const desired = normalizePermissions(permissions);
  const remove = value.prepare('DELETE FROM user_permission_overrides WHERE user_id=? AND permission_key=?');
  const insert = value.prepare(`INSERT INTO user_permission_overrides
    (user_id,permission_key,effect,created_at,updated_at) VALUES (?,?,?,?,?)`);
  for (const [permission, allowed] of Object.entries(desired)) {
    remove.run(userId, permission);
    if (Boolean(defaults[permission]) === Boolean(allowed)) continue;
    insert.run(userId, permission, allowed ? 'allow' : 'deny', now, now);
  }
}
...
function createUser(actor, payload) {
  ...
  try {
    ...
    const now = nowText();
    value.transaction(() => {
      const group = systemDefaultPermissionGroup(value, role);
      ...
      replaceUserPermissionOverrides(value, userId, payload.permissions, group, now);
    })();
    return { userId };
  } catch (error) {
...
function updateUser(actor, userId, payload) {
  ...
  try {
    ...
    if (payload.role !== undefined) {
      ...
      if (!['admin', 'manager', 'sales'].includes(role)) throw new Error('请选择有效角色');
      group = systemDefaultPermissionGroup(value, role);
      fields.push('role=?', 'permission_group_id=?');
      params.push(role, group.id);
    }
    ...
    if (payload.permissions !== undefined) {
      group = group || value.prepare(`SELECT g.id,g.permissions_json FROM sales_users u
        JOIN permission_groups g ON g.id=u.permission_group_id WHERE u.id=?`).get(userId);
      if (!group) throw new Error('用户缺少有效权限组');
    }
    if (payload.password) {
    ...
    if (!fields.length && payload.permissions === undefined) return { userId };
    const now = nowText();
    value.transaction(() => {
      if (fields.length) {
      ...
      if (payload.permissions !== undefined) replaceUserPermissionOverrides(value, userId, payload.permissions, group, now);
    })();
    return { userId };
  } finally { value.close(); }
}
      ...
      try {
        if (res.statusCode === 403) {
          value.prepare(`INSERT INTO crm_audit_log
            ...
            id('AUD'), req.salesUser.id, 'permission_denied', 'sales_route', '',
            JSON.stringify({ route: anonymousSalesRoute(req.method, req.path), permission: req.deniedPermission || 'target_scope' }), nowText(),
          );
          return;
        }
