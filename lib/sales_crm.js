...
function canAccess(user, account) {
  return hasPermission(user, 'view_all_customers')
    || (account.owner_id === user.id && account.assignment_status !== 'returned');
}
...
function loadPayload(user) {
  ...
  try {
    ...
    return {
      ...
      alerts: permissions.view_alerts ? alerts : [],
      countryReport: permissions.view_markets ? countryReport : [],
      ...
      intake: permissions.view_intake ? loadIntakeState(value, user) : { settings: {}, stats: {}, items: [], batches: [] },
