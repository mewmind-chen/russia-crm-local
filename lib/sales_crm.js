...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  try {
    ...
    if (page === 'intake' || page === 'lead_flow') {
      ...
      result = listManagerEvaluationCustomers(value, user, ast, query, runtimeOptions);
      ...
      result = listNotificationRows(value, user, ast, query, runtimeOptions);
    } else {
...
function buildCustomerTimeline(value, accounts, activities, rfqs, quotes, orders, options = {}) {
  ...
  for (const activity of activities) {
    ...
    events.push({
      ...
      superseded_by: activity.superseded_by || '',
    });
  }
...
function loadPayload(user, options = {}) {
  ...
  try {
    ...
    const activityHistory = addActivityProvenance(value.prepare(`SELECT x.*,u.name user_name FROM crm_activities x LEFT JOIN sales_users u ON u.id=x.user_id
    ...
    const alerts = authorizeTodayTaskActions(
      groupAlerts([
        ...
        ...buildAlerts(accounts, effectiveActivities, effectiveRfqs, effectiveQuotes, planEvents),
      ]),
      user,
    );
...
function loadRecycleProfile(user, customerId, options = {}) {
  ...
  try {
    ...
    const activities = addActivityProvenance(value.prepare(`SELECT x.*,u.name user_name
      FROM crm_activities x LEFT JOIN sales_users u ON u.id=x.user_id
      WHERE x.customer_id=? AND COALESCE(x.is_test_data,0)=0
      ORDER BY x.occurred_at DESC`).all(account.id)).map(publicActivityRecord);
...
function exportCrmData(user, query = {}, options = {}) {
  ...
  try {
    ...
    const activities = customerIds.length
      ? addActivityProvenance(value.prepare(`SELECT x.*,
          a.external_customer_id,
