...
function manualAssignmentBlockReason(value, item) {
  ...
  const poolHasRisk = hasTable(value, 'customer_pool')
    && value.prepare('PRAGMA table_info(customer_pool)').all()
      .some(column => column.name === 'risk_status');
  const pool = poolHasRisk
    ? value.prepare('SELECT risk_status FROM customer_pool WHERE customer_id=?')
      .get(item.external_customer_id)
    : null;
  const screening = hasTable(value, 'company_screening')
    ? value.prepare(`SELECT risk_level FROM company_screening
      WHERE customer_id=? ORDER BY updated_at DESC LIMIT 1`).get(item.external_customer_id)
    : null;
  const risk = [item.risk_status, pool?.risk_status, screening?.risk_level]
    .map(signal => String(signal || '').trim()).filter(Boolean).join(' | ');
  const explicitlyClear = /\bclear\b|未发现.{0,8}制裁|制裁.{0,8}未命中|未制裁/i.test(risk);
  const explicitBlock = /blocked|sanctioned|受制裁|高风险|军工|军事|国防|导弹|weapon|military|defen[cs]e/i.test(risk)
    || (!explicitlyClear && /制裁.{0,8}(命中|风险)/i.test(risk));
  if (explicitBlock) {
    return '风险或制裁信息阻断';
  }
  return '';
