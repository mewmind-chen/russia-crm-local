...
function createClaimedAccount(value, item, claimedAt, contactDue) {
  const existing = value.prepare('SELECT id,lifecycle_status FROM crm_accounts WHERE external_customer_id=?').get(item.external_customer_id);
  if (existing) {
    // 复用已存在账户(包括回收站里的历史账户):恢复 active + 新负责人,
    // 保留全部开发历史(活动/询价/报价/订单)。
    if (String(existing.lifecycle_status || 'active') !== 'active') {
      value.prepare(`UPDATE crm_accounts SET lifecycle_status='active',recycle_kind='',recycle_reason='',
        recycled_by='',recycled_at='',previous_owner_id='',owner_id=?,updated_at=? WHERE id=?`)
        .run(item.assigned_owner_id || '', claimedAt, existing.id);
    }
    return existing.id;
  }
