...
function loadPayload(user) {
  ...
  try {
    ...
    const customerEvaluationTags = accounts.map(account => ({
    ...
    const migrationReview = user.role === 'admin'
      ...
      : [];
    const notifications = value.prepare(`SELECT n.*,
...
function registerSalesCrm(app, options = {}) {
  ...
  app.post('/api/sales-crm/notifications/:notificationId/read', (req, res) => {
    ...
    try {
      ...
      const visible = notification.user_id === req.salesUser.id
        || (hasPermission(req.salesUser, 'view_all_customers') && Boolean(notification.user_id));
      if (!visible) return res.status(403).json({ ok: false, error: '无权访问该通知' });
