...
const ROLE_PERMISSIONS = {
  ...
  manager: {
    ...
    manage_evaluations: true, run_recon: true, use_prospect_agent: true, use_ai_assistant: true, manage_users: false,
  },
  sales: {
    view_dashboard: true, view_intake: true, view_customers: true, view_development: true,
    view_pool: true, view_contacts: true, view_recon: true, view_pipeline: true,
    ...
    manage_evaluations: false, run_recon: false, use_prospect_agent: false, use_ai_assistant: false, manage_users: false,
  },
};
...
function permissionsFor(row) {
  const defaults = ROLE_PERMISSIONS[row?.role] || ROLE_PERMISSIONS.sales;
  const overrides = normalizePermissions(json(row?.permissions_json, {}));
...
function assertPermission(user, key) {
  if (hasPermission(user, key)) return;
  const error = new Error(`没有权限：${PERMISSION_DEFINITIONS[key] || key}`);
  error.statusCode = 403;
  throw error;
}

function db() {
  const value = new Database(DB_PATH);
...
function loadPayload(user) {
  ...
  try {
    ...
    const externalIds = accounts.map(row => row.external_customer_id).filter(Boolean);
    const researchScope = user.role === 'sales'
      ? { sql: externalIds.length ? `WHERE p.customer_id IN (${externalIds.map(() => '?').join(',')})` : "WHERE 1=0", params: externalIds }
      : { sql: '', params: [] };
    const customerPool = value.prepare(`SELECT p.*,
      ...
      FROM customer_pool p ${researchScope.sql}
      ORDER BY p.last_found DESC,p.customer_id DESC LIMIT 3000`).all(...researchScope.params);
    const people = externalIds.length || user.role !== 'sales'
      ? value.prepare(`SELECT pc.*,
        ...
        FROM person_candidates pc ${user.role === 'sales' ? `WHERE pc.customer_id IN (${externalIds.map(() => '?').join(',')})` : ''}
        ORDER BY pc.sales_ready DESC,pc.contact_level DESC,pc.updated_at DESC LIMIT 1000`).all(...(user.role === 'sales' ? externalIds : []))
      : [];
    const reconResults = value.prepare(`SELECT r.* FROM recon_results r
      ${user.role === 'sales' ? (externalIds.length ? `WHERE r.customer_id IN (${externalIds.map(() => '?').join(',')})` : 'WHERE 1=0') : ''}
      ORDER BY r.updated_at DESC LIMIT 1000`).all(...(user.role === 'sales' ? externalIds : []));
    ...
    const canSeeResearch = permissions.view_development || permissions.view_pool || permissions.view_contacts || permissions.view_recon;
    return {
      user: safeUser(user),
      ...
      customerPool: canSeeResearch ? customerPool : [],
      people: permissions.view_development || permissions.view_contacts ? people : [],
      reconResults: permissions.view_development || permissions.view_recon ? reconResults : [],
...
function registerSalesCrm(app) {
  ...
  app.get('/api/sales-crm/bootstrap', (req, res) => {
    try { res.json({ ok: true, ...loadPayload(req.salesUser) }); }
