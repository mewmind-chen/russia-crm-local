...
const { installCustomerIdentityRegistry } = require('./customer_identity_registry');
const {
...
function buildAlerts(accounts, activities, rfqs, quotes) {
  ...
  for (const account of accounts) {
    ...
    if (!account.next_action || !account.next_action_at) add(account, 'critical', 'NO_NEXT', '缺少下一步计划', '活跃客户没有明确的下一步动作与日期', '立即补充计划');
  ...
  if (item.duplicate_state !== 'cleared') {
    ...
    if (exact) {
      value.prepare(`UPDATE crm_intake_items SET status='duplicate',crm_customer_id=?,
        duplicate_state='exact',decision_reason='客户已在CRM',updated_at=? WHERE id=?`)
        .run(exact.crmAccountId || '', nowText(), item.id);
      return { assigned: false, reason: '客户已在CRM' };
    }
...
function scanDailyIntake(actor = { id: 'system', role: 'admin' }, options = {}) {
  ...
  try {
    ...
    const duplicateRows = loadDuplicateCustomerRows(value, { crmOnly: true });
      ...
      for (const candidate of candidates) {
        ...
        try {
          insert.run(
            ...
            exactDuplicate ? '客户已在CRM' : (fuzzyDuplicates.length ? '资料已提交管理层核验' : decision.reason),
            '', '', '', '', createdAt, createdAt,
          );
          if (exactDuplicate) {
            ...
            recordDuplicateAudit(value, actor.id || 'system', 'duplicate_intake_blocked', itemId, {
              ...
              existingCustomerId: exactDuplicate.customerId,
            });
  ...
  try {
    ...
    if (action === 'manual_assign') {
      ...
      const transaction = value.transaction(() => {
        ...
        const settings = {
          claimSlaHours: settingsRow.claim_sla_hours,
          contactSlaHours: settingsRow.contact_sla_hours,
          duplicateRows: loadDuplicateCustomerRows(value, { crmOnly: true }),
        };
        const assignedIds = [];
    ...
    if (action === 'claim') {
      ...
      if (item.duplicate_state !== 'cleared') {
        ...
        if (exact) {
          value.prepare(`UPDATE crm_intake_items SET status='duplicate',crm_customer_id=?,
            duplicate_state='exact',assigned_owner_id='',decision_reason='客户已在CRM',updated_at=? WHERE id=?`)
            .run(exact.crmAccountId || '', nowText(), item.id);
          throw conflictError('该线索已受已有客户保护，无法领取。', 'LEAD_DUPLICATE_PROTECTED');
        }
        if (fuzzy.length) {
...
function normalizeTodayTaskDate(input) {
  ...
  if (!raw) throw todayTaskError(400, '计划时间不能为空');
  const local = raw.replace('T', ' ');
  const match = local.match(/^(\d{4}-\d{2}-\d{2})[ ](\d{2}:\d{2})(?::(\d{2}))?$/);
  if (match) {
    const normalized = `${match[1]} ${match[2]}:${match[3] || '00'}`;
    if (!Number.isFinite(new Date(`${normalized.replace(' ', 'T')}Z`).getTime())) {
      throw todayTaskError(400, '计划时间格式无效');
    }
    return normalized;
  }
  const parsed = new Date(raw);
  if (!Number.isFinite(parsed.getTime())) throw todayTaskError(400, '计划时间格式无效');
  return nowText(parsed);
}
...
function addNextPlanTodayTask(value, user, payload, spec, identity) {
  ...
  value.prepare(`UPDATE crm_accounts SET next_action=?,next_action_at=?,updated_at=?
...
function loadPayload(user, options = {}) {
  ...
  try {
    ...
    const alerts = authorizeTodayTaskActions(
      groupAlerts([...buildIntakeAlerts(value, user), ...buildAlerts(accounts, activities, rfqs, quotes)]),
      user,
    );
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      if (spec.activityType === 'lost') {
        value.prepare('UPDATE crm_accounts SET loss_reason=?,next_action=\'\',next_action_at=\'\' WHERE id=?')
...
function createDuplicateReview(value, user, input, candidates, target = {}) {
  ...
  value.prepare(`INSERT INTO crm_duplicate_reviews
    ...
    JSON.stringify(candidates.slice(0, 10)), at, at,
  );
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      if (!externalId) {
        ...
        if (duplicate?.crmAccountId) throw exactDuplicateError(value, user, duplicate);
      ...
      value.prepare(`INSERT INTO crm_accounts
        (id,external_customer_id,company_name,country,city,website,industry,customer_type,source,product_focus,priority,potential_value,established_year,stage,owner_id,created_by,manager_id,manager_required,manager_status,last_activity_at,next_action,next_action_at,loss_reason,created_at,updated_at,assignment_status)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
        ...
        establishedYear, String(customerInput.stage || 'qualified'), ownerId || null, user.id, String(customerInput.managerId || 'USR-MGR'), 0, '', '', String(customerInput.nextAction || '完成首次触达'),
        String(customerInput.nextActionAt || dateOffset(1)), '', now, now, ownerId ? 'claimed' : 'unassigned',
      );
  ...
  app.use('/api/sales-crm', (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      try {
        ...
        const anonymousRoute = aiRoute || protectedConflictRoute;
        ...
        value.prepare(`INSERT INTO crm_audit_log
          ...
          aiRoute ? 'ai_station' : (protectedConflictRoute ? 'protected_customer_identity' : (req.path.split('/').filter(Boolean)[1] || 'crm')),
