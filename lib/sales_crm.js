...
const INITIAL_ACTIVITY_REACTIONS = Object.freeze([
  ...
  ['REACTION-FOLLOW-UP', '需要跟进'],
  ['REACTION-NO-ANSWER', '未接通'],
  ['REACTION-NO-REPLY', '暂无回复'],
  ['REACTION-REJECTED', '明确拒绝'],
]);
...
function ensureActivityProgressSchema(value) {
  ...
  const insert = value.prepare(`INSERT OR IGNORE INTO crm_activity_reaction_options
    (id,name,name_key,sort_order,active,created_by,updated_by,created_at,updated_at)
    VALUES (?,?,?,?,1,'system','system',?,?)`);
  const installedAt = nowText();
  INITIAL_ACTIVITY_REACTIONS.forEach(([reactionId, name], index) => {
    insert.run(
      reactionId,
      name,
...
function ensureIntakeItemColumns(value) {
  ...
  if (!columns.has('duplicate_review_id')) value.exec("ALTER TABLE crm_intake_items ADD COLUMN duplicate_review_id TEXT NOT NULL DEFAULT ''");
  if (!columns.has('duplicate_state')) value.exec("ALTER TABLE crm_intake_items ADD COLUMN duplicate_state TEXT NOT NULL DEFAULT ''");
...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  try {
    ...
    if (page === 'intake' || page === 'lead_flow') {
      ...
      result = listManagerTaskRows(value, user, ast, query);
    } else if (page === 'manager_risks') {
      result = listManagerRiskRows(value, user, ast, query);
    } else if (page === 'manager_metrics') {
      result = listManagerMetricRows(value, user, ast, query, {
        settings: getManagerTaskSettings(value),
      });
    } else if (page === 'notifications') {
      result = listNotificationRows(value, user, ast, query);
    } else {
...
function buildAlerts(accounts, activities, rfqs, quotes, planEvents = []) {
  ...
  const alerts = [];
  const add = (account, severity, code, title, detail, action, overdueHours = 0) => {
    ...
    alerts.push({
      ...
      managerRequest: code === 'MANAGER_NEEDED'
...
function contactAccountForUser(value, user, payload = {}) {
  const accountId = String(payload.customerId || '').trim();
  if (accountId) return getAccountForUser(value, user, accountId);
  ...
  const account = value.prepare(`SELECT a.* FROM crm_accounts a
    WHERE a.external_customer_id=? AND ${scope.sql.replace(/^WHERE\s+/i, '')}
    ORDER BY a.id LIMIT 1`).get(externalCustomerId, ...scope.params);
...
function createAccountContact(user, payload, identity = {}) {
  ...
  const value = db();
  try {
    const account = contactAccountForUser(value, user, payload);
    ...
    const transaction = value.transaction(() => {
      value.prepare(`INSERT INTO crm_account_contacts
        ...
        VALUES (?,?,?,?,?,?,?,?,?,'manual','',?,?,?,?)`).run(
        contactId, account.id, account.external_customer_id || '', fields.name, fields.title,
        fields.department, fields.phone, fields.email, fields.social, user.id, user.id, now, now,
      );
      recordContactAudit(value, user, identity, 'customer_contact_created', {
        id: contactId, customer_id: account.id,
...
function profileContacts(value, user, externalCustomerId) {
  ...
  const local = account
    ? value.prepare(`SELECT * FROM crm_account_contacts
        WHERE (external_customer_id=? OR customer_id=?) AND archived_at=''
        ORDER BY name,id`).all(externalCustomerId, account.id).map(publicAccountContact)
    : [];
...
function assignIntakeItem(value, item, ownerId, settings, reason = '') {
  ...
  if (item.duplicate_state === 'review') return { assigned: false, reason: '待管理层查重核验' };
  ...
  if (item.duplicate_state !== 'cleared') {
    ...
    if (fuzzy.length) {
      ...
      return { assigned: false, reason: '待管理层查重核验' };
    }
  }
...
function manualAssignmentBlockReason(value, item) {
  if (item.duplicate_state === 'review') return '待管理层查重核验';
  if (item.duplicate_state === 'exact') return '客户已在 CRM';
  const existing = value.prepare('SELECT id FROM crm_accounts WHERE external_customer_id=?')
    .get(item.external_customer_id);
  if (existing || item.crm_customer_id) return '客户已在 CRM';
  return '';
}

function manualAssignmentScope(value, user, payload = {}) {
  const requestedIds = Array.isArray(payload.itemIds)
  ...
  if (requestedIds.length) {
    ...
    const byId = new Map(value.prepare(`SELECT * FROM crm_intake_items
      WHERE id IN (${placeholders})`).all(...requestedIds).map(item => [item.id, item]));
    ...
    const statusPlaceholders = MANUAL_ASSIGNABLE_STATUSES.map(() => '?').join(',');
    const where = `${scope.where} AND i.status IN (${statusPlaceholders})`;
    const params = [...scope.params, ...MANUAL_ASSIGNABLE_STATUSES];
    scopeType = 'filter';
  ...
  let ineligible = Math.max(0, scopeTotal - rows.length);
  for (const item of rows) {
    let reason = '';
    if (!MANUAL_ASSIGNABLE_STATUSES.includes(item.status)) reason = '状态不可分配';
    else reason = manualAssignmentBlockReason(value, item);
    if (reason) {
      ineligible += 1;
  ...
  try {
    ...
    if (action === 'manual_assign') {
      ...
      const transaction = value.transaction(() => {
        ...
        const scope = manualAssignmentScope(value, user, payload);
        if (amount > scope.eligibleCount) {
          throw httpError(
            400,
            `当前范围只有 ${scope.eligibleCount} 条可分配线索`,
        ...
        for (const item of scope.eligible.slice(0, amount)) {
          const result = assignIntakeItem(value, item, ownerId, settings, '管理者手动定向分配');
        ...
        response = {
          ...
          blocked: scope.blockedCount,
          blockedReasons: scope.blockedReasons,
          unprocessed: Math.max(0, scope.eligibleCount - assignedIds.length),
        };
      });
      transaction.immediate();
    ...
    if (action === 'claim') {
      ...
      value.transaction(() => {
        ...
        value.prepare(`UPDATE crm_accounts SET assignment_status='claimed',claimed_at=?,
          next_action='完成首次触达',next_action_at=?,next_action_time_basis='utc',updated_at=?
          WHERE id=?`)
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET next_action='',next_action_at='',
        next_action_time_basis='',updated_at=? WHERE id=?`).run(changedAt, account.id);
      value.prepare(`INSERT INTO crm_audit_log
        (id,user_id,action,entity_type,entity_id,detail_json,created_at,
         real_user_id,effective_user_id,impersonation_context_id)
        VALUES (?,?,?,?,?,?,?,?,?,?)`).run(
...
function managerTaskAccount(value, user, taskId) {
  ...
  const account = scopedManagerAccount(value, user, task.customerId);
  return { task, account };
}
...
function managerTaskChange(value, actor, task, account, action = {}) {
  ...
  if (type === 'plan_formed' || type === 'manager_advice') {
    ...
    value.prepare(`UPDATE crm_accounts SET next_action=?,next_action_at=?,
      ...
      .run(nextAction, nextActionAt, at, account.id);
  ...
  if (type === 'terminal_stage') {
    ...
    if (!['lost', 'disqualified'].includes(stage)) throw badRequest('请选择正式终止阶段');
    ...
    value.prepare(`UPDATE crm_accounts SET stage=?,next_action='',next_action_at='',
      next_action_time_basis='',loss_reason=?,updated_at=? WHERE id=?`).run(
      stage, String(action.note || '').trim(), at, account.id,
    );
    return {
      changed: true,
  ...
  if (type === 'reassigned') {
    ...
    value.prepare(`UPDATE crm_accounts SET owner_id=?,assignment_status='claimed',
      assigned_at=?,updated_at=? WHERE id=?`).run(ownerId, at, at, account.id);
...
function resolveManagerTaskAction(user, taskId, payload = {}) {
  const value = db();
  try {
    return value.transaction(() => {
...
function resolveOverdueLeadTodayTask(value, user, payload, spec, identity) {
  ...
  if (resolution === 'reassign') {
    ...
    if (linkedAccountId) {
      value.prepare(`UPDATE crm_accounts SET owner_id=?,assignment_status='assigned',
        assigned_at=?,claim_due_at=?,claimed_at='',return_reason='',updated_at=?
        WHERE id=?`).run(
        ownerId,
        actionAt,
        claimDueAt,
    ...
    if (linkedAccountId) {
      value.prepare(`UPDATE crm_accounts SET owner_id=NULL,previous_owner_id=?,
        assignment_status='returned',assigned_at='',claim_due_at='',claimed_at='',
...
function addNextPlanTodayTask(value, user, payload, spec, identity) {
  ...
  const changedAt = nowText();
  value.prepare(`UPDATE crm_accounts SET next_action=?,next_action_at=?,
    next_action_time_basis='utc',updated_at=?
...
function completeManagerAssistanceTodayTask(value, user, payload, spec, identity) {
  ...
  const account = getAccountForUser(value, user, spec.targetId);
  if (!account.manager_required) {
    throw todayTaskError(409, '该管理协助待办已完成或状态已变化', 'TODAY_TASK_STALE');
  }
  const result = String(payload.result || '').trim();
  if (!result) throw todayTaskError(400, '管理者处理结果不能为空');
  if (Array.from(result).length > 2000) {
    throw todayTaskError(400, '管理者处理结果最多2000个字符');
  }
  ...
  value.prepare(`UPDATE crm_accounts SET manager_required=0,manager_status='已完成',
  ...
  recordTodayTaskAudit(
    ...
    user,
    identity,
    'today_task_manager_assistance_completed',
    'crm_account',
    account.id,
    {
...
function loadPayload(user, options = {}) {
  ...
  try {
    ...
    const activities = value.prepare(`SELECT x.*,u.name user_name FROM crm_activities x LEFT JOIN sales_users u ON u.id=x.user_id
      ...
      .all(...customerIds).map(publicActivityRecord);
    ...
    const planEvents = value.prepare(`SELECT 'deferred' type,id,customer_id,review_at,reason,created_at
      FROM crm_deferred_plan_events WHERE customer_id IN (${planPlaceholders})
      UNION ALL
      SELECT 'explicit' type,id,customer_id,'' review_at,'' reason,created_at
      FROM crm_next_plan_events WHERE customer_id IN (${planPlaceholders})`)
    ...
    const alerts = authorizeTodayTaskActions(
      groupAlerts([
        ...
        ...buildAlerts(accounts, activities, rfqs, quotes, planEvents),
      ]),
      user,
    );
    ...
    const migrationReview = user.role === 'admin'
      ...
      : [];
    const duplicateReviews = hasPermission(user, 'view_all_customers') && hasPermission(user, 'manage_intake')
      ? value.prepare("SELECT * FROM crm_duplicate_reviews WHERE status='pending' ORDER BY created_at,id LIMIT 200")
        .all().map(row => publicDuplicateReview(value, row))
      : [];
...
function publicActivityRecord(row) {
  return {
    ...row,
    externalCustomerId: row.external_customer_id || '',
  ...
  try {
    return value.transaction(() => {
      ...
      value.prepare(`INSERT INTO crm_activity_reaction_options
        (id,name,name_key,sort_order,active,created_by,updated_by,created_at,updated_at)
        VALUES (?,?,?,?,1,?,?,?,?)`).run(
        reactionId, name, nameKey, sortOrder, actor.id, actor.id, at, at,
      );
...
function renameActivityReaction(actor, reactionId, payload, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      if (current.name === name) return { reaction: publicActivityReaction(current), unchanged: true };
      value.prepare(`UPDATE crm_activity_reaction_options
        SET name=?,name_key=?,updated_by=?,updated_at=? WHERE id=? AND active=1`)
        .run(name, nameKey, actor.id, nowText(), cleanId);
...
function addActivity(user, payload, options = {}) {
  ...
  try {
    ...
    const nextAction = stopsFollowUp ? '' : String(payload.nextAction || '').trim();
    const nextActionAt = stopsFollowUp
      ? ''
      : (payload.nextActionAt ? parseBusinessDateTime(payload.nextActionAt) : '');
    if (!stopsFollowUp && Boolean(nextAction) !== Boolean(nextActionAt)) {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET stage=?,last_activity_at=?,next_action=?,next_action_at=?,
        ...
        .run(nextStage, occurredAt, terminal ? '' : nextAction, terminal ? '' : nextActionAt,
          terminal || !nextActionAt ? '' : 'utc', managerRequired ? 1 : 0,
          managerRequired ? 1 : 0, nowText(), account.id);
      ...
      if (spec.activityType === 'lost') {
        value.prepare(`UPDATE crm_accounts SET loss_reason=?,next_action='',next_action_at='',
          next_action_time_basis='' WHERE id=?`)
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET stage='quoted',last_activity_at=?,next_action='报价后跟进',
        next_action_at=?,next_action_time_basis='utc',updated_at=? WHERE id=?`)
        .run(sentAt, nextFollowAt, nowText(), account.id);
      value.prepare(`INSERT INTO crm_activities
        (id,customer_id,user_id,activity_type,channel,outcome,summary,next_action,next_action_at,stage_after,manager_required,occurred_at,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
function addOrder(user, payload) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET stage=?,last_activity_at=?,next_action=?,next_action_at=?,
        next_action_time_basis='utc',updated_at=? WHERE id=?`)
        .run(repeat ? 'repeat' : 'won', orderedAt, repeat ? '维护复购关系' : '首单交付与复购培育', nextActionAt, nowText(), account.id);
      value.prepare(`INSERT INTO crm_activities
        (id,customer_id,user_id,activity_type,channel,outcome,summary,next_action,next_action_at,stage_after,manager_required,occurred_at,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
function duplicateFingerprint(input = {}) {
  let domain = '';
  try { domain = canonicalDomain(input.website); } catch (_error) {}
...
function recordDuplicateAudit(value, actorId, action, entityId, detail = {}) {
  value.prepare(`INSERT INTO crm_audit_log
    (id,user_id,action,entity_type,entity_id,detail_json,created_at)
    VALUES (?,?,?,?,?,?,?)`).run(
    id('AUD'), actorId || '', action, 'duplicate_review', entityId || '',
    JSON.stringify(redactAuditPayload(detail)), nowText(),
  );
}

function createDuplicateReview(value, user, input, candidates, target = {}) {
  ...
  const safeInput = {
    ...
    priority: String(input.priority || 'B'), potentialValue: Number(input.potentialValue || 0),
  ...
  value.prepare(`INSERT INTO crm_duplicate_reviews
    ...
    JSON.stringify(safeCandidates), at, at,
  );
  recordDuplicateAudit(value, user.id, 'duplicate_review_submitted', reviewId, {
...
function approvedDuplicateReview(value, reviewId, input, user) {
  ...
  const review = value.prepare("SELECT * FROM crm_duplicate_reviews WHERE id=? AND target_type='manual_customer'")
    .get(String(reviewId));
  if (!review || review.submitted_by !== user.id || review.status !== 'confirmed_distinct'
      || review.fingerprint !== duplicateFingerprint(input)) {
...
function publicDuplicateReview(value, row) {
  const input = parseJsonObject(row.input_json);
  const candidates = json(row.candidates_json, []).map(candidate => {
    const account = candidate.crmAccountId
      ? value.prepare(`SELECT a.id,a.owner_id,a.stage,a.assignment_status,u.name owner_name
        FROM crm_accounts a LEFT JOIN sales_users u ON u.id=a.owner_id WHERE a.id=?`).get(candidate.crmAccountId)
      : value.prepare(`SELECT a.id,a.owner_id,a.stage,a.assignment_status,u.name owner_name
        FROM crm_accounts a LEFT JOIN sales_users u ON u.id=a.owner_id
        WHERE a.external_customer_id=? AND COALESCE(a.lifecycle_status,'active')='active'
        ORDER BY a.updated_at DESC,a.id LIMIT 1`).get(candidate.customerId);
    return {
      ...candidate,
      crmAccountId: account?.id || candidate.crmAccountId || '',
      ownerId: account?.owner_id || '',
      ownerName: account?.owner_name || '',
...
function listDuplicateReviews(user, query = {}) {
  ...
  if (!['pending', 'confirmed_same', 'confirmed_distinct', 'all'].includes(status)) {
    throw badRequest('无效的查重核验状态');
  }
  const value = db();
  try {
    const rows = status === 'all'
      ? value.prepare('SELECT * FROM crm_duplicate_reviews ORDER BY created_at DESC,id DESC LIMIT 500').all()
      : value.prepare(`SELECT * FROM crm_duplicate_reviews WHERE status=?
        ORDER BY created_at DESC,id DESC LIMIT 500`).all(status);
    return { reviews: rows.map(row => publicDuplicateReview(value, row)) };
  } finally { value.close(); }
}

function resolveDuplicateReview(user, reviewId, payload = {}) {
  assertDuplicateReviewManager(user);
  const resolution = String(payload.resolution || payload.action || '');
  if (!['confirmed_same', 'confirmed_distinct'].includes(resolution)) {
    throw badRequest('请选择“确认同一客户”或“确认不是同一客户”');
  }
  const value = db();
  try {
    return value.transaction(() => {
      ...
      if (row.status !== 'pending') {
        ...
        return { review: publicDuplicateReview(value, row), deduplicated: true };
      }
      const candidates = json(row.candidates_json, []);
      let selected = null;
      if (resolution === 'confirmed_same') {
        const candidateCustomerId = String(payload.candidateCustomerId || '');
        selected = candidates.find(candidate => candidate.customerId === candidateCustomerId)
          || (candidateCustomerId ? null : candidates[0]);
        if (!selected) throw badRequest('请选择确认为同一客户的候选记录');
      }
      const at = nowText();
      const note = String(payload.note || '').trim().slice(0, 500);
      value.prepare(`UPDATE crm_duplicate_reviews SET status=?,resolution_note=?,reviewed_by=?,reviewed_at=?,updated_at=?
        WHERE id=? AND status='pending'`).run(resolution, note, user.id, at, at, row.id);
      if (row.target_type === 'intake_item' && row.target_id) {
        if (resolution === 'confirmed_same') {
          const account = selected.crmAccountId
            ? { id: selected.crmAccountId }
            : value.prepare(`SELECT id FROM crm_accounts WHERE external_customer_id=?
              AND COALESCE(lifecycle_status,'active')='active' ORDER BY updated_at DESC,id LIMIT 1`)
              .get(selected.customerId);
          value.prepare(`UPDATE crm_intake_items SET status='duplicate',crm_customer_id=?,
            duplicate_state='exact',assigned_owner_id='',suggested_owner_id='',
            decision_reason='管理层已确认为同一客户',updated_at=? WHERE id=?`)
            .run(account?.id || '', at, row.target_id);
        } else {
          value.prepare(`UPDATE crm_intake_items SET status='approved',duplicate_state='cleared',
            assigned_owner_id='',suggested_owner_id='',decision_reason='查重核验已放行',updated_at=? WHERE id=?`)
            .run(at, row.target_id);
        }
      }
      recordDuplicateAudit(value, user.id, 'duplicate_review_resolved', row.id, {
        resolution,
        targetType: row.target_type,
        targetId: row.target_id,
...
function addAccount(user, payload, options = {}) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      if (!externalId) {
        ...
        if (!approvedReview) {
          ...
          value.prepare(`UPDATE crm_duplicate_reviews SET created_account_id=?,updated_at=? WHERE id=?`)
            .run(customerId, now, approvedReview.id);
        }
      ...
      if (!externalId) {
        ...
        value.prepare(`INSERT INTO customer_pool
          (customer_id,company_name,country,city,website,industry,customer_type,products,established_year,current_pool,source_file,first_found,last_found)
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      ...
      value.prepare(`INSERT INTO crm_accounts
        ...
        establishedYear, initialStage, ownerId || null, user.id, String(customerInput.managerId || 'USR-MGR'), 0, '', '', initialNextAction,
    ...
    if (!result.reviewRequired && customerInput.duplicateReviewId) {
      value.prepare(`UPDATE crm_duplicate_reviews SET created_account_id=?,created_external_customer_id=?,updated_at=?
        WHERE id=?`).run(customerId, result.externalCustomerId, nowText(), String(customerInput.duplicateReviewId));
    }
    ...
    if (error.code === 'CUSTOMER_DUPLICATE') {
      recordDuplicateAudit(value, user.id, 'duplicate_create_blocked', error.publicDetails?.existingCustomerId || '', {
...
function bulkAssignAccounts(user, payload) {
  ...
  try {
    return value.transaction(() => {
      ...
      const rows = value.prepare(`SELECT id,external_customer_id FROM crm_accounts
  ...
  try {
    ...
    const activities = customerIds.length
      ? value.prepare(`SELECT x.*,
        ...
        ORDER BY x.occurred_at DESC,x.id`).all(...customerIds).map(publicActivityRecord)
      : [];
...
function exportCrmCsv(user, query = {}, options = {}) {
  ...
  const headers = ['昵称', '正式名称', '客户编码', '国家', '行业', '阶段', '负责人', '优先级', '潜在金额', '成立年份', '最近动作', '下一步'];
  const rows = value.customers.map(row => [
    ...
    row.ownerName, row.priority, row.potential_value, row.established_year || '', row.last_activity_at, row.next_action,
  ]);
  return `\uFEFF${[headers, ...rows].map(row => row.map(csvCell).join(',')).join('\r\n')}\r\n`;
}
...
function registerSalesCrm(app, options = {}) {
    ...
    res.on('finish', () => {
      ...
      try {
        ...
        value.prepare(`INSERT INTO crm_audit_log
          ...
          anonymousRoute ? '' : (req.params.customerId || req.params.userId || req.body?.customerId || req.body?.itemId || ''),
          JSON.stringify(anonymousRoute ? { route: auditRoute } : redactAuditPayload({ params: req.params, body: req.body || {} })),
          nowText(), identity.realUserId, identity.effectiveUserId, identity.contextId,
        );
      } finally { value.close(); }
    });
  ...
  app.get('/api/sales-crm/intake/:itemId/profile', (req, res) => {
    ...
    try {
      ...
      payload.contactAccess = {
        ...
        canMaintain: hasPermission(req.salesUser, 'manage_customer_contacts')
          && Boolean(payload.profileAccess?.crmAccessible)
          && !payload.profileAccess?.readOnly,
      };
      res.json(redactUnauthorizedProfileTags(value, req.salesUser, payload));
    } catch (error) {
      sendApiError(res, error);
  ...
  app.post('/api/sales-crm/accounts', (req, res) => {
    ...
    try {
      const result = addAccount(req.salesUser, req.body || {}, { enrichmentFlags: runtimeEnrichmentFlags });
      res.status(result.reviewRequired ? 202 : 200).json({ ok: true, ...result });
    }
    catch (error) { sendApiError(res, error); }
  });
  ...
  app.post('/api/sales-crm/duplicate-reviews/:reviewId/resolve', (req, res) => {
    try { res.json({ ok: true, ...resolveDuplicateReview(req.salesUser, req.params.reviewId, req.body || {}) }); }
    catch (error) { sendApiError(res, error); }
  });
    ...
    try {
      res.json({
        ...
        ...deferAccountPlan(req.salesUser, req.params.customerId, req.body || {}, {
          ...
          contextId: req.impersonation?.id || '',
        }),
      });
    } catch (error) { sendApiError(res, error); }
  });
  ...
  app.get('/api/sales-crm/manager-tasks/export', (req, res) => {
    try {
      ...
      const body = rows.map(row => [
        ...
        row.reason, row.status, row.triggeredAt, row.dueAt, row.resolvedAt,
      ]);
      res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  ...
  app.get('/api/sales-crm/manager-tasks/:taskId', (req, res) => {
    const value = db();
    try {
      const { task, account } = managerTaskAccount(value, req.salesUser, req.params.taskId);
      ...
      res.json({
        ...
        interventions,
        risk: buildCustomerPlanRisk(value, {
          user: req.salesUser,
          customerId: task.customerId,
        }),
      });
  ...
  app.post('/api/sales-crm/manager-tasks/:taskId/resolve', (req, res) => {
    try {
      res.json({
        ok: true,
        ...resolveManagerTaskAction(req.salesUser, req.params.taskId, req.body || {}),
      });
    } catch (error) { sendApiError(res, error); }
  });
  ...
  app.get('/api/sales-crm/protected-customer-conflicts', (req, res) => {
    ...
    const value = db();
    try {
      const result = listProtectedIdentityConflicts(value, {
        ...req.salesUser,
        isImpersonating: Boolean(req.impersonation),
      }, {
  ...
  app.post('/api/sales-crm/protected-customer-conflicts/rescan', (req, res) => {
    ...
    const value = db();
    try {
      const result = rescanProtectedIdentityConflicts(value, {
        ...req.salesUser,
        isImpersonating: Boolean(req.impersonation),
      }, {
  ...
  app.post('/api/sales-crm/protected-customer-conflicts/:conflictId/resolve', (req, res) => {
    ...
    const value = db();
    try {
      const result = resolveProtectedIdentityConflict(value, {
        ...req.salesUser,
        isImpersonating: Boolean(req.impersonation),
      }, {
  ...
  app.put('/api/sales-crm/users/:userId/permission-overrides', (req, res) => {
    ...
    try {
      const body = req.body || {};
      const unsupported = Object.keys(body).find(key => key !== 'permissions');
