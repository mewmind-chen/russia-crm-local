...
function addAccount(user, payload, options = {}) {
  ...
  const value = db();
  try {
    const ownerId = String(customerInput.ownerId || '') || (user.role === 'sales' ? user.id : '');
    if (!ownerId || !authorizedSalesUser(value, ownerId)) throw new Error('请选择有效的销售负责人');
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`INSERT INTO crm_accounts
        ...
        String(customerInput.stage || 'qualified'), ownerId, String(customerInput.managerId || 'USR-MGR'), 0, '', '', String(customerInput.nextAction || '完成首次触达'),
...
function updateAccount(user, customerId, payload) {
  ...
  try {
    ...
    if (payload.ownerId !== undefined) {
      ...
      if (!authorizedSalesUser(value, ownerId)) {
