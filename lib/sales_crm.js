...
} = require('./domains/identity');
const accountNormalize = require('./domains/customer/normalize');
const customerIdentity = require('./domains/customer/identity');
const customerDedupe = require('./domains/customer/dedupe');
const { validateRecycleReason } = require('./domains/customer/recycle');
const customerRecycle = require('./domains/customer/recycle');
const { customerCreateRequestHash } = require('./domains/customer/create');
const accountSummary = require('./domains/customer/summary');
const customerContacts = require('./domains/customer/contacts');
const activityPresent = require('./domains/activity/present');
const activityProgress = require('./domains/activity/progress');
const activitySerialize = require('./domains/activity/serialize');
const activityRequest = require('./domains/activity/request');
const planningStreak = require('./domains/planning/streak');
const planningAlerts = require('./domains/planning/alerts');
const planningRisk = require('./domains/planning/risk');
const planningTodayTask = require('./domains/planning/today_task');
const intakeOwner = require('./domains/intake/owner');
const intakeQuery = require('./domains/intake/query');
const intakeDecision = require('./domains/intake/decision');
const intakeAssignment = require('./domains/intake/assignment');
const commerceRules = require('./domains/commerce/rules');
const assignmentLink = require('./domains/assignment/link');
const authAccess = require('./domains/auth/access');
const reportingBuilders = require('./domains/reporting/builders');
const reportingCsv = require('./domains/reporting/csv');
const auditRedact = require('./domains/audit/redact');
const authCredentials = require('./domains/auth/credentials');
const authSession = require('./domains/auth/session');
const authUser = require('./domains/auth/user');
const listPagination = require('./domains/list/pagination');
const jsonParse = require('./domains/json/parse');
const filterErrors = require('./domains/filter/errors');
const httpRoutes = require('./domains/http/routes');
const httpErrorDomain = require('./domains/http/error');
const notificationVisibility = require('./domains/notifications/visibility');
const insightLabels = require('./domains/insights/labels');
const insightEvaluation = require('./domains/insights/evaluation');
const {
...
const {
  ...
  projectManagerState,
  projectNextAction,
} = require('./domains/lifecycle/state_projection');
const {
  applyAccountStatePatch,
  buildAccountInsertState,
} = require('./domains/lifecycle/state_write');
const {
  applyAccountPlanPatch,
  applyManagerStatusPatch,
  PLAN_TIME_BASIS,
} = require('./domains/lifecycle/collaboration_write');
const {
...
function notificationVisibleForFeatures(code, features) {
  return notificationVisibility.notificationVisibleForFeatures(code, features);
}

function anonymousSalesRoute(method, requestPath) {
  return httpRoutes.anonymousSalesRoute(method, requestPath);
}

const ACTIVITY_STAGE = activityProgress.ACTIVITY_STAGE;

const PROGRESS_TYPE_MAP = activityProgress.PROGRESS_TYPE_MAP;

const LEGACY_ACTIVITY_TYPES = activityProgress.LEGACY_ACTIVITY_TYPES;

const LEGACY_ACTIVITY_CHANNELS = activityProgress.LEGACY_ACTIVITY_CHANNELS;
...
function json(value, fallback = []) {
  return jsonParse.json(value, fallback);
}
...
function parseJsonObject(value) {
  return jsonParse.parseJsonObject(value);
}

function serializeArbitrationDecision(decision = {}) {
  return intakeDecision.serializeArbitrationDecision(decision);
}

function withoutArbitrationAI(decision = {}, fallbackReason = '') {
  return intakeDecision.withoutArbitrationAI(decision, fallbackReason);
}

function serializeRecommendation(recommendation = {}) {
  return intakeDecision.serializeRecommendation(recommendation);
}
...
function redactAuditPayload(value) {
  return auditRedact.redactAuditPayload(value);
}

function hashPassword(password, salt = crypto.randomBytes(16).toString('hex')) {
  return authCredentials.hashPassword(password, salt);
}

function safeUser(row) {
  return authUser.safeUser(row);
}

function httpError(statusCode, message, code = '') {
  return httpErrorDomain.httpError(statusCode, message, code);
}

const badRequest = message => httpErrorDomain.badRequest(message);
const notFound = message => httpErrorDomain.notFound(message);
const conflictError = (message, code = '') => httpErrorDomain.conflictError(message, code);

function normalizeAccountNickname(input) {
  return accountNormalize.normalizeAccountNickname(input, { badRequest });
}

function normalizeActivityReactionName(input) {
  return activityPresent.normalizeActivityReactionName(input, { badRequest });
}

function activityReactionNameKey(input) {
  return activityPresent.activityReactionNameKey(input, { badRequest });
}

function normalizeCustomerStarReason(value) {
  return accountNormalize.normalizeCustomerStarReason(value, { badRequest });
}
...
function parseCookies(header = '') {
  return authSession.parseCookies(header);
}
...
const { createIdentityMiddleware } = require('./domains/identity/middleware');
const identityMiddleware = createIdentityMiddleware({
  openDb: db,
  getSession: sessionIdentity,
  buildAccessContext,
});
const { requireSalesUser, requireUnifiedUser } = identityMiddleware;

function rate(numerator, denominator) {
  return reportingBuilders.rate(numerator, denominator);
}
...
function filterVersionError() {
  return filterErrors.filterVersionError({ httpError });
}
...
function listCustomerAccounts(user, query = {}) {
  ...
  try {
    ...
    rows.forEach(row => {
      row.customerTags ||= [];
      Object.assign(row, projectAccountState(row));
    });
...
function noPlanStreakForActivities(activities) {
  return planningStreak.noPlanStreakForActivities(activities);
}
...
function buildAlerts(accounts, activities, rfqs, quotes, planEvents = [], managerTasks = []) {
  ...
  for (const account of accounts) {
    ...
    const assignmentProjection = projectAssignmentState(account);
    const managerProjection = projectManagerState(account);
    const nextProjection = projectNextAction(account);
    if (assignmentProjection.key === 'assigned' && claimDue && claimDue < now) add(account, 'critical', 'UNCLAIMED', '每日客户未按时领取', '系统推送的客户已超过领取时限', '立即领取或重新分配', (now - claimDue) / 3600000);
    ...
    if (nextProjection.overdue) add(account, 'critical', 'OVERDUE', '跟进任务已超期', `${account.next_action} 已超过计划时间`, '今天完成跟进', (now - nextAt) / 3600000);
    ...
    if (managerProjection.required && managerProjection.status === '待介入') add(account, 'warning', 'MANAGER_NEEDED', '需要主管协助', managerProjection.status || '销售已发起主管协助', '安排主管参与');
    if (managerProjection.required && managerProjection.status === '已回复') {
    ...
    if (last && last.manager_required && age > 72 && managerProjection.status === '已介入') add(account, 'critical', 'POST_MANAGER_IDLE', '管理者介入后销售未承接', '管理者参与后超过3天没有销售跟进行动', '销售立即承接');
...
const ALERT_REASON_ORDER = planningAlerts.ALERT_REASON_ORDER;

function reasonOrder(alert) {
  return planningAlerts.reasonOrder(alert);
}

function urgencyFor(alert) {
  return planningAlerts.urgencyFor(alert);
}

function groupAlerts(alerts) {
  return planningAlerts.groupAlerts(alerts);
}
...
function buildCountryReport(accounts, activities, orders) {
  return reportingBuilders.buildCountryReport(accounts, activities, orders);
}

function buildCohortReport(accounts, activities, orders) {
  return reportingBuilders.buildCohortReport(accounts, activities, orders);
}

function buildTeamReport(users, accounts, activities, rfqs, quotes, orders) {
  return reportingBuilders.buildTeamReport(users, accounts, activities, rfqs, quotes, orders);
}

function normalizeCountry(value) {
  return accountNormalize.normalizeCountry(value);
}

function chooseIntakeOwner(candidate, users, loadByOwner = {}, dailyByOwner = {}, quota = 5) {
  return intakeOwner.chooseIntakeOwner(candidate, users, loadByOwner, dailyByOwner, quota);
}
...
function intakeQueryValues(value, limit = 50) {
  return intakeQuery.intakeQueryValues(value, limit);
}

function intakeQueryBoolean(value) {
  return intakeQuery.intakeQueryBoolean(value);
}

function intakeQueryDate(value, endOfDay = false) {
  return intakeQuery.intakeQueryDate(value, endOfDay);
}
...
function normalizeEvaluation(row) {
  return insightEvaluation.normalizeEvaluation(row);
}

function withoutEvaluationAI(evaluation) {
  return insightEvaluation.withoutEvaluationAI(evaluation);
}

function withoutEvaluationAIRow(evaluation) {
  return insightEvaluation.withoutEvaluationAIRow(evaluation);
}

function aiFeatureDisabled() {
  return insightEvaluation.aiFeatureDisabled();
}

function safeEvaluationLabel(value) {
  return insightLabels.safeEvaluationLabel(value);
}
...
function cleanContactFields(payload = {}) {
  return customerContacts.cleanContactFields(payload);
}

const CONTACT_MATCH_STATUS_LABELS = customerContacts.CONTACT_MATCH_STATUS_LABELS;
const CONTACT_PROCUREMENT_ROLE_LABELS = customerContacts.CONTACT_PROCUREMENT_ROLE_LABELS;

function publicAccountContact(row) {
  return customerContacts.publicAccountContact(row);
}
...
function isReturnedAccountForIntake(account, item) {
  return assignmentLink.isReturnedAccountForIntake(account, item);
}

function isCurrentIntakeAccount(account, item) {
  return assignmentLink.isCurrentIntakeAccount(account, item);
}
...
function reusableReturnedAccountForIntake(accounts, item) {
  return assignmentLink.reusableReturnedAccountForIntake(accounts, item);
}
...
function createClaimedAccount(value, item, claimedAt, contactDue, claimerId = '') {
  ...
  if (existing) {
    ...
    applyAccountStatePatch(value, existing.id, {
      ...
      ownerId: item.assigned_owner_id || null,
  ...
  const insertState = buildAccountInsertState({
    stage: 'qualified',
    lifecycleStatus: 'active',
    assignmentStatus: 'claimed',
    ownerId: item.assigned_owner_id,
  ...
  value.prepare(`INSERT INTO crm_accounts
    ...
    0, insertState.stage, insertState.owner_id, String(item.created_by || '').trim() || 'system', claimerId, claimedAt, 'USR-MGR', 0, '', '', '完成首次触达',
    contactDue, 'utc', '', claimedAt, claimedAt, item.id, insertState.assignment_status, item.assigned_at || claimedAt,
...
function intakeActionIdempotencyKey(user, payload) {
  return intakeAssignment.intakeActionIdempotencyKey(user, payload);
}
...
function manualAssignmentRequestHash(user, payload) {
  return intakeAssignment.manualAssignmentRequestHash(user, payload);
}
...
function manualAssignmentRequiresPreview(payload = {}) {
  return intakeAssignment.manualAssignmentRequiresPreview(payload);
}
...
function todayTaskError(statusCode, message, code = 'TODAY_TASK_INVALID') {
  return planningTodayTask.todayTaskError(statusCode, message, code, { error: httpError });
}
...
function normalizeTodayTaskDate(input) {
  return planningTodayTask.normalizeTodayTaskDate(input, { parseBusinessDateTime });
}
...
function emptyCustomerPlanRisk(task, account) {
  return planningRisk.emptyCustomerPlanRisk(task, account);
}
...
function managerTaskChange(value, actor, task, account, action = {}) {
  ...
  if (type === 'terminal_stage') {
    ...
    applyAccountPlanPatch(value, account.id, { nextAction: '', nextActionAt: '', timeBasis: '' });
    value.prepare('UPDATE crm_accounts SET loss_reason=? WHERE id=?').run(
      String(action.note || '').trim(), account.id,
    );
    return {
      changed: true,
  ...
  if (resolution === 'reassign') {
    ...
    if (linkedAccountId) {
      ...
      value.prepare(`UPDATE crm_accounts SET assigned_at=?,claim_due_at=?,claimed_at='',return_reason=''
        WHERE id=?`).run(actionAt, claimDueAt, linkedAccountId);
    }
  } else {
    value.prepare(`UPDATE crm_intake_items SET status='returned',assigned_owner_id='',
      suggested_owner_id='',decision_reason=?,return_reason=?,assigned_at='',claim_due_at='',
    ...
    if (linkedAccountId) {
      ...
      value.prepare(`UPDATE crm_accounts SET previous_owner_id=?,assigned_at='',claim_due_at='',claimed_at='',
        return_reason=? WHERE id=?`).run(previousOwnerId, reason, linkedAccountId);
    }
  }
  const detail = {
    resolution,
...
function confirmManagerAssistanceTodayTask(value, user, payload, spec, identity) {
  ...
  applyManagerStatusPatch(value, account.id, { required: false, status: '已完成' });
  recordExplicitPlanIfEnabled(
...
function normalizeListQuery(query = {}) {
  return listPagination.normalizeListQuery(query);
}
...
function creatorDisplayName(row) {
  return accountSummary.creatorDisplayName(row);
}

function changedFieldLabels(changed, key) {
  return accountSummary.changedFieldLabels(changed, key);
}
...
function loadPayload(user, options = {}) {
  ...
  try {
    ...
    accounts.forEach(account => {
      ...
      Object.assign(account, projectAccountState(account));
    });
    ...
    return {
      ...
      accounts: canSeeAccounts
        ? attachCustomerStarState(value, user, permissions.view_contacts ? accounts : contactSafeAccountRecord(accounts))
        : [],
      activities: canSeeAccounts ? (permissions.view_contacts ? activityHistory : contactSafeActivityRecord(activityHistory)) : [],
      rfqs: canSeeAccounts ? (permissions.view_contacts ? rfqs : contactSafeCommerceRecord(rfqs)) : [],
      quotes: canSeeAccounts ? (permissions.view_contacts ? quotes : contactSafeCommerceRecord(quotes)) : [],
      orders: canSeeAccounts ? (permissions.view_contacts ? orders : contactSafeCommerceRecord(orders)) : [],
      timeline,
...
function historyAccountSummary(account) {
  return accountSummary.historyAccountSummary(account);
}
...
function inaccessibleOrMissing(user, missingMessage) {
  return authAccess.inaccessibleOrMissing(user, missingMessage);
}

function escapeActivitySearchLike(value) {
  return activityPresent.escapeActivitySearchLike(value);
}
...
function publicActivityReaction(row) {
  return activityPresent.publicActivityReaction(row);
}

function normalizeActivityActionQueueKey(value) {
  return activityPresent.normalizeActivityActionQueueKey(value, { badRequest });
}

function scopedActivityProvenance(row, visibleActivityIds) {
  return activityPresent.scopedActivityProvenance(row, visibleActivityIds);
}

function publicActivityRecord(row, visibleActivityIds = new Set()) {
  return activitySerialize.publicActivityRecord(row, visibleActivityIds);
}

function publicActivityRecords(rows) {
  return activitySerialize.publicActivityRecords(rows);
}
...
function advanceStage(current, proposed) {
  return commerceRules.advanceStage(current, proposed);
}
...
function validateMargin(value, allowNegative) {
  return commerceRules.validateMargin(value, allowNegative, { badRequest });
}

function validateRfqPayload(payload = {}) {
  return commerceRules.validateRfqPayload(payload, { badRequest });
}

function commerceActionIdempotencyKey(user, action, payload, customerId) {
  return commerceRules.commerceActionIdempotencyKey(user, action, payload, customerId);
}
...
function legacyProgressKey(activityType, channel) {
  return activityPresent.legacyProgressKey(activityType, channel);
}

function resolveActivityRequestSpec(payload = {}) {
  return activityProgress.resolveActivityRequestSpec(payload, { badRequest });
}
...
function resolveActivityReaction(value, payload = {}) {
  return activityRequest.resolveActivityReaction(payload, {
    badRequest,
    conflictError,
...
function addActivity(user, payload, options = {}) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      const stateUpdatedAt = nowText();
      applyAccountStatePatch(value, account.id, { stage: nextStage, updatedAt: stateUpdatedAt });
      applyAccountPlanPatch(value, account.id, {
        nextAction: managerRequired ? String(account.next_action || '') : (terminal ? '' : nextAction),
        nextActionAt: managerRequired ? String(account.next_action_at || '') : (terminal ? '' : nextActionAt),
        timeBasis: managerRequired ? String(account.next_action_time_basis || '')
          : (terminal || !nextActionAt ? '' : 'utc'),
      });
      if (managerRequired) {
        applyManagerStatusPatch(value, account.id, { required: true, status: '待介入' });
      }
      value.prepare('UPDATE crm_accounts SET last_activity_at=? WHERE id=?')
...
function addQuote(user, payload, options = {}) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      applyAccountStatePatch(value, account.id, { stage: 'quoted', updatedAt: nowText() });
      value.prepare(`UPDATE crm_accounts SET last_activity_at=?,next_action='报价后跟进',
        next_action_at=?,next_action_time_basis=? WHERE id=?`)
        .run(sentAt, nextFollowAt, 'utc', account.id);
      value.prepare(`INSERT INTO crm_activities
        (id,customer_id,user_id,activity_type,channel,outcome,summary,next_action,next_action_at,stage_after,manager_required,occurred_at,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
function addOrder(user, payload) {
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      const nextStage = repeat ? 'repeat' : 'won';
      applyAccountStatePatch(value, account.id, { stage: nextStage, updatedAt: nowText() });
      value.prepare(`UPDATE crm_accounts SET last_activity_at=?,next_action=?,next_action_at=?,
        next_action_time_basis='utc' WHERE id=?`)
        .run(orderedAt, repeat ? '维护复购关系' : '首单交付与复购培育', nextActionAt, account.id);
      value.prepare(`INSERT INTO crm_activities
        (id,customer_id,user_id,activity_type,channel,outcome,summary,next_action,next_action_at,stage_after,manager_required,occurred_at,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
function duplicateFingerprint(input = {}, ruleVersion = DUPLICATE_RULE_VERSION) {
  return customerDedupe.duplicateFingerprint(input, ruleVersion);
}
...
function identityConflictNote(input) {
  return customerIdentity.identityConflictNote(input);
}
...
function hydrateDuplicateCandidate(candidate, catalog) {
  return customerDedupe.hydrateDuplicateCandidate(candidate, catalog);
}

function reviewCandidateRows(row) {
  return customerDedupe.reviewCandidateRows(row);
}

function reviewHasProtectedExact(row) {
  return customerDedupe.reviewHasProtectedExact(row);
}
...
function normalizeEstablishedYear(value) {
  return accountNormalize.normalizeEstablishedYear(value, { badRequest });
}
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`INSERT INTO crm_accounts
        ...
        establishedYear, insertState.stage, insertState.owner_id, creatorId, String(customerInput.managerId || 'USR-MGR'), 0, '', '', initialNextAction,
        initialNextActionAt, stopsFollowUp ? '' : 'utc', '', now, now, insertState.assignment_status,
      );
      if (!stopsFollowUp) {
...
function updateAccount(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    const allowed = {
      ...
      managerRequired: 'manager_required',
    ...
    for (const [key, column] of Object.entries(allowed)) {
      ...
      params.push(key === 'managerRequired'
        ? (payload[key] ? 1 : 0)
        : key === 'establishedYear' ? establishedYear : payload[key]);
    }
    if (stopsFollowUp) {
      planPatch = { nextAction: '', nextActionAt: '', timeBasis: '' };
    } else if (touchesPlan) {
      planPatch = { nextAction, nextActionAt, timeBasis: nextAction && nextActionAt ? PLAN_TIME_BASIS : '' };
    }
    if (payload.ownerId !== undefined) {
      ...
      if (ownerId !== String(account.owner_id || '')) {
        ...
        fields.push('assigned_at=?', "return_reason=''");
        params.push(ownerId ? nowText() : '');
        ownerPatch = { ownerId: ownerId || null, assignmentStatus: ownerId ? 'claimed' : 'unassigned' };
        changedOwnerId = ownerId;
      }
    }
    if (payload.stage !== undefined) {
      stagePatch = payload.stage;
    }
    const masterAllowed = {
    ...
    if (!fields.length && !masterFields.length && !nicknameChange
        && stagePatch === undefined && !ownerPatch && !planPatch) {
      return { customerId: account.id, nickname: account.nickname || '' };
    }
    const transaction = value.transaction(() => {
      const stateUpdatedAt = nowText();
      if (stagePatch !== undefined) {
        applyAccountStatePatch(value, account.id, { stage: stagePatch, updatedAt: stateUpdatedAt });
      }
      if (ownerPatch) {
        applyAccountStatePatch(value, account.id, { ...ownerPatch, updatedAt: stateUpdatedAt });
      }
      if (planPatch) {
        applyAccountPlanPatch(value, account.id, { ...planPatch, updatedAt: stateUpdatedAt });
      }
      if (nicknameChange) {
        try {
          assertCustomerIdentityAvailable(value, {
...
function bulkAssignAccounts(user, payload, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      const now = nowText();
      for (const row of rows) {
        applyAccountStatePatch(value, row.id, {
          ownerId: ownerId || null,
          assignmentStatus: ownerId ? 'claimed' : 'unassigned',
          updatedAt: now,
        });
        value.prepare('UPDATE crm_accounts SET assigned_at=? WHERE id=?')
          .run(ownerId ? now : '', row.id);
      }
...
function assertCustomerReturnEligible(account) {
  return customerRecycle.assertCustomerReturnEligible(account, { httpError: recycleError });
}

function manualReturnBatchId(at) {
  return customerRecycle.manualReturnBatchId(at);
}
...
function applyCustomerReturn(value, user, identity, account, reason, action, batchSize = null) {
  const updatedAt = nowText();
  const updated = applyAccountStatePatch(value, account.id, {
    lifecycleStatus: 'active',
    assignmentStatus: 'returned',
    ownerId: null,
    updatedAt,
...
function returnCustomer(user, customerId, payload = {}, identity = {}) {
  const reason = validateRecycleReason(payload.reason, { httpError });
...
function rejectCrmCustomer(user, customerId, payload = {}, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      const updatedAt = nowText();
      const updated = value.prepare(`UPDATE crm_accounts SET lifecycle_status='recycled',recycle_kind='mismatch',
        recycle_reason=?,recycled_by=?,recycled_at=?,previous_owner_id=?,owner_id=NULL,
        assignment_status='returned',stage='lost',loss_reason=?,return_reason=?,updated_at=? WHERE id=?`).run(
        reason, user.id, updatedAt, account.owner_id || '', reason, reason, updatedAt, account.id,
      );
      if (updated.changes !== 1) {
...
function parseMismatchRecordKey(recordKey) {
  return customerRecycle.parseMismatchRecordKey(recordKey, { httpError: recycleError });
}

function mismatchRecordNotFound() {
  return customerRecycle.mismatchRecordNotFound({ httpError: recycleError });
}
...
function restoreManualCustomer(user, customerId, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET recycle_kind='',recycle_reason='',
        recycled_by='',recycled_at='',previous_owner_id='',return_reason='' WHERE id=?`)
        .run(account.id);
  ...
  try {
    return value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET recycle_kind='',recycle_reason='',
        ...
        claimed_at='',return_reason='' WHERE id=?`).run(assignedAt, claimDue, account.id);
      if (account.intake_item_id) {
        value.prepare(`UPDATE crm_intake_items SET status='assigned',assigned_owner_id=?,
...
function csvCell(value) {
  return reportingCsv.csvCell(value);
}

function csvSerialize(headers, rows) {
  return reportingCsv.csvSerialize(headers, rows);
}
...
function listPage(input = {}, fallback = 50) {
  return listPagination.listPage(input, fallback);
}
...
function registerSalesCrm(app, options = {}) {
  ...
  app.get('/api/sales-crm/profile/:customerId', (req, res) => {
    ...
    try {
      ...
      if (payload.profileAccess?.accountId && payload.profileAccess?.crmAccessible) {
        const profileState = value.prepare('SELECT * FROM crm_accounts WHERE id=? LIMIT 1')
          .get(payload.profileAccess.accountId);
        if (profileState) payload.state = projectAccountState(profileState).state;
      }
    ...
    try {
      ...
      const transaction = value.transaction(() => {
        if (!account) {
          ...
          value.prepare(`INSERT INTO crm_accounts
            (id,external_customer_id,company_name,country,city,website,industry,customer_type,source,product_focus,
             priority,potential_value,stage,owner_id,assignment_status,lifecycle_status,next_action,next_action_at,created_at,updated_at)
            SELECT ?,customer_id,company_name,country,city,website,industry,customer_type,'旧跟进复核迁移',products,
              'B',0,?,?,?,?,?,?, ?,? FROM customer_pool WHERE customer_id=?`).run(
            accountId, insertState.stage, insertState.owner_id, insertState.assignment_status,
            insertState.lifecycle_status, row.next_action || '复核并继续跟进',
