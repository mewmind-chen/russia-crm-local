  ...
  try {
    ...
    if (action === 'claim') {
      ...
      value.transaction(() => {
        ...
        value.prepare(`UPDATE crm_accounts SET claimed_at=?,
          next_action='完成首次触达',next_action_at=?,next_action_time_basis='utc'
          WHERE id=?`)
          .run(claimedAt, contactDue, accountId);
      }).immediate();
      recordIntakeDecision(value, item.id, {
        decisionType: 'manual',
        actorId: user.id,
...
function managerTaskChange(value, actor, task, account, action = {}) {
  ...
  if (type === 'terminal_stage') {
    ...
    value.prepare(`UPDATE crm_accounts SET next_action='',next_action_at='',
      next_action_time_basis='',loss_reason=? WHERE id=?`).run(
  ...
  try {
    ...
    const transaction = value.transaction(() => {
      ...
      value.prepare(`UPDATE crm_accounts SET last_activity_at=?,next_action=?,next_action_at=?,
        ...
        manager_status=CASE WHEN ?=1 THEN '待介入' ELSE manager_status END
        WHERE id=?`)
        .run(occurredAt,
          ...
          managerRequired ? 1 : 0,
          managerRequired ? 1 : 0, account.id);
      if (managerRequired) {
...
function updateAccount(user, customerId, payload, identity = {}) {
  ...
  try {
    ...
    if (!fields.length && !masterFields.length && !nicknameChange
        && stagePatch === undefined && !ownerPatch) {
