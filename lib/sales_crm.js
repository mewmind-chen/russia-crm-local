...
function loadAuthorizedBusinessPage(user, pageKey, query = {}, options = {}) {
  ...
  if (page === 'intake') allowedQueryKeys.add('sort');
  if (page === 'pipeline') {
  ...
  if (page === 'intake') {
    ...
    if (requestedSort && !INTAKE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'alerts') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort && !ALERT_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'notifications') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort && !NOTIFICATION_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'insights') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort && !INSIGHT_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  ...
  if (page === 'recycle_bin') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort && !RECYCLE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  if (page === 'pipeline') {
    const requestedSort = String(query.sort || '').trim();
    if (requestedSort && !PIPELINE_SORT_VALUES.includes(requestedSort)) {
      throw httpError(403, '排序条件未获授权', 'SORT_NOT_AUTHORIZED');
    }
  }
  const listQuery = {
