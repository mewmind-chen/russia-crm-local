...
function loadIntakeState(value, user) {
  ...
  const scoped = !hasPermission(user, 'manage_intake');
  const where = scoped ? 'WHERE i.assigned_owner_id=?' : '';
...
function exportCrmData(user) {
  ...
  try {
    ...
    const accounts = value.prepare(`SELECT a.*,
      ...
      LEFT JOIN sales_users creator ON creator.id=a.created_by
      ${scope.sql} ORDER BY a.id`).all(...scope.params).map(row => ({
        ...row,
...
function registerSalesCrm(app, options = {}) {
  ...
  app.get('/api/sales-crm/export', (req, res) => {
    try {
      ...
      res.send(JSON.stringify(exportCrmData(req.salesUser), null, 2));
    } catch (error) { sendApiError(res, error); }
  });
