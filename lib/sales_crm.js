...
function loadPayload(user) {
  ...
  try {
    ...
    const notifications = value.prepare(`SELECT * FROM crm_notifications
      WHERE (?=1 AND user_id!='') OR (
        user_id=? AND (
          customer_id='' OR customer_id IN (${placeholders}) OR customer_id IN (
...
function registerSalesCrm(app, options = {}) {
  installSalesCrm();
  const aiStationsEnabled = resolveAIStationsEnabled({ enabled: options.aiStationsEnabled });
  const enrichmentFlags = resolveCustomerEnrichmentFlags({
    enabled: options.customerEnrichmentEnabled ?? options.salesCrm?.customerEnrichmentEnabled,
    autoTriggerEnabled: options.customerEnrichmentAutoTriggerEnabled
      ?? options.salesCrm?.customerEnrichmentAutoTriggerEnabled,
  });
  ...
  app.get('/api/sales-crm/bootstrap', (req, res) => {
    ...
    try {
      ...
      res.json({
        ok: true,
        ...payload,
        features: { aiStations: aiStationsEnabled },
  ...
  app.post('/api/sales-crm/accounts', (req, res) => {
    try { res.json({ ok: true, ...addAccount(req.salesUser, req.body || {}, { enrichmentFlags }) }); }
