  ...
  for (const account of accounts) {
    ...
    if (noPlanStreak && Number(noPlanStreak.count) >= 3) {
      ...
      add(
        ...
        `${customerLabel} · 当前负责人 ${ownerLabel} · 已连续 ${noPlanStreak.count} 次暂无计划 · 建议经理介入并协助形成明确下一步`,
        '经理介入并协助形成明确下一步',
        0,
...
function filterTodayTaskAlertsForUser(alerts, user) {
  ...
  if (canSeeManagerReasons) return alerts;
  return (Array.isArray(alerts) ? alerts : []).filter(alert => alert.code !== 'MANAGER_NEEDED');
}
...
function intakeDevelopmentHistory(value, items) {
  ...
  const rows = value.prepare(`SELECT a.id,a.company_name,a.stage,a.lifecycle_status,a.owner_id,
    a.previous_owner_id,a.updated_at,
...
function loadIntakeState(value, user, query = {}, options = {}) {
  ...
  for (const item of items) {
    ...
    const assignmentBlockReason = assignableStatus
      ? manualAssignmentBlockReason(value, item)
      : '状态不可分配';
    item.assignable = assignableStatus && !assignmentBlockReason;
    item.assignmentBlockReason = item.assignable ? '' : assignmentBlockReason;
    item.developmentHistory = developmentHistoryByItem.get(item.id) || null;
  ...
  const local = account
    ? value.prepare(`SELECT * FROM crm_account_contacts
        ...
        ORDER BY CASE match_status WHEN 'match' THEN 0 WHEN 'pending' THEN 1 ELSE 2 END,
          ...
          updated_at DESC,id DESC`).all(externalCustomerId, account.id).map(publicAccountContact)
    : [];
...
function intakeAccountsByExternalId(value, item) {
  return value.prepare(`SELECT id,lifecycle_status,recycle_kind,intake_item_id,assignment_status
...
function assignIntakeItem(value, item, ownerId, settings, reason = '') {
  if (!['pending', 'approved', 'assigned', 'returned'].includes(item.status)) return { assigned: false, reason: '状态不可分配' };
  ...
  const reusable = existingAccounts.find(account => isReturnedAccountForIntake(account, item));
  const blocking = existingAccounts.find(account => !isReturnedAccountForIntake(account, item));
  ...
  value.prepare(`UPDATE crm_intake_items SET status='assigned',crm_customer_id=?,assigned_owner_id=?,
    ...
    .run(reusable?.id || '', ownerId, reason, assignedAt, claimDue, assignedAt, item.id);
...
function createClaimedAccount(value, item, claimedAt, contactDue, claimerId = '') {
  ...
  if (existing) {
    ...
    value.prepare(`UPDATE crm_accounts SET lifecycle_status='active',recycle_kind='',recycle_reason='',
      recycled_by='',recycled_at='',previous_owner_id='',owner_id=?,assignment_status='claimed',
      intake_item_id=?,assigned_at=?,claim_due_at=?,claimed_at=?,return_reason='',updated_at=? WHERE id=?`)
      .run(item.assigned_owner_id || '', item.id, item.assigned_at || claimedAt,
        item.claim_due_at || '', claimedAt, claimedAt, existing.id);
    if (claimerId) {
...
function manualAssignmentBlockReason(value, item) {
  ...
  const existingAccounts = intakeAccountsByExternalId(value, item);
  const reusable = existingAccounts.find(account => isReturnedAccountForIntake(account, item));
  if (existingAccounts.some(account => !isReturnedAccountForIntake(account, item))
      || (item.crm_customer_id && !reusable)) return '客户已在 CRM';
  return '';
}
...
function notifyNoPlanStreak(value, account) {
  ...
  const detail = `${customerLabel} · 当前负责人 ${ownerName || '未分配'}`
    + ` · 已连续 ${streak.count} 次暂无计划 · 建议经理介入并协助形成明确下一步`;
  for (const recipient of recipients) {
    createNotification(value, {
      userId: recipient.id,
      customerId: account.id,
...
function completeManagerAssistanceTodayTask(value, user, payload, spec, identity) {
  ...
  const assistanceTask = listManagerTasks(value, {
    customerId: account.external_customer_id || account.id,
    reason: 'manager_assistance',
    statuses: ['open', 'overdue', 'escalated'],
    limit: 1,
  })[0];
  if (assistanceTask) {
    ...
    value.prepare(`INSERT INTO crm_activities
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      activityId, account.id, user.id, 'manager_join', '', '已完成', result,
      account.next_action || '', account.next_action_at || '', account.stage, account.stage,
      0, 'manager_join', '', '已完成', completedAt, completedAt,
    );
    value.prepare(`UPDATE crm_accounts SET manager_required=0,manager_status='已完成',
      ...
      user.id, completedAt, completedAt, account.id,
    );
    const taskResult = {
      action: 'manager_assistance_completed',
      result,
      activityId,
      completedAt,
    };
    value.prepare(`UPDATE crm_manager_tasks SET status='completed',result_json=?,resolved_by=?,
      resolved_at=?,updated_at=? WHERE id=? AND status IN ('open','overdue','escalated')`).run(
      JSON.stringify(taskResult), user.id, completedAt, completedAt, assistanceTask.id,
    );
    ...
    recordTodayTaskAudit(
      ...
      account.id,
      {
        requesterId: request?.requesterId || assistanceTask.actorIdSnapshot || '',
        requestedAt: request?.requestedAt || assistanceTask.triggeredAt || '',
        requestReason: request?.reason || assistanceTask.evidence?.summary || '',
        requestProgress: request?.progress || assistanceTask.evidence?.progressType || '',
        handlerId: user.id,
        result,
        completedAt,
        activityId,
...
function addActivity(user, payload, options = {}) {
  ...
  try {
    ...
    if (!stopsFollowUp && !noPlan && Boolean(nextAction) !== Boolean(nextActionAt)) {
    ...
    const transaction = value.transaction(() => {
      ...
      if (managerRequired) {
        const assistanceTask = upsertManagerTask(value, {
          ...
          completionCondition: '记录经理协助结果并形成主管介入历史',
    ...
    return value.transaction(() => {
      ...
      for (const row of rows) {
        ...
        if (!safeCandidates.length && !manualSelection && !protectedExact) {
          ...
          }, identity, 'rule_recalculation');
          releasedCount += 1;
          recordDuplicateAudit(value, user, 'duplicate_review_recalculated_released', row.id, {
          ...
          retainedCount += 1;
          recordDuplicateAudit(value, user, 'duplicate_review_recalculated_retained', row.id, {
      ...
      recordDuplicateAudit(value, user, 'duplicate_review_recalculation_completed', runId, {
...
function applyCustomerReturn(value, user, identity, account, reason, action, batchSize = null) {
  const updatedAt = nowText();
  const updated = value.prepare(`UPDATE crm_accounts SET lifecycle_status='active',recycle_kind='',
    ...
    account.owner_id || '', reason, updatedAt, account.id,
  );
  if (updated.changes !== 1) {
    throw recycleError(
      409,
...
function returnCustomer(user, customerId, payload = {}, identity = {}) {
  ...
  try {
    return value.transaction(() => {
      ...
      return { customerId: account.id, returnedToPool: true, intakeItemId: account.intake_item_id || '' };
    }).immediate();
  } finally { value.close(); }
}
...
function registerSalesCrm(app, options = {}) {
  ...
  app.post('/api/sales-crm/accounts', (req, res) => {
    ...
    try {
      ...
      const publicResult = result.reviewRequired && req.salesUser.role === 'sales'
        ? { accepted: true, message: '资料已提交，系统将继续处理。' }
        : result;
  ...
  app.get('/api/sales-crm/accounts/:customerId/history', (req, res) => {
    const value = db();
    try {
      const account = getAccountForUser(value, req.salesUser, req.params.customerId);
      res.json({ ok: true, timeline: buildAccountHistory(value, account) });
    } catch (error) { sendApiError(res, error); }
    finally { value.close(); }
  });
