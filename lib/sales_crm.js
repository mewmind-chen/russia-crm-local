...
function loadIntakeState(value, user, query = {}, options = {}) {
  ...
  const contacted = value.prepare(`SELECT COUNT(*) n FROM crm_accounts a WHERE a.intake_item_id!=''
    AND COALESCE(a.lifecycle_status,'active')='active' AND COALESCE(a.is_test_data,0)=0
...
function isReturnedAccountForIntake(account, item) {
  if (!account || String(account.lifecycle_status || 'active') !== 'recycled'
      || String(account.recycle_kind || '') !== 'sales_return') return false;
  return isCurrentIntakeAccount(account, item);
}
...
function intakeAccountsByExternalId(value, item) {
  return value.prepare(`SELECT id,lifecycle_status,recycle_kind,intake_item_id
...
function manageIntake(user, payload, options = {}) {
  ...
  try {
    ...
    if (action === 'claim') {
      ...
      let accountId = '';
      value.transaction(() => {
        accountId = createClaimedAccount(value, item, claimedAt, contactDue);
