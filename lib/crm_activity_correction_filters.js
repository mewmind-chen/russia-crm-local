...
function activityCorrectionFilterOptions(db, user, page, authorizedFields, input = {}) {
  ...
  const linkageAst = input.linkageAst || { page, filters: [] };
  if (page === FILTER_PAGES.targets && keys.has('stage')) {
    const catalogQuery = buildCorrectionTargetQuery(
      user,
      { page, filters: [] },
      input,
    );
    const catalog = db.prepare(`SELECT a.stage value,COUNT(*) count ${catalogQuery.from}
      WHERE ${catalogQuery.conditions.join(' AND ')} AND TRIM(COALESCE(a.stage,''))!=''
      GROUP BY a.stage ORDER BY a.stage`).all(...catalogQuery.params).map(row => ({
      value: row.value,
      label: row.value,
      count: Number(row.count || 0),
    }));
    const remaining = astWithoutField(linkageAst, 'stage');
    if (!remaining.filters?.length) {
      options.stage = catalog;
    } else {
      const countedQuery = buildCorrectionTargetQuery(user, remaining, input);
      options.stage = overlayOptionCounts(catalog, db.prepare(
        `SELECT a.stage value,COUNT(*) count ${countedQuery.from}
          WHERE ${countedQuery.conditions.join(' AND ')} AND TRIM(COALESCE(a.stage,''))!=''
          GROUP BY a.stage ORDER BY a.stage`,
      ).all(...countedQuery.params).map(row => ({
        value: row.value,
        label: row.value,
        count: Number(row.count || 0),
      })));
  ...
  if (page === FILTER_PAGES.proposals && keys.has('correction_status')) {
    const catalogQuery = buildActivityCorrectionProposalQuery(user, { page, filters: [] });
    const labels = { pending: '待审批', approved: '已批准', rejected: '已拒绝' };
    const catalog = db.prepare(`SELECT p.status value,COUNT(*) count ${catalogQuery.from}
      WHERE ${catalogQuery.conditions.join(' AND ')} GROUP BY p.status ORDER BY p.status`)
      .all(...catalogQuery.params).map(row => ({
        value: row.value,
        label: labels[row.value] || row.value,
        count: Number(row.count || 0),
      }));
    const remaining = astWithoutField(linkageAst, 'correction_status');
    if (!remaining.filters?.length) {
      options.correction_status = catalog;
    } else {
      const countedQuery = buildActivityCorrectionProposalQuery(user, remaining);
      options.correction_status = overlayOptionCounts(catalog, db.prepare(
        `SELECT p.status value,COUNT(*) count ${countedQuery.from}
          WHERE ${countedQuery.conditions.join(' AND ')} GROUP BY p.status ORDER BY p.status`,
      ).all(...countedQuery.params).map(row => ({
        value: row.value,
        label: labels[row.value] || row.value,
        count: Number(row.count || 0),
