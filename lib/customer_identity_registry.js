...
function identityComparisonEvidenceVersion(db, externalCustomerIds = []) {
  const ids = [...new Set((externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean))].sort(compareText);
  const rows = [];
  const collect = (table, idColumn, fields) => {
    const columns = tableColumns(db, table);
    if (!ids.length || !columns.has(idColumn)) return;
    const selected = fields.map(field => columns.has(field) ? field : `'' AS ${field}`);
    const placeholders = ids.map(() => '?').join(',');
    for (const row of db.prepare(`SELECT ${idColumn} external_customer_id,${selected.join(',')}
      FROM ${table} WHERE ${idColumn} IN (${placeholders})`).all(...ids)) {
      rows.push({
        source: table,
  ...
  collect('customer_pool', 'customer_id', ['country', 'city', 'website', 'email', 'industry']);
  collect('crm_accounts', 'external_customer_id', ['country', 'city', 'website', 'industry']);
  collect('crm_account_contacts', 'external_customer_id', ['email']);
  rows.sort((left, right) => compareText(JSON.stringify(left), JSON.stringify(right)));
  return `sha256:${digest({ externalCustomerIds: ids, rows })}`;
}

function distinctResolutionDetails(value) {
  const details = parseDetailsJson(value);
  return details.resolutionKind === 'distinct_sources'
    && /^sha256:[a-f0-9]{64}$/.test(String(details.comparisonEvidenceVersion || ''))
    ? details : null;
}

function distinctResolutionValid(db, row, currentExternalCustomerIds = []) {
  const details = distinctResolutionDetails(row.details_json);
  if (!details) return false;
  const currentIds = [...new Set((currentExternalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean))].sort(compareText);
  if (currentIds.length < 2 || !currentIds.includes(row.target_external_customer_id)) return false;
  return details.comparisonEvidenceVersion === identityComparisonEvidenceVersion(db, currentIds);
}
...
function collectMigrationCandidateAliases(db, aliases, seen, normalizedNames = null) {
  ...
  for (const row of rows) {
    ...
    for (const candidate of candidates) {
      ...
      if (normalizedNames && !normalizedNames.has(normalizeCustomerName(candidate.nickname))) {
        continue;
      }
...
function collectAliases(db, normalizedNames = null) {
  const aliases = [];
  const seen = new Set();
  for (const spec of SOURCE_SPECS) {
    ...
    for (const nameColumn of spec.names) {
      ...
      const rows = db.prepare(`SELECT ${spec.primaryKey} source_row_id,
        ...
        FROM ${spec.table}`).all();
      for (const row of rows) {
        if (normalizedNames && !normalizedNames.has(normalizeCustomerName(row.raw_name))) continue;
        addAlias(aliases, seen, {
          externalCustomerId: row.external_customer_id,
  ...
  collectMigrationCandidateAliases(db, aliases, seen, normalizedNames);
...
function crmIdentityIds(db) {
  const columns = tableColumns(db, 'crm_accounts');
  if (!columns.has('external_customer_id')) return new Set();
  return new Set(db.prepare(`SELECT DISTINCT external_customer_id
    FROM crm_accounts WHERE TRIM(COALESCE(external_customer_id,''))!=''`)
    .all().map(row => String(row.external_customer_id || '').trim()).filter(Boolean));
}

function classifyIdentityConflict(conflict, crmIds) {
  const crmExternalCustomerIds = conflict.externalCustomerIds
    .filter(customerId => crmIds.has(customerId));
  const leadExternalCustomerIds = conflict.externalCustomerIds
    .filter(customerId => !crmIds.has(customerId));
  const disposition = leadExternalCustomerIds.length && crmExternalCustomerIds.length <= 1
    ? 'lead_warning'
    : 'blocking';
  return {
    ...conflict,
    disposition,
    crmExternalCustomerIds,
    leadExternalCustomerIds,
  };
}

function publicLeadIdentityWarning(conflictId = '') {
...
function effectiveConflictResolutions(db, conflicts) {
  ...
  return db.prepare(`SELECT conflict_id,decision,resolved_source_version,
        target_external_customer_id,details_json
    ...
    .filter(row => {
      ...
      if (row.decision === 'confirm_new'
          && distinctResolutionValid(db, row, conflict.externalCustomerIds)) {
        return Boolean(db.prepare(`SELECT 1 FROM crm_customer_identity_conflict_audit
          WHERE conflict_id=? AND decision='confirm_new' AND target_external_customer_id=?
            AND source_version=? LIMIT 1`).get(
          row.conflict_id, row.target_external_customer_id, row.resolved_source_version,
        ));
      }
...
function invalidResolvedMappings(db, conflicts, aliases) {
  ...
  return db.prepare(`SELECT conflict_id,normalized_name,decision,resolved_source_version,
    ...
    .map(row => {
      if (distinctResolutionDetails(row.details_json)) return null;
...
function auditProtectedCustomerIdentities(db, options = {}) {
  ...
  const conflicts = [];
  const crmIds = crmIdentityIds(db);
  for (const [normalizedName, rows] of grouped) {
    ...
    conflicts.push(classifyIdentityConflict(snapshot, crmIds));
  }
  ...
  const effectiveIds = new Set(effectiveResolutions.map(item => item.conflictId));
  const unresolvedLive = conflicts.filter(conflict => !effectiveIds.has(conflict.conflictId));
  const leadWarningConflicts = unresolvedLive
    .filter(conflict => conflict.disposition === 'lead_warning');
  const blockingConflicts = unresolvedLive
    .filter(conflict => conflict.disposition === 'blocking');
  const blockingUnresolved = blockingConflicts.length
  ...
  return {
    ...
    reopenedTombstones,
    storedUnresolved,
    leadWarningConflicts,
    blockingConflicts,
    leadWarnings: leadWarningConflicts.length,
    blockingUnresolved,
...
function leadIdentityWarningsForExternalCustomerIds(db, externalCustomerIds = []) {
  const requested = new Set((externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean));
  const warnings = new Map();
  if (!requested.size) return warnings;
  const report = auditProtectedCustomerIdentities(db, { apply: false });
  for (const conflict of report.leadWarningConflicts) {
    for (const customerId of conflict.leadExternalCustomerIds) {
      if (requested.has(customerId)) {
        warnings.set(customerId, publicLeadIdentityWarning(conflict.conflictId));
      }
    }
  }
  return warnings;
...
// Returns the normalized names used by the given external customer ids, queried
// directly from their own source rows (no full identity audit).
function normalizedNamesForExternalIds(db, externalCustomerIds = []) {
  const requested = new Set((externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean));
  const names = new Set();
  ...
  for (const spec of SOURCE_SPECS) {
    ...
    for (const nameColumn of spec.names) {
      ...
      const rows = db.prepare(`SELECT ${spec.primaryKey} source_row_id,
        ${spec.externalCustomerId} external_customer_id, ${nameColumn} raw_name
        FROM ${spec.table} WHERE ${spec.externalCustomerId} IN (${placeholders})`)
        .all(...params);
      for (const row of rows) {
        const normalizedName = normalizeCustomerName(row.raw_name);
        if (!String(row.external_customer_id || '').trim()
            || !normalizedName
            || !String(row.source_row_id || '').trim()) continue;
        names.add(normalizedName);
      }
    }
  }
  const migration = MIGRATION_CANDIDATE_SOURCE;
  const migrationColumns = tableColumns(db, migration.table);
  if ([migration.primaryKey, migration.externalCustomerId, migration.column]
    .every(column => migrationColumns.has(column))) {
    const rows = db.prepare(`SELECT ${migration.primaryKey} source_row_id,
      ${migration.externalCustomerId} external_customer_id, ${migration.column} candidates_json
      FROM ${migration.table} WHERE ${migration.externalCustomerId} IN (${placeholders})`)
      .all(...params);
    for (const row of rows) {
      const sourceRowId = String(row.source_row_id || '').trim();
      let candidates;
      ...
      if (!Array.isArray(candidates)) throw invalidMigrationCandidates(sourceRowId);
      for (const candidate of candidates) {
        if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)
          || ['accountId', 'nickname', 'updatedAt', 'createdAt']
            .some(key => typeof candidate[key] !== 'string')
        ...
        const normalizedName = normalizeCustomerName(candidate.nickname);
...
// Single-conflict equivalent of the effective-resolution filter used by the full
// audit, so a scoped lookup can skip resolved lead warnings identically.
function conflictEffectivelyResolved(db, conflict) {
  if (!hasTable(db, 'crm_customer_identity_conflicts')
      || !hasTable(db, 'crm_customer_identity_registry')
      || !hasTable(db, 'crm_customer_identity_conflict_audit')) return false;
  const columns = tableColumns(db, 'crm_customer_identity_conflicts');
  if (!['conflict_id', 'status', 'decision', 'resolved_source_version',
    'target_external_customer_id'].every(column => columns.has(column))) return false;
  const row = db.prepare(`SELECT conflict_id,decision,resolved_source_version,
      target_external_customer_id,details_json
    FROM crm_customer_identity_conflicts
    WHERE conflict_id=? AND status='resolved' AND decision IN ('link_existing','confirm_new')
    LIMIT 1`).get(conflict.conflictId);
  if (!row) return false;
  if (conflict.expectedVersion !== row.resolved_source_version) return false;
  if (activeNameTombstone(db, conflict.normalizedName)) return false;
  if (row.decision === 'confirm_new'
      && distinctResolutionValid(db, row, conflict.externalCustomerIds)) {
    return Boolean(db.prepare(`SELECT 1 FROM crm_customer_identity_conflict_audit
  ...
  const owner = db.prepare(`SELECT external_customer_id
    FROM crm_customer_identity_registry WHERE normalized_name=?`).get(conflict.normalizedName);
  if (!owner || owner.external_customer_id !== row.target_external_customer_id) return false;
  return Boolean(db.prepare(`SELECT 1 FROM crm_customer_identity_conflict_audit
    WHERE conflict_id=? AND decision=? AND target_external_customer_id=? AND source_version=? LIMIT 1`)
    .get(
      row.conflict_id,
      row.decision,
      row.target_external_customer_id,
...
// Scoped lead-identity warning lookup: computes warnings only for the requested
// external ids without running the full protected-identity audit. Results are
// identical to leadIdentityWarningsForExternalCustomerIds for those ids.
function leadIdentityWarningsForExternalIds(db, externalCustomerIds = []) {
  const requested = new Set((externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean));
  const warnings = new Map();
  if (!requested.size) return warnings;

  const requestedNames = normalizedNamesForExternalIds(db, [...requested]);
  if (!requestedNames.size) return warnings;

  const aliases = collectAliases(db, requestedNames);
  const grouped = new Map();
  for (const alias of aliases) {
    const values = grouped.get(alias.normalizedName) || [];
    values.push(alias);
  ...
  for (const [normalizedName, rows] of grouped) {
    const snapshot = identitySourceSnapshot(rows, normalizedName);
    if (snapshot.externalCustomerIds.length < 2) continue;
    const conflict = classifyIdentityConflict(snapshot, crmIds);
    if (conflict.disposition !== 'lead_warning') continue;
    if (conflictEffectivelyResolved(db, conflict)) continue;
    for (const customerId of conflict.leadExternalCustomerIds) {
      if (requested.has(customerId)) {
        warnings.set(customerId, publicLeadIdentityWarning(conflict.conflictId));
      }
...
// ---- Identity conflict resolution passthrough (issue #306) -------------------
...
function supplementRequirementText(detailsJson) {
  const details = parseDetailsJson(detailsJson);
  return typeof details.reason === 'string' ? details.reason.trim() : '';
}

function leadSupplementFields(db, externalCustomerId) {
  const fields = { contactName: '', website: '', industry: '' };
  if (!hasTable(db, 'crm_intake_items')) return fields;
  const columns = tableColumns(db, 'crm_intake_items');
  if (!['external_customer_id', 'contact_name', 'website', 'industry']
    .every(column => columns.has(column))) return fields;
  const row = db.prepare(`SELECT contact_name,website,industry
    FROM crm_intake_items WHERE external_customer_id=?
    ORDER BY updated_at DESC,id DESC LIMIT 1`).get(externalCustomerId);
...
function masterSupplementState(db, externalCustomerId) {
  const state = { accountId: '', companyName: '', website: '', industry: '', hasContacts: false };
  if (!hasTable(db, 'crm_accounts')) return state;
  const columns = tableColumns(db, 'crm_accounts');
  if (!['external_customer_id', 'company_name', 'website', 'industry']
    .every(column => columns.has(column))) return state;
  const account = db.prepare(`SELECT id,company_name,website,industry
    FROM crm_accounts WHERE external_customer_id=?
    ORDER BY updated_at DESC,id DESC LIMIT 1`).get(externalCustomerId);
  if (!account) return state;
  state.accountId = String(account.id || '');
  state.companyName = String(account.company_name || '').trim();
  state.website = String(account.website || '').trim();
  state.industry = String(account.industry || '').trim();
  if (hasTable(db, 'crm_account_contacts')) {
    const contactColumns = tableColumns(db, 'crm_account_contacts');
    if (['customer_id', 'external_customer_id', 'archived_at']
      .every(column => contactColumns.has(column))) {
      state.hasContacts = Boolean(db.prepare(`SELECT 1 FROM crm_account_contacts
        WHERE (customer_id=? OR external_customer_id=?)
          AND COALESCE(archived_at,'')='' LIMIT 1`).get(state.accountId, externalCustomerId));
    }
  }
  return state;
}

function identityConflictComplementaryInfo(db, leadExternalCustomerId, masterExternalCustomerId) {
  const lead = leadSupplementFields(db, leadExternalCustomerId);
  const master = masterSupplementState(db, masterExternalCustomerId);
  const info = {};
  if (lead.contactName && !master.hasContacts) info.contact = true;
  if (lead.website && lead.website !== master.website) info.website = true;
  if (lead.industry && !master.industry) info.industry = true;
  return Object.keys(info).length ? info : null;
}

// Scoped lookup of identity-conflict resolutions for the requested external ids.
// Unlike the full audit, this reads only the stored conflict rows (no registry
// audit) so it can be called once per intake list without a full preflight pass.
function identityConflictResolutionsForExternalIds(db, externalCustomerIds = []) {
  const requested = new Set((externalCustomerIds || [])
    .map(value => String(value || '').trim()).filter(Boolean));
  const resolutions = new Map();
  if (!requested.size || !hasTable(db, 'crm_customer_identity_conflicts')) return resolutions;
  const columns = tableColumns(db, 'crm_customer_identity_conflicts');
  if (!['conflict_id', 'status', 'decision', 'target_external_customer_id',
    'latest_external_customer_ids_json', 'details_json', 'updated_at']
    .every(column => columns.has(column))) return resolutions;
  const rows = db.prepare(`SELECT conflict_id,status,decision,target_external_customer_id,
      latest_external_customer_ids_json,details_json
    FROM crm_customer_identity_conflicts
    WHERE status IN ('resolved','retry')
    ORDER BY updated_at DESC, conflict_id`).all();
  for (const row of rows) {
    const externalIds = parseJsonArray(row.latest_external_customer_ids_json);
    const masterExternalCustomerId = String(row.target_external_customer_id || '').trim();
    if (row.status === 'resolved' && row.decision === 'link_existing'
        && masterExternalCustomerId) {
      const master = masterSupplementState(db, masterExternalCustomerId);
      for (const externalId of externalIds) {
        if (externalId === masterExternalCustomerId || !requested.has(externalId)) continue;
        resolutions.set(externalId, {
          decision: 'link_existing',
          linkedMasterName: master.companyName,
          linkedMasterExternalId: masterExternalCustomerId,
          supplementRequirement: '',
          complementaryInfo: identityConflictComplementaryInfo(
            db, externalId, masterExternalCustomerId,
          ),
        });
      }
    } else if (row.status === 'retry' && row.decision === 'supplement_and_retry') {
      const supplementRequirement = supplementRequirementText(row.details_json);
      for (const externalId of externalIds) {
        if (!requested.has(externalId)) continue;
        resolutions.set(externalId, {
          decision: 'retry',
          linkedMasterName: '',
          linkedMasterExternalId: '',
          supplementRequirement,
...
module.exports = {
  ...
  identityComparisonEvidenceVersion,
