...
// Dynamic account/commerce/timeline columns may arrive as JSON text rather
// than hydrated objects. These common contact aliases are intentionally kept
// separate from the legacy key set: only the explicitly opted-in dynamic
// boundary uses them, so existing AI and legacy redaction call sites retain
// their exact behavior until their own shape contracts are reviewed.
const DYNAMIC_CONTACT_ALIAS_KEYS = new Set([
  'emailaddress', 'phonenumber', 'mobilephone', 'telephone', 'tel', 'mobile',
  'contactemail', 'contactphone', 'contactinfo', 'contactdetails',
  'personemail', 'personphone', 'personalemail', 'personalphone',
  'workemail', 'workphone', 'emailvalue', 'phonevalue',
]);
...
const CONTACT_SAFE_POOL_KEYS = new Set([
  ...
  'website', 'industry', 'customertype', 'establishedyear', 'rating', 'currentpool', 'inn', 'riskstatus',
...
// Legacy `customers` rows are rendered as the camelCase shape produced by
// buildCustomer(). Keep the exact business/status fields that the recursive
// CONTACT_KEYS blacklist leaves visible, while excluding contact narratives
// (email/phone/contact/products/reason/feedback/nextAction/notes, etc.). Tags
// are the only nested value on this shape and receive their own projection so
// a future tag field cannot bypass the field-level boundary.
const CONTACT_SAFE_CUSTOMER_TAG_KEYS = new Set([
  'id', 'name', 'category', 'color', 'ispreset', 'createdat', 'readonly',
]);
const CONTACT_SAFE_CUSTOMER_ROW_KEYS = new Set([
  'rownumber', 'followid', 'customerid', 'companyname', 'website',
  'customertype', 'industry', 'rating', 'owner', 'assigneddate', 'status',
  'firstcontactdate', 'lastfollowdate', 'channel', 'nextfollowdate',
  'statusgroup', 'nextfollowdatekey', 'isduetoday', 'isoverdue', 'isrisk',
  'riskreasons', 'tags',
]);

// P1/P3 intake-state is a mixed aggregate rather than a flat list row. Keep
// the legacy recursive blacklist as the default rule, but add the compound
// activity key and common contact aliases that can occur in dynamic decision
// JSON. These keys are local to the intake aggregate so other payloads (for
// example pipeline's `latestActivitySummary`) retain their existing contract.
const INTAKE_AGGREGATE_CONTACT_KEYS = new Set([
  ...CONTACT_KEYS,
  'lastactivitysummary',
  'supplementpendingjson',
  'emailaddress', 'phonenumber', 'mobilephone',
  'contactemail', 'contactphone', 'contactinfo', 'contactdetails',
  'personemail', 'personphone', 'personalemail', 'personalphone',
]);

// `supplement_pending_json` is persisted JSON and therefore cannot be treated
// as a trusted shape. The only non-contact flags currently consumed by the
// intake UI are booleans indicating missing website/industry data. `contact`
// remains intentionally hidden by the contact boundary; every unknown key,
// nested object, array, and non-boolean value is dropped by this projection.
const INTAKE_COMPLEMENTARY_SAFE_KEYS = new Set(['website', 'industry']);

// `developmentHistory` is assembled by the server and has a deliberately
// small metadata shape. Projecting this shape prevents a future enrichment
// field from bypassing the explicit `lastActivitySummary` redaction.
const INTAKE_DEVELOPMENT_HISTORY_SAFE_KEYS = new Set([
  'accountid', 'companyname', 'stage', 'recycled', 'previousownerid',
  'activitycount', 'rfqcount', 'quotecount', 'ordercount',
  'lastactivityat', 'lastactivitytype',
]);

const INTAKE_IDENTITY_WARNING_SAFE_KEYS = new Set([
  'active', 'code', 'label', 'message', 'conflictid',
]);

// Export is a separate security boundary from contact visibility. The export
// aggregate includes several `SELECT *` shapes and is available to an
// explicitly authorized user, so credentials must be removed even when the
// caller has `view_contacts`. Matching is performed on normalized keys so
// snake_case/camelCase/vendor aliases receive the same fail-closed treatment.
const EXPORT_CREDENTIAL_KEYS = new Set([
  'password', 'passwords', 'passwordhash', 'passwordsalt',
  'passwd', 'passphrase', 'passhash', 'passsalt',
  'token', 'tokens', 'tokenhash', 'accesstoken', 'refreshtoken', 'csrftoken',
  'session', 'sessions', 'sessionid', 'sessionhash', 'sessiontoken', 'salessessions',
  'secret', 'secrets', 'secretkey', 'clientsecret', 'apisecret',
  'apikey', 'apikeys', 'privatekey', 'encryptionkey',
  'credential', 'credentials', 'authorization', 'cookie', 'cookies',
  'salt', 'previewtoken', 'previewtokenhash',
]);

function isExportCredentialKey(key) {
  const normalized = normalizedRedactionKey(key);
  if (!normalized) return false;
  if (EXPORT_CREDENTIAL_KEYS.has(normalized)) return true;
  // Match composed aliases such as authToken, sessionTokenHash,
  // credentialsJson, or setCookie while avoiding ordinary words such as
  // secretary and tokenCount.
  return /(?:password|passwd|passphrase|token|session|secret|credential|authorization|cookie|apikey|apisecret|privatekey|encryptionkey|previewtoken)(?:s|hash|salt|token|secret|key|value|id|json|metadata|data|digest|encoded|encrypted|header|material)*$/.test(normalized);
}
...
const CONTACT_SAFE_ACCOUNT_KEYS = new Set([
  ...
  'lifecyclestatus', 'recyclekind', 'recycledby',
...
function redactContactJsonText(value, options) {
  ...
  const redacted = redactContactFields(parsed, options);
  // Keep untouched JSON text byte-for-byte; only re-serialize when a contact
  // key was actually removed from the parsed object/array.
...
function redactContactFields(value, options = {}) {
  const contactKeys = options.contactKeys || CONTACT_KEYS;
  if (Array.isArray(value)) return value.map(child => redactContactFields(child, options));
  if (typeof value === 'string' && options.parseJsonText === true) {
    return redactContactJsonText(value, options);
  }
  ...
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => {
      ...
      return !contactKeys.has(normalizedKey)
        || (preserveAlertCopy && ALERT_COPY_KEYS.has(normalizedKey));
    })
    .map(([key, child]) => [key, redactContactFields(child, options)]));
}

// Composite profile/recycle/bootstrap callers use this boundary only after
// their resource and permission checks. It protects both hydrated dynamic
// descendants and JSON object/array text while preserving authorized raw
// shapes and untouched JSON text. This is deliberately not used by AI routes.
function redactContactDynamicFields(value, options = {}) {
  const contactKeys = new Set([...CONTACT_KEYS, ...DYNAMIC_CONTACT_ALIAS_KEYS]);
  return redactContactFields(value, { ...options, contactKeys, parseJsonText: true });
}

function normalizedRedactionKey(key) {
  return String(key || '').toLowerCase().replace(/[^a-z0-9]/g, '');
}

function redactExportCredentialJsonString(value) {
  const trimmed = String(value || '').trim();
  if (!trimmed || !(
    (trimmed.startsWith('{') && trimmed.endsWith('}'))
    || (trimmed.startsWith('[') && trimmed.endsWith(']'))
  ...
  let parsed;
  try { parsed = JSON.parse(value); } catch (_error) { return value; }
  const redacted = redactExportCredentials(parsed);
  // Preserve untouched JSON strings byte-for-byte; only re-serialize when a
  // nested credential key was actually removed.
  return JSON.stringify(parsed) === JSON.stringify(redacted)
    ? value
    : JSON.stringify(redacted);
}

// Export credential projection is intentionally recursive and independent of
// `view_contacts`: users with export permission may still never receive
// password/token/session/secret material. JSON-encoded object/array strings
// are inspected as well because several legacy tables persist dynamic JSON in
// TEXT columns and export them through `SELECT *`.
function redactExportCredentials(value) {
  if (Array.isArray(value)) return value.map(child => redactExportCredentials(child));
  if (typeof value === 'string') return redactExportCredentialJsonString(value);
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !isExportCredentialKey(key))
    .map(([key, child]) => [key, redactExportCredentials(child)]));
}

function projectIntakeComplementaryInfo(value) {
  if (value === null || value === undefined) return value;
  if (typeof value !== 'object' || Array.isArray(value)) return {};
  return Object.fromEntries(Object.entries(value)
    .filter(([key, child]) => INTAKE_COMPLEMENTARY_SAFE_KEYS.has(normalizedRedactionKey(key))
      && typeof child === 'boolean')
    .map(([key, child]) => [key, child]));
}

function projectIntakeDevelopmentHistory(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  return Object.fromEntries(Object.entries(value)
    .filter(([key, child]) => INTAKE_DEVELOPMENT_HISTORY_SAFE_KEYS.has(normalizedRedactionKey(key))
      && (child === null || typeof child !== 'object'))
    .map(([key, child]) => [key, child]));
}

function projectIntakeIdentityWarning(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  return Object.fromEntries(Object.entries(value)
    .filter(([key, child]) => INTAKE_IDENTITY_WARNING_SAFE_KEYS.has(normalizedRedactionKey(key))
      && (child === null || typeof child !== 'object'))
    .map(([key, child]) => [key, child]));
}

// P1/P3 are intentionally handled as a recursive shape boundary, not as a
// top-level whitelist. Arbitration and assignment-audit JSON may evolve, so
// every descendant is visited and normalized sensitive aliases are removed;
// `complementaryInfo` and `developmentHistory` additionally use their strict
// shape projections above. This keeps AI/rule producers untouched while making
// the contact-restricted read path fail closed for the two residual shapes.
function redactIntakeAggregate(value) {
  if (Array.isArray(value)) return value.map(child => redactIntakeAggregate(child));
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !INTAKE_AGGREGATE_CONTACT_KEYS.has(normalizedRedactionKey(key)))
    .map(([key, child]) => {
      const normalizedKey = normalizedRedactionKey(key);
      if (normalizedKey === 'complementaryinfo') {
        return [key, projectIntakeComplementaryInfo(child)];
      }
      if (normalizedKey === 'developmenthistory') {
        return [key, projectIntakeDevelopmentHistory(child)];
      }
      if (normalizedKey === 'identitywarning') {
        return [key, projectIntakeIdentityWarning(child)];
      }
      return [key, redactIntakeAggregate(child)];
    }));
}
...
function contactSafeCustomerTagRecord(value) {
  return contactSafeRecord(value, CONTACT_SAFE_CUSTOMER_TAG_KEYS);
}

function contactSafeCustomerRecord(value) {
  const safe = contactSafeRecord(value, CONTACT_SAFE_CUSTOMER_ROW_KEYS);
  if (Array.isArray(safe.tags)) {
    safe.tags = safe.tags.map(contactSafeCustomerTagRecord);
  } else if (safe.tags && typeof safe.tags === 'object') {
    safe.tags = contactSafeCustomerTagRecord(safe.tags);
  }
  return safe;
}
