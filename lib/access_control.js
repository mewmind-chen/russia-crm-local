const PERMISSION_DEFINITIONS = {
  ...
  manage_ai_governance: '管理AI运行审计与治理',
...
const ROLE_PERMISSIONS = {
  ...
  manager: {
    ...
    manage_ai_budgets: false, review_ai_tasks: true, manage_ai_governance: false,
  ...
  sales: {
    ...
    manage_ai_budgets: false, review_ai_tasks: false, manage_ai_governance: false,
    manage_users: false, manage_data_maintenance: false,
    export_data: false,
  },
};
...
const LEGACY_ROUTE_POLICIES = Object.freeze({
  ...
  'GET /assistant/runtime': { permissions: ['manage_ai_governance'], realAdminOnly: true },
  'PATCH /assistant/runtime': {
    permissions: ['manage_ai_governance'], realAdminOnly: true, blockedWhileImpersonating: true,
  },
...
const SALES_ROUTE_POLICIES = Object.freeze({
  ...
  'GET /ai/tasks': { permissions: ['view_customers', 'review_ai_tasks'] },
  'GET /ai/tasks/:taskId': { permissions: ['view_customers', 'review_ai_tasks'] },
  ...
  'GET /ai/budgets': {
    permissions: ['manage_ai_budgets'], realAdminOnly: true, blockedWhileImpersonating: true,
  },
...
function permissionsFor(user) {
  const source = user?.permissions && typeof user.permissions === 'object' && !Array.isArray(user.permissions)
    ? user.permissions
    : {};
  return Object.fromEntries(Object.keys(PERMISSION_DEFINITIONS).map(key => [
    key,
    Boolean(source[key]) && (key !== 'manage_ai_governance' || user?.role === 'admin'),
  ]));
}
...
function buildAccessContext(db, user) {
  ...
  const hasLifecycle = db.prepare('PRAGMA table_info(crm_accounts)').all()
    .some(column => column.name === 'lifecycle_status');
  ...
  const rows = permissions.view_all_customers
    ? permissions.manage_intake
      ? db.prepare(`SELECT id,external_customer_id FROM crm_accounts WHERE 1=1${activeClause}`).all()
      : db.prepare(`SELECT id,external_customer_id FROM crm_accounts WHERE owner_id IS NOT NULL${activeClause}`).all()
    : db.prepare(`SELECT id,external_customer_id FROM crm_accounts
        WHERE owner_id=? AND COALESCE(assignment_status,'')!='returned'
          ${hasLifecycle ? "AND COALESCE(lifecycle_status,'active')='active'" : ''}`).all(user.id);
  return {
    user,
    permissions,
