...
// account fields must be added here explicitly; they never leak by default.
const CONTACT_SAFE_ACCOUNT_KEYS = new Set([
  'id', 'externalcustomerid', 'companyname', 'nickname', 'country', 'city', 'website', 'industry',
  'customertype', 'source', 'priority', 'potentialvalue', 'stage', 'ownerid',
...
function contactSafeAccountRecord(value) {
  if (Array.isArray(value)) return value.map(child => contactSafeAccountRecord(child));
  if (!value || typeof value !== 'object') return value;
  const safe = contactSafeRecord(value, CONTACT_SAFE_ACCOUNT_KEYS);
