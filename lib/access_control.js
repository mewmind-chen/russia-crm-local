...
const CONTACT_KEYS = new Set([
  ...
  'methods_summary', 'full_name', 'full_name_local', 'title', 'person_summary',
  'contact_summary', 'evidence', 'evidence_urls', 'method', 'methods',
]);
...
const LEGACY_ACTION_POLICIES = Object.freeze({
  ...
  'prospect-agent': {
    ...
    runTask: { permissions: ['use_prospect_agent'] },
...
function policyForLegacyRequest(method, requestPath, action = '') {
  const path = String(requestPath || '').split('?')[0];
  ...
  if (String(method).toUpperCase() === 'POST' && path === '/prospect-agent') {
    return LEGACY_ACTION_POLICIES['prospect-agent'][action] || { deny: true };
  }
