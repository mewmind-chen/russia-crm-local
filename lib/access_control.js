...
const SALES_ROUTE_POLICIES = Object.freeze({
  ...
  'PATCH /intake/settings': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
  'GET /intake/assignment-rules': { permissions: ['view_intake', 'manage_intake'] },
  'GET /intake/assignment-rules/versions': { permissions: ['view_intake', 'manage_intake'] },
  'POST /intake/assignment-rules/simulate': { permissions: ['view_intake', 'manage_intake'] },
  'POST /intake/assignment-rules/draft': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
  'PATCH /intake/assignment-rules/:ruleId': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
  'POST /intake/assignment-rules/reorder': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
  'POST /intake/assignment-rules/publish': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
  'POST /intake/assignment-rules/versions/:versionId/restore': {
    permissions: ['view_intake', 'manage_intake'],
    realAdminOnly: true,
    blockedWhileImpersonating: true,
  },
...
function salesRouteKey(method, requestPath) {
  ...
  if (/^\/intake\/assignment-rules\/versions\/[^/]+\/restore$/.test(path)) {
    return `${verb} /intake/assignment-rules/versions/:versionId/restore`;
  }
  if (/^\/intake\/assignment-rules\/[^/]+$/.test(path)
      && !['draft', 'reorder', 'publish', 'simulate', 'versions'].includes(path.split('/').pop())) {
    return `${verb} /intake/assignment-rules/:ruleId`;
  }
