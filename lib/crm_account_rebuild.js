...
function rebuildAccountDerivedState(db, accountId, options = {}) {
  ...
  if (changed) {
    ...
    const updatedAt = updateColumns.has('updated_at')
      ? new Date(nowMs).toISOString().slice(0, 19).replace('T', ' ')
      : undefined;
    if (updateColumns.has('stage') && isValidStage(stage)) {
      applyAccountStatePatch(db, account.id, { stage, updatedAt });
    }
    if (updateColumns.has('next_action') && updateColumns.has('next_action_at')) {
      applyAccountPlanPatch(db, account.id, {
        nextAction,
        nextActionAt,
        timeBasis: nextBasis,
        updatedAt,
    ...
    if (updateColumns.has('manager_status')) {
      applyManagerStatusPatch(db, account.id, {
        required: managerState.required,
        status: managerState.status,
        managerId: managerState.managerId,
        updatedAt,
      });
    }
    if (updateColumns.has('last_activity_at')) {
      db.prepare('UPDATE crm_accounts SET last_activity_at=? WHERE id=?')
        .run(lastActivityAt, account.id);
    }
  }
  return result;
}
