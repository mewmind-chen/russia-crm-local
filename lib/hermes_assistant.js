...
function effectiveHermesTimeout(options = {}) {
  const config = hermesConfig();
  return boundedInteger(options.timeoutMs, config.timeoutMs, 1000, 180000);
}
...
async function callHermes(messages, options = {}) {
  const config = hermesConfig();
  const effectiveTimeout = effectiveHermesTimeout(options);
      ...
      execFile(config.bin, args, {
        ...
        if (error) {
          const wrapped = new Error(
            error.killed
              ? `Hermes 请求超过 ${Math.round(effectiveTimeout / 1000)} 秒，已停止。`
