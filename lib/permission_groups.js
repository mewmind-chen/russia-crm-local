...
function strictPermissionMap(value) {
  ...
  for (const [key, allowed] of Object.entries(value)) {
    if (!PERMISSION_DEFINITIONS[key]) throw httpError(400, `未知权限：${key}`);
...
function replaceUserOverrides(db, actor, userId, overrideMap) {
  assertPermissionManager(actor);
  const overrides = normalizeOverrideMap(overrideMap);
  if (!db.prepare('SELECT 1 FROM sales_users WHERE id=?').get(userId)) throw httpError(404, '用户不存在');
  const transaction = db.transaction(() => {
    ...
    for (const [permission, effect] of Object.entries(overrides)) {
      if (effect !== 'inherit') insert.run(userId, permission, effect, now, now);
    }
    assertValidAdminRemains(db);
  });
  transaction();
