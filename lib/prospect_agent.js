...
async function runProspectTask(taskId) {
  const task = getProspectTask(taskId);
  markProspectTaskRunning(taskId);
  try {
    const local = localProspectSearch(task).map(item => ({
      ...item,
    ...
    return markProspectTaskFailed(taskId, e.message || String(e));
  }
}
