...
function installFilterAuthorization(db, options = {}) {
  ...
  const install = db.transaction(() => {
    ...
    const managerPagesMigrationKey = 'issue170-manager-pages-v1';
    const managerPagesCatalogUpgrade = Boolean(existingState && !db.prepare(
      'SELECT 1 FROM filter_catalog_migrations WHERE migration_key=?',
    ).get(managerPagesMigrationKey));
    const managerMetricRecipientMigrationKey = 'issue196-manager-metric-recipient-v1';
    const managerMetricRecipientUpgrade = Boolean(existingState && !db.prepare(
      'SELECT 1 FROM filter_catalog_migrations WHERE migration_key=?',
    ).get(managerMetricRecipientMigrationKey));
    const teamStatusPagesMigrationKey = 'issue174-team-status-pages-v1';
    const teamStatusPagesUpgrade = Boolean(existingState && !db.prepare(
      'SELECT 1 FROM filter_catalog_migrations WHERE migration_key=?',
    ).get(teamStatusPagesMigrationKey));
    const removeAlertUrgencyMigrationKey = 'issue225-remove-alert-urgency-v1';
    const removeAlertUrgencyUpgrade = Boolean(existingState && !db.prepare(
      'SELECT 1 FROM filter_catalog_migrations WHERE migration_key=?',
    ).get(removeAlertUrgencyMigrationKey));
    ...
    const safeSalesDefaults = new Set([
      ...
      'unassigned_only', 'created_today', 'claim_overdue', 'intake_flow',
      'due_status', 'due_at',
      'evaluation_status', 'recycle_kind', 'recycled_at',
      'task_status', 'task_reason', 'task_due_at', 'task_resolved_at',
      'metric_window', 'notification_status', 'notification_code',
      'notification_severity', 'progress_kind', 'collaboration_status',
      'collaboration_source', 'collaboration_relation',
    ]);
    ...
    const insertedDefinitions = FILTER_DEFINITIONS.filter(item => !beforeKeys.includes(item.key));
    if (!existingState) {
      ...
      db.prepare(`INSERT OR IGNORE INTO filter_catalog_migrations
        (migration_key,applied_at) VALUES (?,?)`).run(managerPagesMigrationKey, now);
      db.prepare(`INSERT OR IGNORE INTO filter_catalog_migrations
        (migration_key,applied_at) VALUES (?,?)`).run(managerMetricRecipientMigrationKey, now);
      db.prepare(`INSERT OR IGNORE INTO filter_catalog_migrations
        (migration_key,applied_at) VALUES (?,?)`).run(teamStatusPagesMigrationKey, now);
    }
    let migratedPages = 0;
    let migratedGrants = existingState && insertedDefinitions.length
      ? grantDefaults(insertedDefinitions)
      : 0;
    ...
    if (managerMetricRecipientUpgrade) {
      const row = db.prepare(
        "SELECT pages_json FROM filter_definitions WHERE filter_key='recipient'",
      ).get();
      if (row) {
        const currentPages = parseJson(row.pages_json, []);
        const nextPages = [...new Set([...currentPages, 'manager_metrics'])];
        if (!sameValues(currentPages, nextPages)) {
          migratedPages += db.prepare(`UPDATE filter_definitions
            SET pages_json=?,updated_at=? WHERE filter_key='recipient'`)
            .run(JSON.stringify(nextPages), now).changes;
        }
      }
      migratedGrants += grantDefaults(FILTER_DEFINITIONS.filter(item => item.key === 'recipient'));
      db.prepare(`INSERT INTO filter_catalog_migrations
        (migration_key,applied_at) VALUES (?,?)`).run(managerMetricRecipientMigrationKey, now);
    }
    if (teamStatusPagesUpgrade) {
      const newPages = new Set(['team_status_progress', 'team_status_collaboration']);
      const updatePages = db.prepare(`UPDATE filter_definitions
        SET pages_json=?,updated_at=? WHERE filter_key=?`);
      for (const definition of FILTER_DEFINITIONS) {
        const row = db.prepare(
    ...
    if (removeAlertUrgencyUpgrade) {
      const disabled = db.prepare(`UPDATE filter_definitions
        SET enabled=0,displayed=0,updated_at=? WHERE filter_key='urgency'`).run(now).changes;
      db.prepare(`INSERT OR IGNORE INTO filter_catalog_migrations
        (migration_key,applied_at) VALUES (?,?)`).run(removeAlertUrgencyMigrationKey, now);
      if (disabled) {
        const version = bumpVersion(db, now);
        auditChange(db, { id: 'system' }, {
          action: 'definition_disabled',
          targetType: 'filter_definition',
          targetId: 'urgency',
          before: { enabled: true, displayed: true },
          after: { enabled: false, displayed: false },
          note: 'issue225 remove duplicate alert urgency filter',
          version,
          now,
        });
      }
...
function pageAllowed(user, page) {
  if (page === 'recycle_bin') {
    return hasPermission(user, 'manage_customer_recycle')
      || hasPermission(user, 'view_own_mismatch_history');
  }
...
function effectiveFilterSchemaFor(db, user, page) {
  ...
  const filters = listFilterDefinitions(db)
    ...
    .filter(item => user?.role !== 'sales' || item.key !== 'source_batch')
